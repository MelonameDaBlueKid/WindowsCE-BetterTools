        ...
        case WM_INITDIALOG:
            {
				...
				if(iRetVal!=0)
				{
					StringCchPrintf(szMessage, sizeof(szMessage), L"%d: Server socket failed.", iRetVal); 
					SendMessage(GetDlgItem(hDlg, IDC_MESSAGES), LB_ADDSTRING,0,(LPARAM)szMessage);					
				}
				else
				{
					StringCchPrintf(szMessage, sizeof(szMessage), L"You are now ready to receive messages.");
					SendMessage(GetDlgItem(hDlg, IDC_MESSAGES), LB_ADDSTRING,0,(LPARAM)szMessage);		
				}
...
DWORD WINAPI ShowDevices(LPVOID voidDlg)
{
	...
	{
		StringCchPrintf(szMessage, sizeof(szMessage), L"%d: Scanning failed.", iRetVal); 
		SendMessage(GetDlgItem(hDlg, IDC_MESSAGES), LB_ADDSTRING,0,(LPARAM)szMessage);
...
void SendData(HWND hDlg)
{
	...
	StringCchPrintf(szMessage, sizeof(szMessage), L"%s>%s", g_pLocalDeviceInfo->szDeviceNameAddr, szDlgText);
	...
	if(iRetVal)
	{
		StringCchPrintf(szMessage, sizeof(szMessage), L"Could not deliver to %s. Socket error: %d", szPeerDeviceName, iRetVal);
		SendMessage(GetDlgItem(hDlg, IDC_MESSAGES), LB_ADDSTRING,0,(LPARAM) szMessage);
	}
	else
	{
