...
#ifdef _WIN32_WCE
  hMenu = (HMENU)SendMessage((hWndCB), SHCMBM_GETSUBMENU, (WPARAM)0, 
...
// For WinCE the command bar is a control on the window client area, though it
...
//  client area smaller by the amount of the command bar.
  ...
  lpSize.top += rect.bottom - rect.top;
...
HWND    hWindow)
{
  flashtimes = 0;
  count = 4;
  SetTimer(hWindow, 366, 200, (WNDPROC)InverseMessage);    /* Timer ID is 366 */
}
#endif
...
TCHAR           *pchPostStr)
{
  ...
  if (n > 0)
  {
    StringCchPrintf(strBuf, sizeof(strBuf)/sizeof(strBuf[0]), _T("%s%d"), pS, n);
  }
  else
  {
    StringCchCopy(strBuf, sizeof(strBuf)/sizeof(strBuf[0]), pS);
  }

  if (pchPostStr)
  {
    StringCchCat(strBuf, sizeof(strBuf)/sizeof(strBuf[0]), pchPostStr);
  }
  ...
  switch (idval)
    {
      ...
      case MN_HELP_ABOUT:
          DialogBox(hInst, MAKEINTRESOURCE(3), hWindow, (DLGPROC)AboutDlgProc);
          break;
...

{
  ...
  hbrPat   = (HBRUSH)GetStockObject(LTGRAY_BRUSH);
  ...
  COLOR = (UINT)GetDeviceCaps(hdc, NUMCOLORS) > 4U;
...
int APIENTRY WinMain(
  ...
  hWnd = FindWindow(szReversi, fCheated ? (LPTSTR)szReversiPractice : (LPTSTR)szReversi);  
  if (hWnd) 
  {
  ...
  hWnd = CreateWindow((LPTSTR)TEXT("Reversi"),    // If target is NT, create a small window
                        fCheated ? (LPTSTR)szReversiPractice : (LPTSTR)szReversi,// to emulate WinCE window size.
                        WS_POPUP,
                        0,
                        0,
                        CW_USEDEFAULT,
