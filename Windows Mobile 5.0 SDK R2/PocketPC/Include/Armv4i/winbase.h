...
CeGetCanonicalPathNameW(
    IN LPCWSTR lpPathName,
    __out_ecount(cchCanonicalPathName) LPWSTR lpCanonicalPathName,
    IN DWORD cchCanonicalPathName,
    IN DWORD dwReserved
    );
#ifdef UNICODE
...
WINBASEAPI BOOL WINAPI DeviceIoControl (HANDLE hDevice, DWORD dwIoControlCode, LPVOID lpInBuf, DWORD nInBufSize, __out_bcount_opt(nOutBufSize) LPVOID lpOutBuf, DWORD nOutBufSize, LPDWORD lpBytesReturned, LPOVERLAPPED lpOverlapped);
...
BOOL CeGenRandom(DWORD dwLen, __out_bcount(dwLen) PBYTE pbBuffer);
