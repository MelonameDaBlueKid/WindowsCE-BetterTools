                      ...
                      int       nCmdShow)
{
  ...
  retCode = DialogBox ((HANDLE)hInst, (LPCTSTR)TEXT("MonkeyDlg"),
...
int APIENTRY MonkeyDlgProc ( HWND  hDlg,UINT uMsg, WPARAM  wParam, LPARAM  lParam )
{
    ...
    switch (uMsg)
    {
        case WM_INITDIALOG:
            ...
            SetWindowPos(hDlg,0, 0,0,0,0,SWP_NOZORDER|SWP_NOSIZE);
            return (0);
                ...
                case IDB_NEXT:
                    ...
                    if (indexLB == LB_ERR)
                    {
                        MessageBox (hDlg, TEXT("Please select an item from the list box"),
                                    TEXT("Registry Monkey Utility"), MB_OK);
                        return (0);
                    }
...
*
\************************************************************************/
VOID EnumerateLevel (HWND hDlg, LPTSTR NameLBSelect, LPTSTR RegPath, HKEY *hKeyRoot)
{
    HKEY    hKey;
    DWORD   retCode;
    TCHAR   Buf[80];
    ...
    if (*hKeyRoot)
    {
        // We are lower than the first level. (see note above)
    ...
    {
        lstrcpy(pszRoot,TEXT("HKEY_USERS"));
    }
    else if(*hKeyRoot == HKEY_LOCAL_MACHINE)
    {
        lstrcpy(pszRoot,TEXT("HKEY_LOCAL_MACHINE"));
    }
    else if(*hKeyRoot == HKEY_CURRENT_USER)
    {
        lstrcpy(pszRoot,TEXT("HKEY_CURRENT_USER"));
    }
    else if(*hKeyRoot == HKEY_CLASSES_ROOT)
    {
        lstrcpy(pszRoot,TEXT("HKEY_CLASSES_ROOT"));
    }
    else
    {
        lstrcpy(pszRoot,TEXT("UNKNOWN ROOT"));
    }
...
VOID QueryKey (HWND hDlg, HANDLE hKey)
{
    ...
    lResult = RegQueryInfoKey ( hKey,              // Key handle.
    ...
    if (lResult)
    {
        wsprintf (Buf, TEXT("Cannot open key %ld.  Error # %ld\n"),hKey,GetLastError());
...
VOID DisplayKeyData (HWND hDlg, TCHAR *RegPath, HANDLE hKeyRoot)
  {
  HKEY      hKey;
  DWORD     dwLBIndex;
  TCHAR     Buf[LINE_LEN];
...
//    REG_FULL_RESOURCE_DESCRIPTOR ( 9 )  // Resource list in the hardware description
    ...
    default:
      wsprintf (Buf, TEXT("Undefine in this verion of the Registry Monkey. %d"),
               dwType);
