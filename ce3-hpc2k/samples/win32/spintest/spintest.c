          ...
          int     nCmdShow)
{
  ...
  wc.hIcon         = LoadIcon(hInstance, MAKEINTRESOURCE(SPINTESTICON));
  wc.hCursor       = NULL;
  ...
  if (!(hwnd = CreateWindow (TEXT("Main"), TEXT("spintest Sample Application"),
                             dwStyle,
...
LRESULT CALLBACK MainWndProc (HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  {
	case WM_CREATE:
		if ( g_hWndCmdBar = CommandBar_Create( ghInst, hwnd, IDC_COMMANDBAR ) )
		{
			// add the main menu
			CommandBar_InsertMenubar( g_hWndCmdBar, ghInst, Menu , 0 );
			
			// NOTE: the following appears to be an NOP,
      // but it actually adds a Close button to the CommandBar.
			CommandBar_AddAdornments( g_hWndCmdBar, 0, 0 );
		}
		return 0;
	case IDM_EXIT:
  ...
  case WM_COMMAND:
    ...
    case WM_SIZE:
    {
      ...
      // adjust height to take into account CommandBar
      int CBHeight = CommandBar_Height( g_hWndCmdBar );
	    height -= CBHeight;

      SetWindowPos (gahwndSpin[0], NULL,
                    BORDER, BORDER + CBHeight,
                    width/2 - BORDER, height/2 - BORDER,
                    SWP_SHOWWINDOW);
      SetWindowPos (gahwndSpin[1], NULL,
                    width/2 + BORDER, BORDER + CBHeight,
                    width/2 - 2*BORDER, height/2 - BORDER,
                    SWP_SHOWWINDOW);
      SetWindowPos (gahwndSpin[2], NULL,
                    BORDER, height/2 + BORDER + CBHeight,
                    width/2 - BORDER, height/2 - 2*BORDER,
                    SWP_SHOWWINDOW);
      SetWindowPos (gahwndSpin[3], NULL,
                    width/2 + BORDER, height/2 + BORDER + CBHeight,
