...
// WinCE: file names don't have drive letters or volume names, but to have a 
// meaningful abbreviation, let's treat the first directory (if it exists)
// as the volume name.
	lpszCur = lpszBase;  
	ASSERT(*lpszCur == '\\');

	lpszCur = _tcsinc(lpszCur);
	while ((*lpszCur != '\\') && (*lpszCur != '\0'))
		lpszCur = _tcsinc(lpszCur);
	// If we hit the end of the string, then there is no directory, so rollback.
	if(*lpszCur == '\0')
		lpszCur = lpszBase;
#else // _WIN32_WCE
