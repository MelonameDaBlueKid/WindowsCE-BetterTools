...
void AFXAPI AfxBSTR2CString(CString* pStr, BSTR bstr)
{
	...
	ASSERT(lpsz != NULL);
	WCE_FCTN(memcpy)(lpsz, bstr, nLen*sizeof(TCHAR));
#else
...
LPDEVMODEW AFXAPI AfxDevModeA2W(LPDEVMODEW lpDevModeW, LPDEVMODEA lpDevModeA)
{
	...
	WCE_FCTN(memcpy)(&lpDevModeW->dmSpecVersion, &lpDevModeA->dmSpecVersion,
	...
	WCE_FCTN(memcpy)(&lpDevModeW->dmLogPixels, &lpDevModeA->dmLogPixels,
		sizeof(DEVMODEW) - offsetof(DEVMODEW, dmLogPixels));
	if (lpDevModeA->dmDriverExtra != 0)
		WCE_FCTN(memcpy)(lpDevModeW+1, lpDevModeA+1, lpDevModeA->dmDriverExtra);
...
LPDEVMODEA AFXAPI AfxDevModeW2A(LPDEVMODEA lpDevModeA, LPDEVMODEW lpDevModeW)
{
	...
	WCE_FCTN(memcpy)(&lpDevModeA->dmSpecVersion, &lpDevModeW->dmSpecVersion,
		offsetof(DEVMODEA, dmFormName) - offsetof(DEVMODEA, dmSpecVersion));
	AfxW2AHelper((LPSTR)lpDevModeA->dmFormName, lpDevModeW->dmFormName, 32*sizeof(char));
	WCE_FCTN(memcpy)(&lpDevModeA->dmLogPixels, &lpDevModeW->dmLogPixels,
		sizeof(DEVMODEA) - offsetof(DEVMODEA, dmLogPixels));
	if (lpDevModeW->dmDriverExtra != 0)
		WCE_FCTN(memcpy)(lpDevModeA+1, lpDevModeW+1, lpDevModeW->dmDriverExtra);
...
LPTEXTMETRICW AFXAPI AfxTextMetricA2W(LPTEXTMETRICW lptmW, LPTEXTMETRICA lptmA)
{
	if (lptmA == NULL)
		return NULL;
	ASSERT(lptmW != NULL);
	WCE_FCTN(memcpy)(lptmW, lptmA, sizeof(LONG) * 11);
	WCE_FCTN(memcpy)(&lptmW->tmItalic, &lptmA->tmItalic, sizeof(BYTE) * 5);
...
LPTEXTMETRICA AFXAPI AfxTextMetricW2A(LPTEXTMETRICA lptmA, LPTEXTMETRICW lptmW)
{
	if (lptmW == NULL)
		return NULL;
	ASSERT(lptmA != NULL);
	WCE_FCTN(memcpy)(lptmA, lptmW, sizeof(LONG) * 11);
	WCE_FCTN(memcpy)(&lptmA->tmItalic, &lptmW->tmItalic, sizeof(BYTE) * 5);
...
LPWSTR AFXAPI AfxAllocTaskWideString(LPCWSTR lpszString)
{
	...
	if (lpszResult != NULL)
		WCE_FCTN(memcpy)(lpszResult, lpszString, nSize);
	return lpszResult;
}
