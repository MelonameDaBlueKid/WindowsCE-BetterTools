...
AFX_EXCEPTION_LINK::AFX_EXCEPTION_LINK()
{
	...
	WCE_DYN_MEMBER(m_nType) = 0;
#endif
	WCE_DYN_MEMBER(m_pException) = NULL;    // no current exception yet

	// wire into top of exception link stack
	AFX_EXCEPTION_CONTEXT* pContext = AfxGetExceptionContext();
	WCE_DYN_MEMBER(m_pLinkPrev) = pContext->m_pLinkTop;
...
void AFXAPI AfxTryCleanup()
{
	...
	if (pLinkTop->WCE_DYN_MEMBER(m_pException) != NULL)
		pLinkTop->WCE_DYN_MEMBER(m_pException)->Delete();

	// remove ourself from the top of the chain
	pContext->m_pLinkTop = pLinkTop->WCE_DYN_MEMBER(m_pLinkPrev);
}
...
void AFXAPI AfxThrowLastCleanup()
{
	...
	if (pLinkTop != NULL)
	{
		...
		pLinkTop->WCE_DYN_MEMBER(m_pException) = NULL;
	}
...
// WinCE: if an exception gets fired we might be in the middle of a CWaitCursor constructor/destructor pair, 
// so let's halt the hour glass (which gets in the way of any error dialog box).
	CWinApp *pApp = AfxGetApp();
	if(pApp != NULL)
		pApp->DoWaitCursor(INT_MIN);
#endif // _WIN32_WCE
	...
	if (pNewException == NULL)
	{
		...
		pNewException = pContext->m_pLinkTop->WCE_DYN_MEMBER(m_pException);
	}
	...
	while (pContext->m_pLinkTop != NULL)
	{
		...
		if (pReceiver->WCE_DYN_MEMBER(m_pException) != NULL)
		{
			...
			if (pReceiver->WCE_DYN_MEMBER(m_pException) != pNewException)
				pReceiver->WCE_DYN_MEMBER(m_pException)->Delete();
			pReceiver->WCE_DYN_MEMBER(m_pException) = NULL;
			pContext->m_pLinkTop = pReceiver->WCE_DYN_MEMBER(m_pLinkPrev);
		}
		else
		{
			// throw the exception to the top handler (if appropriate type)
			if (pReceiver->WCE_DYN_MEMBER(m_nType) == 0)
			{
				// setup the receiver's context for the new exception
				pReceiver->WCE_DYN_MEMBER(m_pException) = pNewException;
...
BOOL AFXAPI AfxCatchProc(CRuntimeClass* pClass)
{
	...
	AFX_EXCEPTION_CONTEXT* pContext = AfxGetExceptionContext();
	ASSERT(pContext->m_pLinkTop != NULL);
	CException* pException = pContext->m_pLinkTop->WCE_DYN_MEMBER(m_pException);
