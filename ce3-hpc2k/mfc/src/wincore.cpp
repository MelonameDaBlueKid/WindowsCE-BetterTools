...
const MSG* PASCAL CWnd::GetCurrentMessage()
{
	...
	_AFX_THREAD_STATE* pThreadState = _afxThreadState.GetData();
	pThreadState->m_lastSentMsg.time = ::WCE_FCTN(GetMessageTime)();
...
#endif
		pState->m_pmapHWND = new CHandleMap(WCE_IF(RUNTIME_CLASS(CWnd),RUNTIME_CLASS(CTempWnd)),
...
#if !defined(_WIN32_WCE)	
// WinCE: this and _AfxGrayBackgroundWndProc not used
LRESULT CALLBACK
...
_AfxCbtFilterHook(int code, WPARAM wParam, LPARAM lParam) 
{
	...
	// In WinCE, we don't have real hooks.  We get here artificially, and without a proper lParam;
	// Since all we need is the window style bits, we can reconstruct CREATESTRUCT.
	CREATESTRUCT cs; 
	LPCREATESTRUCT lpcs = &cs; 
	cs.style = GetWindowLong((HWND)wParam,GWL_STYLE); 
	...
	HWND hWndParent, HMENU nIDorHMenu, LPVOID lpParam)
{
	...
	if (!((cs.style & WS_CHILD) | (cs.style & WS_POPUP)))
	{
		...
		{
			TRACE0("Warning: Can't get the SIP Info.");
		}
		else
		{
			int iDelta = (si.fdwFlags & SIPF_ON) ? 0 : MENU_HEIGHT;
			cs.cx = si.rcVisibleDesktop.right - si.rcVisibleDesktop.left;
			cs.cy = si.rcVisibleDesktop.bottom - si.rcVisibleDesktop.top - iDelta;
		}
	}
#endif // _WIN32_WCE_PSPC
...
#if defined(_WIN32_WCE)
	wce_PostCreateWindow(cs, hWnd, nIDorHMenu);
#endif // _WIN32_WCE
	...
	HCURSOR hCursor, HBRUSH hbrBackground, HICON hIcon)
{
	...
	wndcls.lpfnWndProc = WCE_IF(wce_FirstDefWindowProc, DefWindowProc);
...
void CWnd::WinHelp(DWORD dwData, UINT nCmd)
{
	...
	if (!::WCE_FCTN(WinHelp)(pWnd->m_hWnd, pApp->m_pszHelpFilePath, nCmd, dwData))
...
WCE_DEL	ON_WM_ENTERIDLE()
	...
	ON_WM_VKEYTOITEM()
	ON_WM_NCDESTROY()
WCE_DEL	ON_WM_PARENTNOTIFY()
	ON_WM_SYSCOLORCHANGE()
WCE_DEL	ON_WM_DEVMODECHANGE()
	ON_WM_HELPINFO()
	ON_WM_SETTINGCHANGE()
	//}}AFX_MSG_MAP
#ifndef _AFX_NO_OCC_SUPPORT
...
WCE_DEL	ON_MESSAGE(WM_DISPLAYCHANGE, OnDisplayChange)
WCE_DEL	ON_REGISTERED_MESSAGE(CWnd::m_nMsgDragList, OnDragList)
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
	ON_WM_LBUTTONDOWN()
#endif // _WIN32_WCE_PSPC
#ifdef _AFX_NO_OCC_SUPPORT
...
CWnd* PASCAL CWnd::GetDescendantWindow(HWND hWnd, int nID, BOOL bOnlyPerm)
{
	...
	{
		if (::WCE_FCTN(GetTopWindow)(hWndChild) != NULL)
		{
	...
	// walk each child
	for (hWndChild = ::WCE_FCTN(GetTopWindow)(hWnd); hWndChild != NULL;
	...
	WPARAM wParam, LPARAM lParam, BOOL bDeep, BOOL bOnlyPerm)
{
	...
	for (HWND hWndChild = ::WCE_FCTN(GetTopWindow)(hWnd); hWndChild != NULL;
		hWndChild = ::WCE_FCTN(GetNextWindow)(hWndChild, GW_HWNDNEXT))
	{
		...
		if (bDeep && ::WCE_FCTN(GetTopWindow)(hWndChild) != NULL)
		{
...
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
	// send message to menubar if current window is CFrameWnd object
	CWnd* pWndTemp = CWnd::FromHandlePermanent(hWnd);

	if (pWndTemp != NULL && pWndTemp->IsKindOf(RUNTIME_CLASS(CFrameWnd)))
	{
		CFrameWnd* pFrameWnd = (CFrameWnd*)pWndTemp;
		if (pFrameWnd->m_hCommandBar != NULL)
		{
			if (bOnlyPerm)
			{
				CWnd* pWnd = CWnd::FromHandlePermanent(pFrameWnd->m_hCommandBar);
				if (pWnd != NULL)
				{
					// call window proc directly since it is a C++ window
					AfxCallWndProc(pWnd, pWnd->m_hWnd, message, wParam, lParam);
			...
			{
				// send message with Windows SendMessage API
				::SendMessage(pFrameWnd->m_hCommandBar, message, wParam, lParam);
			}
		}
	}
#endif // _WIN32_WCE_PSPC
...
int CWnd::GetScrollPos(int nBar) const
{
	...
	if ((pScrollBar = GetScrollBarCtrl(nBar)) != NULL)
		...
		return ::WCE_FCTN(GetScrollPos)(m_hWnd, nBar);
}
...
void CWnd::GetScrollRange(int nBar, LPINT lpMinPos, LPINT lpMaxPos) const
{
	...
	if ((pScrollBar = GetScrollBarCtrl(nBar)) != NULL)
		...
		::WCE_FCTN(GetScrollRange)(m_hWnd, nBar, lpMinPos, lpMaxPos);
}
...
int CWnd::GetScrollLimit(int nBar)
{
	...
	SCROLLINFO info;
#if defined(_WIN32_WCE)
	memset( &info, 0, sizeof(SCROLLINFO) ); // Added for WinCE
	info.cbSize = sizeof(SCROLLINFO);       // Added for WinCE
#endif // _WIN32_WCE
	...
	LPCRECT lpRect, LPCRECT lpClipRect)
{
	...
	if (IsWindowVisible() || lpRect != NULL || lpClipRect != NULL)
	{
		...
		::WCE_FCTN(ScrollWindow)(m_hWnd, xAmount, yAmount, lpRect, lpClipRect);
	}
	else
	{
	...
	UINT nFlags, LPRECT lpRectParam, LPCRECT lpRectClient, BOOL bStretch)
{
	...
	if (nFlags != reposQuery)
		layout.hDWP = ::WCE_FCTN(BeginDeferWindowPos)(8); // reasonable guess
	else
		layout.hDWP = NULL; // not actually doing layout

	for (HWND hWndChild = ::WCE_FCTN(GetTopWindow)(m_hWnd); hWndChild != NULL; 
		hWndChild = ::WCE_FCTN(GetNextWindow)(hWndChild, GW_HWNDNEXT))
	{
...
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
	if(nIDC == AFXCE_ID_DOCLIST) // CCeDocList
	{
		CWnd* pWnd = FromHandlePermanent(hWndChild);
		if(pWnd != NULL && pWnd->IsKindOf(RUNTIME_CLASS(CCeDocList)))
		{
			CCeDocList* pDocList = static_cast<CCeDocList*>(pWnd);
			if(pDocList->m_pDocTemplate != NULL)
	...
	if (layout.hDWP == NULL || !::WCE_FCTN(EndDeferWindowPos)(layout.hDWP))
	...
	HWND hWnd, LPCRECT lpRect)
{
	...
	if (lpLayout != NULL)
	{
		lpLayout->hDWP = ::WCE_FCTN(DeferWindowPos)(lpLayout->hDWP, hWnd, NULL,
...
void CWnd::CalcWindowRect(LPRECT lpClientRect, UINT nAdjustType)
{
	...
	::AdjustWindowRectEx(lpClientRect, WCE_IF((GetStyle() & ~(WS_HSCROLL | WS_VSCROLL)), GetStyle()), FALSE, dwExStyle);
}
...
LRESULT CWnd::OnActivateTopLevel(WPARAM wParam, LPARAM lParam)
{
#if !defined(_WIN32_WCE_NO_TOOLTIPS)
...
void CWnd::CenterWindow(CWnd* pAlternateOwner)
{
	...
	if (!(dwStyle & WS_CHILD))
	{
		...
		WCE_FCTN(SystemParametersInfo)(SPI_GETWORKAREA, NULL, &rcArea, NULL);
...
#if defined(_WIN32_WCE)
			#define CBEM_INSERTITEM     WIN16_CB_ADDSTRING
#endif // _WIN32_WCE
...
#if defined(_WIN32_WCE)
// WinCE: lpnRes is not aligned, so we need to transfer its contents into a temporary, aligned buffer
				void* lpnResAligned = malloc(dwLen);
				if(lpnResAligned != NULL)
				{
					WCE_FCTN(memcpy)(lpnResAligned, lpnRes, dwLen);
					if (::WCE_FCTN(SendDlgItemMessage)(m_hWnd, nIDC, nMsg, 0, (LPARAM)lpnResAligned) == -1)
						bSuccess = FALSE;
					free(lpnResAligned);
				}
#else // _WIN32_WCE
...
#if defined(_WIN32_WCE)
			// if the dlg was not displayed yet and the crt msg is a mouse msg or 
			// WM_SYSKEYUP msg for a disabled window
			// get rid of it in order to be able to display the dlg successfully
			BOOL bPeek = ::PeekMessage(pMsg, NULL, NULL, NULL, PM_NOREMOVE);
			if(bPeek && ((pMsg->message >= WM_MOUSEFIRST && pMsg->message <= WM_MOUSELAST) || pMsg->message == WM_SYSKEYUP) && !(::IsWindowEnabled(pMsg->hwnd)) && bShowIdle)
...
#if !defined(_WIN32_WCE) 
//WinCE: IDI_APPLICATION default icon is not defined
...
LONG AFXAPI _AfxInitCommonControls(LPINITCOMMONCONTROLSEX lpInitCtrls, LONG fToRegister)
{
...
#ifndef _AFXDLL
	HINSTANCE hInstBefore = ::WCE_FCTN(GetModuleHandleA)(WCE_IF("COMMCTRL.DLL","COMCTL32.DLL"));
#endif

	// load the COMCTL32.DLL library because it may not be loaded yet (delayload)
	HINSTANCE hInst = ::WCE_FCTN(LoadLibraryA)(WCE_IF("COMMCTRL.DLL","COMCTL32.DLL"));
	if (hInst == NULL)
		return 0;
	...
	(FARPROC&)pfnInit = ::WCE_FCTN(GetProcAddress)(hInst, "InitCommonControlsEx");
#if defined(_WIN32_WCE_EMULATION) && (_WIN32_WCE == 200) 
// WinCE: Workaround for problem in H/PC 2.0 Emulation: handle ICC_BAR_CLASSES separately.
...
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
BOOL CWnd::ShowDoneButton(BOOL bShow)
{
	...
	if(bShow)
	{
		// Allow a window to steal ownership.
		if(pTopFrame->m_pDoneBtnOwner != NULL)
			pTopFrame->m_pDoneBtnOwner->m_bShowDoneButton = FALSE;

		if(!::SHDoneButton(pTopFrame->m_hWnd, SHDB_SHOW))
			return FALSE;

		pTopFrame->m_pDoneBtnOwner = this;
		m_bShowDoneButton = TRUE;
	}
	else
	{
		// If this window doesn't own the Done button, it can't be hidden.
		// The OS enforces this anyway.
		ASSERT(pTopFrame->m_pDoneBtnOwner == this);
		...
		m_bShowDoneButton = FALSE;
		pTopFrame->m_pDoneBtnOwner = NULL;

		if(!::SHDoneButton(pTopFrame->m_hWnd, SHDB_HIDE))
			return FALSE;
	}
...
{
	if(IsKindOf(RUNTIME_CLASS(CToolBar)))
	{
		Default();
	}
	else
