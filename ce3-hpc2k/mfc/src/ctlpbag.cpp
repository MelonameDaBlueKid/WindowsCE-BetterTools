...
STDMETHODIMP CBlobProperty::Load(LPSTREAM pStream)
{
	...
	HGLOBAL hBlobNew = WCE_FCTN(GlobalAlloc)(GMEM_MOVEABLE, sizeof(ULONG)+cb);
	if (hBlobNew == NULL)
		return E_OUTOFMEMORY;

	void* pBlobNew = WCE_FCTN(GlobalLock)(hBlobNew);
	*(ULONG*)pBlobNew = cb;
	hr = pStream->Read(((ULONG*)pBlobNew)+1, cb, &cbRead);
	WCE_FCTN(GlobalUnlock)(hBlobNew);

	if (FAILED(hr))
	{
		WCE_FCTN(GlobalFree)(hBlobNew);
		return hr;
	}
	if (cb != cbRead)
	{
...
{
	void* pBlob = WCE_FCTN(GlobalLock)(m_hBlob);
	if (pBlob == NULL)
		return E_OUTOFMEMORY;
	...
	void* pvProp, const void* pvDefault)
{
	...
	if (m_bLoading)
	{
		...
		case VT_R4:
			WCE_FCTN(memcpy)(pvProp, &V_R4(&var), sizeof(float));
			break;

		case VT_R8:
			WCE_FCTN(memcpy)(pvProp, &V_R8(&var), sizeof(double));
			break;
	...
	else
	{
			...
			!_AfxIsSamePropValue(vtProp, pvProp, pvDefault))
		{
			...
			case VT_R4:
				WCE_FCTN(memcpy)(&V_R4(&var), pvProp, sizeof(float));
				break;

			case VT_R8:
				WCE_FCTN(memcpy)(&V_R8(&var), pvProp, sizeof(double));
				break;
	...
	LPFONTDISP pFontDispAmbient)
{
	...
	if (m_bLoading)
	{
		LPFONT pFont = NULL;
			...
			bSuccess = 
				SUCCEEDED(WCE_FCTN(OleCreateFontIndirect)(NULL, IID_IFont, (LPVOID *)&pFont)) &&
				SUCCEEDED(pFont->QueryInterface(IID_IPersistPropertyBag, (void**)&ppb)) &&
			    SUCCEEDED(ppb->Load(m_pPropBag, m_pErrorLog));
#else // _WIN32_WCE
		bSuccess =
	...
	else
	{
		...
		else
		{
			...
			IPersistPropertyBag* ppb;
			font.FontToCeFont(); 
			bSuccess = SUCCEEDED(font.m_pFont->QueryInterface(IID_IPersistPropertyBag, (void**)&ppb)) &&
			           SUCCEEDED(ppb->Save(m_pPropBag, FALSE, TRUE));
			font.RestoreFont();
#else // _WIN32_WCE
