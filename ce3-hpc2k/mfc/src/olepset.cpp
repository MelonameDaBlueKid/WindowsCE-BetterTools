	...
	size_t cbCharSize)
{
	...
	while (nReps--)
	{
		...
		WCE_FCTN(memcpy)(pbResult + cbResult, (LPBYTE)&cchCopy, sizeof(DWORD));
		WCE_FCTN(memcpy)(pbResult + cbResult + sizeof(DWORD), pbBuffer, cbCopy);
...
BOOL CProperty::Set(const  LPVOID pVal)
{
	...
	WCE_FCTN(memcpy)(m_pValue, pValue, (int)cbValue);
...
CPropertySet::CPropertySet()
{
	...
	m_PH.wFormat = 0;
	m_PH.dwOSVer = (DWORD)MAKELONG(LOWORD(WCE_FCTN(GetVersion())), 2);
