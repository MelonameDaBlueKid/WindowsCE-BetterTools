	...
	const DLGTEMPLATE* pOrigTemplate)
{
	...
	pDlgInfo->m_bDialogEx = IsDialogEx(pOrigTemplate); // See header for explanation
#endif // _WIN32_WCE
...
void COccManager::PostCreateDialog(_AFX_OCC_DIALOG_INFO* pDlgInfo)
{
	if (pDlgInfo->m_pNewTemplate != NULL)
		WCE_FCTN(GlobalFree)(pDlgInfo->m_pNewTemplate);
	...
	DLGITEMTEMPLATE** ppOleDlgItems)
{
	...
	for (iItem = 0; iItem < nItems; iItem++)
	{
		...
		else
		{
			...
			if(bDialogEx) 
			{
				BYTE* wce_convertDialogItemTemplate(DLGITEMTEMPLATE*,DLGITEMTEMPLATEEX*,ULONG); 
				pNew = wce_convertDialogItemTemplate((DLGITEMTEMPLATE*)pNew,
				                                     (DLGITEMTEMPLATEEX*)pItem,cbItem);
			}
			else
			{
				WCE_FCTN(memcpy)(pNew, pItem, cbItem);
				pNew += cbItem;
			}
#else // _WIN32_WCE
	...
	_AFX_OCC_DIALOG_INFO* pOccDlgInfo)
{
	...
	BOOL bDialogEx = WCE_IF(pOccDlgInfo->m_bDialogEx, IsDialogEx(pOccDlgInfo->m_pNewTemplate));
	BOOL bSuccess = TRUE;
	if (lpResource != NULL)
	{
	...
	BOOL bDialogEx, LPDLGITEMTEMPLATE pItem, WORD nMsg, BYTE* lpData, DWORD cb)
{
	...
	if (pszClass[0] == '{')
		...
		hr = WCE_FCTN(CLSIDFromProgID)(pszClass, &clsid);
