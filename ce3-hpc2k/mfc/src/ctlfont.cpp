...
AFX_STATIC_DATA const FONTDESC _afxFontDescDefault =
	{ sizeof(FONTDESC), WCE_IF(OLESTR("Tahoma"),OLESTR("MS Sans Serif")), WCE_IF(FONTSIZE(8),FONTSIZE(12)), FW_NORMAL,
	...
	LPDISPATCH pFontDispAmbient)
{
	...
	else
	{
		...
		if (FAILED(::WCE_FCTN(OleCreateFontIndirect)((LPFONTDESC)pFontDesc, IID_IFont,
				(LPVOID *)&pFontNew)))
			pFontNew = NULL;
	}
...
void CFontHolder::FontToCeFont()
{
	// WinCE: we're not assuming that m_pFont is *not* a CCeFont*
	FONTDESC fd;
	...
	BSTR bstrName = NULL;
	if(SUCCEEDED(m_pFont->get_Name(&bstrName)))
		fd.lpstrName = bstrName;
	else
		return;

	m_pFont->get_Size(&fd.cySize);
	m_pFont->get_Weight(&fd.sWeight);
	m_pFont->get_Charset(&fd.sCharset);
	m_pFont->get_Italic(&fd.fItalic);
	m_pFont->get_Underline(&fd.fUnderline);
	m_pFont->get_Strikethrough(&fd.fStrikethrough);

	IFont *pFontNew;
	if(SUCCEEDED(::WCE_FCTN(OleCreateFontIndirect)(&fd, IID_IFont, (LPVOID *)&pFontNew)))
	{
		m_pSavedFont = m_pFont;
		m_pFont = pFontNew;
	}
...
void CFontHolder::RestoreFont()
{
	if(m_pSavedFont != NULL)
	{
		m_pFont->Release();
		m_pFont = m_pSavedFont;
		m_pSavedFont = NULL;
	}
}
#endif // _WIN32_WCE
