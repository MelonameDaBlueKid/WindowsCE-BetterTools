...
#if defined(_WIN32_WCE)
// WinCE: Reflector windows need to be inflated as well so it doesn't hide parts of the control window.
		if (lprcClipRect != NULL)
		{
			::InflateRect((RECT*)lprcClipRect, nHandleSize, nHandleSize);
			m_pRectTracker->m_rectClip = *lprcClipRect;
		}
#endif // _WIN32_WCE
	}
