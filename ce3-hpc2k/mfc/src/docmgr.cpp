...
BOOL AFXAPI _AfxDeleteRegKey(LPCTSTR lpszKey)
{
	...
	while (lpszLast != NULL)
	{
		*lpszLast = '\0';
		...
		HKEY hKey;
		if (WCE_FCTN(RegOpenKey)(HKEY_CLASSES_ROOT, lpszKeyCopy, &hKey) != ERROR_SUCCESS)
			break;
		...
		if (WCE_FCTN(RegEnumKey)(hKey, 0, szScrap, dwLen) == ERROR_SUCCESS)
			bItExists = TRUE;
		::RegCloseKey(hKey);
...
_AfxSetRegKey(LPCTSTR lpszKey, LPCTSTR lpszValue, LPCTSTR lpszValueName = NULL)
{
	...
	{
		if (::WCE_FCTN(RegSetValue)(HKEY_CLASSES_ROOT, lpszKey, REG_SZ,
	...
	else
	{
		...
		if(::WCE_FCTN(RegCreateKey)(HKEY_CLASSES_ROOT, lpszKey, &hKey) == ERROR_SUCCESS)
		{
