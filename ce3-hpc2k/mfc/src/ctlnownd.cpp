...
void COleControl::GetClientOffset(long* pdxOffset, long* pdyOffset) const
{
	...
	if (nOffset > 0)
	{
		*pdxOffset = nOffset * WCE_FCTN(GetSystemMetrics)(SM_CXBORDER);
		*pdyOffset = nOffset * WCE_FCTN(GetSystemMetrics)(SM_CYBORDER);
	}
	else
	{
	...
	DWORD dwMouseMsg, BOOL bSetAlways)
{
	...
	if (bSetAlways && ! lResult)
		::SetCursor(::WCE_FCTN(LoadCursor)(NULL, IDC_ARROW));
...
BOOL COleControl::OnWindowlessMessage(UINT msg, WPARAM wParam, LPARAM lParam,
	LRESULT* plResult)
{
	if ((msg >= WM_MOUSEFIRST && msg <= WM_MOUSELAST) || WCE_IF(0,(msg == WM_CONTEXTMENU)))
	{
