...
const AFX_DATADEF CRect CFrameWnd::rectDefault(
	WCE_IF(0,CW_USEDEFAULT), WCE_IF(0,CW_USEDEFAULT),
	WCE_IF(CW_USEDEFAULT,0 /* 2*CW_USEDEFAULT */), WCE_IF(CW_USEDEFAULT,0 /* 2*CW_USEDEFAULT */));
...
WCE_DEL	ON_WM_INITMENU()
	ON_WM_INITMENUPOPUP()
WCE_DEL	ON_WM_MENUSELECT()
...
WCE_DEL ON_WM_ENTERIDLE()
	...
	ON_WM_ERASEBKGND()
	ON_WM_ACTIVATE()
WCE_DEL	ON_WM_NCACTIVATE()
	ON_WM_SYSCOMMAND()
WCE_DEL	ON_WM_DROPFILES()
WCE_DEL	ON_WM_QUERYENDSESSION()
WCE_DEL	ON_WM_ENDSESSION()
...
WCE_DEL ON_UPDATE_COMMAND_UI(ID_CONTEXT_HELP, OnUpdateContextHelp)
	...
	ON_NOTIFY(DLN_FOLDER, AFXCE_ID_DOCLIST, OnFolderChanged)
...
WCE_DEL	ON_MESSAGE(WM_DDE_INITIATE, OnDDEInitiate)
WCE_DEL	ON_MESSAGE(WM_DDE_EXECUTE, OnDDEExecute)
WCE_DEL	ON_MESSAGE(WM_DDE_TERMINATE, OnDDETerminate)
WCE_DEL ON_REGISTERED_MESSAGE(_afxMsgMouseWheel, OnRegisteredMouseWheel)
END_MESSAGE_MAP()
...
BOOL CFrameWnd::PreTranslateMessage(MSG* pMsg)
{
	...
	if (pMsg->message == WM_LBUTTONDOWN || pMsg->message == WCE_IF(WM_LBUTTONDOWN,WM_NCLBUTTONDOWN))
...
void CFrameWnd::ShowOwnedWindows(BOOL bShow)
{
	...
	HWND hWnd = ::GetWindow(::WCE_FCTN(GetDesktopWindow)(), GW_CHILD);
	while (hWnd != NULL)
	{
...
#if !defined(_WIN32_WCE)
// WinCE: FrameWindows should not have borders.
	if (afxData.bWin4)
	...
	CCreateContext* pContext)
{
	HMENU hMenu = NULL;
#if defined(_WIN32_WCE)
// WinCE: cache the menu resource id for old command bar compatibility
...
#if defined(_WIN32_WCE)
//WinCE: set icon image for taskbar
	if(lpszMenuName != NULL)
	{
		HICON hIcon = NULL;
		HINSTANCE hInst = AfxFindResourceHandle(lpszMenuName, RT_ICON);
		if ((hIcon = (HICON)::LoadImage(hInst, lpszMenuName,  
			IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR)) != NULL)
		{
			PostMessage(WM_SETICON, FALSE, (LPARAM)hIcon);
		}
	}
#endif // _WIN32_WCE
	return TRUE;
...
CWnd* CFrameWnd::CreateView(CCreateContext* pContext, UINT nID)
{
	...
	CRect initRect(0, 0, rcMain.right, rcMain.bottom);
 
	// views are always created with a border! 
	if (!pView->Create(NULL, NULL, AFX_WS_DEFAULT_VIEW,	initRect, this, nID, pContext))
	{
...
BOOL CFrameWnd::OnCreateClient(LPCREATESTRUCT, CCreateContext* pContext)
{
	...
	{
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
// WinCE: CCeDocListDocTemplate creates temporary documents that don't have a view.
	if (pContext->m_pCurrentDoc != NULL && pContext->m_pCurrentDoc->m_bTempDoc)
		return TRUE;
#endif // _WIN32_WCE_PSPC
	...
	CWnd* pParentWnd, CCreateContext* pContext)
{
	...
	if (!Create(lpszClass, lpszTitle, WCE_IF(WS_VISIBLE, dwDefaultStyle), rectDefault,
...
void CFrameWnd::OnUpdateFrameMenu(HMENU hMenuAlt)
{
	...
	::WCE_FCTN(SetMenu)(m_hWnd, hMenuAlt);
}
...
void CFrameWnd::OnDestroy()
{
	...
	if (m_hMenuDefault != NULL && ::WCE_FCTN(GetMenu)(m_hWnd) != m_hMenuDefault)
	{
		::WCE_FCTN(SetMenu)(m_hWnd, m_hMenuDefault);
		ASSERT(::WCE_FCTN(GetMenu)(m_hWnd) == m_hMenuDefault);
	}

#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
// WinCE: hide done button, if visible, when the main window is destroyed
	if(m_pDoneBtnOwner != NULL)
	{
		::SHDoneButton(m_hWnd, SHDB_HIDE);
		m_pDoneBtnOwner->m_bShowDoneButton = FALSE;
		m_pDoneBtnOwner = NULL;
	}
#endif // _WIN32_WCE_PSPC
#if !defined(_WIN32_WCE)
...
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
void CFrameWnd::OnSettingChange(UINT uFlags, LPCTSTR lpszSection)
{
	if (GetTopLevelFrame() == this) 
	{
...
void CFrameWnd::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
	...
	if(m_pDoneBtnOwner != NULL)
	{
		HWND hWndOwner = m_pDoneBtnOwner->GetSafeHwnd();
...
void CFrameWnd::OnOK()
{
	if(m_pDoneBtnOwner == this)
	{
		; // do nothing; app is assumed to implement override as needed
	}
	else
	{
		ASSERT_VALID(m_pDoneBtnOwner);
		if(::IsWindow(m_pDoneBtnOwner->m_hWnd))
			::SendMessage(m_pDoneBtnOwner->m_hWnd, WM_COMMAND, IDOK, 0);
	}
}
#endif // _WIN32_WCE_PSPC
#if !defined(_WIN32_WCE)
...
void CFrameWnd::BringToTop(int nCmdShow)
{
		...
		nCmdShow != SW_SHOWNA && nCmdShow != SW_SHOWNOACTIVATE)
	{
		...
		HWND hWndLastPop = ::WCE_FCTN(GetLastActivePopup)(m_hWnd);
		::BringWindowToTop(hWndLastPop);
	}
}

#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
void CFrameWnd::OnFolderChanged(DLNHDR* pNotifyStruct, LRESULT* result)
{
	HWND hWndFrom = pNotifyStruct->nmhdr.hwndFrom;
	CWnd* pWnd = FromHandlePermanent(hWndFrom);
	if(pWnd != NULL && pWnd->IsKindOf(RUNTIME_CLASS(CCeDocList)))
	{
		...
		if(pDocList->m_pDocTemplate != NULL)
		{
			...
			pDocList->m_pDocTemplate->m_crtFolder.LoadString(AFXCE_IDS_INTL_DIRMYDOCUMENTS);

			CString sAllFolders;
			sAllFolders.LoadString(AFXCE_IDS_INTL_ALLFOLDERS);

			if(_tcscmp(pNotifyStruct->pszPath, sAllFolders)){
				pDocList->m_pDocTemplate->m_crtFolder += CString("\\") + pNotifyStruct->pszPath;
				pDocList->m_pDocTemplate->m_strFolder = pNotifyStruct->pszPath;
			}
			else
				pDocList->m_pDocTemplate->m_strFolder = sAllFolders;
		}
	}
}

void CFrameWnd::OnCreateDocList(DLNHDR* pNotifyStruct, LRESULT* result)
{
	// Save SIP info when create DocList
	m_saiFrm = m_sai;
}

void CFrameWnd::OnDestroyDocList(DLNHDR* pNotifyStruct, LRESULT* result)
{
	// Restore SIP status
	...
	::SendMessage(m_hWnd, WM_SETTINGCHANGE, -1, 0);
}
#endif // _WIN32_WCE_PSPC
