...
BOOL CControlBar::AllocElements(int nElements, int cbElement)
{
    ...
    {
        ASSERT(cbElement > 0);
        if ((pData = WCE_FCTN(calloc)(nElements, cbElement)) == NULL)
            return FALSE;
    }
...
void CControlBar::OnTimer(UINT nIDEvent)
{
    ...
    CPoint point; WCE_FCTN(GetCursorPos)(&point);
    ...
    if (m_nStateFlags & (delayHide|delayShow))
    {
        ...
        if (swpFlags != 0)
        {
            ...
            if (lpLayout->hDWP != NULL)
            {
                ...
                lpLayout->hDWP = ::WCE_FCTN(DeferWindowPos(lpLayout->hDWP, m_hWnd, NULL,
                    0, 0, 0, 0, swpFlags|
                    SWP_NOMOVE|SWP_NOSIZE|SWP_NOZORDER|SWP_NOACTIVATE));
            }
        }
