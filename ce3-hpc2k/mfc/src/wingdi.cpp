...
#endif
		pState->m_pmapHDC = new CHandleMap(WCE_IF(RUNTIME_CLASS(CDC),RUNTIME_CLASS(CTempDC)),
...
#if !defined(_WIN32_WCE_NO_GDITRANSFORM)
...
#endif // _WIN32_WCE_NO_GDITRANSFORM
...
int CDC::OffsetClipRgn(int x, int y)
{
	...
	if (m_hDC != m_hAttribDC)
		nRetVal = ::WCE_FCTN(OffsetClipRgn)(m_hDC, x, y);
	if (m_hAttribDC != NULL)
		nRetVal = ::WCE_FCTN(OffsetClipRgn)(m_hAttribDC, x, y);
	return nRetVal;
}
...
int CDC::OffsetClipRgn(SIZE size)
{
	...
	if (m_hDC != m_hAttribDC)
		nRetVal = ::WCE_FCTN(OffsetClipRgn)(m_hDC, size.cx, size.cy);
	if (m_hAttribDC != NULL)
		nRetVal = ::WCE_FCTN(OffsetClipRgn)(m_hAttribDC, size.cx, size.cy);
	return nRetVal;
}
...
{ 
	ASSERT(m_hDC != NULL); 
	CPoint ptTo(x,y); 
	CPoint points[2] = { m_ptCurPos, ptTo }; 
	BOOL bResult = ::Polyline(m_hDC, points, 2); 
	if (bResult)  
		m_ptCurPos = ptTo; 
	return bResult; 
} 
...
{
	ASSERT(m_hDC != NULL);
	CPoint firstSeg[2] = { m_ptCurPos, lpPoints[0] };
	BOOL bResult = ::Polyline(m_hDC, firstSeg, 2);
	if (bResult) 
	{
		bResult = ::Polyline(m_hDC, lpPoints, nCount);
		if (bResult) 
			m_ptCurPos = lpPoints[nCount-1];
	}
	return bResult;
}
#else // _WIN32_WCE
...
int CDC::SelectClipRgn(CRgn* pRgn, int nMode)
{
	...
	int nRetVal = ERROR;
	if (m_hDC != m_hAttribDC)
		nRetVal = ::WCE_FCTN(ExtSelectClipRgn)(m_hDC, (HRGN)pRgn->GetSafeHandle(), nMode);
	if (m_hAttribDC != NULL)
		nRetVal = ::WCE_FCTN(ExtSelectClipRgn)(m_hAttribDC, (HRGN)pRgn->GetSafeHandle(), nMode);
	return nRetVal;
}
...
void CDC::LPtoDP(LPSIZE lpSize) const
{
	...
	lpSize->cx = WCE_FCTN(MulDiv)(lpSize->cx, abs(sizeVpExt.cx), abs(sizeWinExt.cx));
	lpSize->cy = WCE_FCTN(MulDiv)(lpSize->cy, abs(sizeVpExt.cy), abs(sizeWinExt.cy));
}
...
void CDC::DPtoLP(LPSIZE lpSize) const
{
	...
	CSize sizeWinExt = GetWindowExt();
	CSize sizeVpExt = GetViewportExt();
	lpSize->cx = WCE_FCTN(MulDiv)(lpSize->cx, abs(sizeWinExt.cx), abs(sizeVpExt.cx));
	lpSize->cy = WCE_FCTN(MulDiv)(lpSize->cy, abs(sizeWinExt.cy), abs(sizeVpExt.cy));
}
...
CHandleMap* PASCAL afxMapHGDIOBJ(BOOL bCreate)
{
	...
	if (pState->m_pmapHGDIOBJ == NULL && bCreate)
	{
		...
		_PNH pnhOldHandler = AfxSetNewHandler(&AfxCriticalNewHandler);
#endif
		pState->m_pmapHGDIOBJ = new CHandleMap(WCE_IF(RUNTIME_CLASS(CGdiObject),RUNTIME_CLASS(CTempGdiObject)),
...
{
	if (!Attach(::WCE_FCTN(CreatePen)(nPenStyle, nWidth, crColor)))
		AfxThrowResourceException();
}
...
BOOL CBrush::CreateDIBPatternBrush(HGLOBAL hPackedDIB, UINT nUsage)
{
	...
	const void* lpPackedDIB = ::WCE_FCTN(GlobalLock)(hPackedDIB);
	ASSERT(lpPackedDIB != NULL);
	BOOL bResult = Attach(::CreateDIBPatternBrushPt(lpPackedDIB, nUsage));
	::WCE_FCTN(GlobalUnlock)(hPackedDIB);
	return bResult;
}
