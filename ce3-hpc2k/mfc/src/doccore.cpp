...
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
		CDocTemplate* pDocTemplate = CDocument::GetDocTemplate();
		if(pDocTemplate != NULL && pDocTemplate->IsKindOf(RUNTIME_CLASS(CCeDocListDocTemplate))) // CEPC 300 specific doc template
		{
			MSG lastMsg = AfxGetThreadState()->m_lastSentMsg;
			if((lastMsg.message) != WM_CLOSE && ((lastMsg.message) != WM_COMMAND || (LOWORD(lastMsg.wParam)) != IDOK))
			{
			...
			else
			{
				...
				{
					CCeDocListDocTemplate* pDocListTemplate	= static_cast<CCeDocListDocTemplate*>(pDocTemplate);
					newName = pDocListTemplate->m_crtFolder + CString("\\") + m_strTitle;
					CString strExt;
					...
					int i = 1;
					while(GetFileAttributes(newName) != 0xFFFFFFFF)
					{
						TCHAR buff[256];
						newName = pDocListTemplate->m_crtFolder + CString("\\") + m_strTitle + CString(_itow(i, buff, 10)) + strExt;
						i++;
					}
				}
			}
		}
		else // CEPC 300 specific doc template
		{
			if (!AfxGetApp()->DoPromptFileName(newName,
				bReplace ? AFX_IDS_SAVEFILE : AFX_IDS_SAVEFILECOPY,
				OFN_HIDEREADONLY | OFN_PATHMUSTEXIST, FALSE, pTemplate))
...
BOOL CDocument::SaveModified()
{
	...
	{
		CDocTemplate* pDocTemplate = CDocument::GetDocTemplate();
		if(pDocTemplate != NULL && pDocTemplate->IsKindOf(RUNTIME_CLASS(CCeDocListDocTemplate)))
		{
			if (!DoFileSave())
	...
	LPTSTR pName = name.GetBuffer(_MAX_PATH);
	LPTSTR pFileName = pName;
	while(*pName != '\0')
	{
		if(*pName == '\\')
		{
	...
	CString prompt1;
	CString name1(pFileName);

	pName = name1.GetBuffer(_MAX_PATH);
	if(_tcslen(pName) >= 15)
	{
		pName[13] = '.';
		pName[14] = '.';
	...
	name1.ReleaseBuffer(-1);

	AfxFormatString1(prompt1, AFX_IDP_ASK_TO_SAVE, name1);
	CCeSaveModifiedDlg smDlg(prompt1);
	switch (smDlg.DoModal())
	{
		case IDCANCEL:
			return FALSE;       // don't continue
...
CString CMirrorFile::GetTempName(LPCTSTR lpszOriginalFile, BOOL bCreate)
{
	...
	LPTSTR lpszName;
	WCE_FCTN(GetFullPathName)(lpszOriginalFile, _MAX_PATH, szPath, &lpszName);
	*lpszName = NULL;

	// let's create a temporary file name, and create
	// a file too!

	WCE_FCTN(GetTempFileName)(szPath, _T("MFC"), 0,
	...
	CFileException* pError)
{
	...
	if (nOpenFlags & CFile::modeCreate) //opened for writing
	{
		if (CFile::GetStatus(lpszFileName, status))
		{
			...
			DWORD nBytes = (DWORD)wce_GetObjectStoreFreeSpace();
#else // _WIN32_WCE
...
BOOL CDocument::OnNewDocument()
{
   ...
   while (pos != NULL)
   {
		...
		if(pView != NULL && pView->IsKindOf(RUNTIME_CLASS(CEditView)))
		{
			CEditView* pEditView = (CEditView*)pView;
			pEditView->DismissFindReplaceBar();
		}
   }
#endif //_WIN32_WCE_PSPC
...
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
// WinCE: CCeDocListDocTemplate doesn't recycle docs (thereby keeping views alive) when
// a doc is closed.  Instead it permits a CFrameWnd to be alive w/o views.  The exception
// to this is when the app is being closed.
		CDocTemplate* pDocTemplate = GetDocTemplate();
		if ((AfxGetThreadState()->m_lastSentMsg.message != WM_CLOSE) && 
			pDocTemplate->IsKindOf(RUNTIME_CLASS(CCeDocListDocTemplate)))
		{
			pView->DestroyWindow();
			continue;
		}
