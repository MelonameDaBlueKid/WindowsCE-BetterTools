...
BOOL AFXAPI _AfxInitBlob(HGLOBAL* phDst, void* pvSrc)
{
	...
	if ((cb = *(long*)pvSrc) > 0)
	{
		...
		*phDst = WCE_FCTN(GlobalAlloc)(GMEM_MOVEABLE, sizeof(cb) + cb);
		if (*phDst != NULL)
		{
			void* pvDst = WCE_FCTN(GlobalLock)(*phDst);
			ASSERT(pvDst != NULL);
			WCE_FCTN(memcpy)(pvDst, pvSrc, sizeof(cb) + cb);
			bResult = TRUE;
			WCE_FCTN(GlobalUnlock)(*phDst);
		}
	}
	return bResult;
}
...
{
	BOOL bResult = FALSE;
	void* pvSrc = WCE_FCTN(GlobalLock)(hSrc);
	if (pvSrc != NULL)
	{
		bResult = _AfxInitBlob(phDst, pvSrc);
		WCE_FCTN(GlobalUnlock)(hSrc);
	}
	return bResult;
}
	...
	HGLOBAL* phBlob, HGLOBAL /*hBlobDefault*/)
{
	...
	if (m_bLoading)
	{
		...
		*phBlob = WCE_FCTN(GlobalAlloc)(GMEM_MOVEABLE, sizeof(cb)+cb);
		if (*phBlob != NULL)
		{
			void* pvBlob = WCE_FCTN(GlobalLock)(*phBlob);
			ASSERT(pvBlob != NULL);
			*(long*)pvBlob = cb;
		...
		else
		{
			...
			if (*ppUnk != NULL)
			{
						...
						IID_IPersistStreamInit, (void**)&pps)))
				{
					ASSERT_POINTER(pps, IPersistStream);
					LPSTREAM pstm = _AfxGetArchiveStream(m_ar, stm);
					bResult = SUCCEEDED(::WCE_FCTN(OleSaveToStream)(pps, pstm));
					pps->Release();
				}
			}
			else
