	...
	WORD wVerMinor, LCID lcid)
{
	...
	HKEY hKeyTypeLib;
	if (WCE_FCTN(RegOpenKey)(HKEY_CLASSES_ROOT, szKeyTypeLib, &hKeyTypeLib) ==
		ERROR_SUCCESS)
	{
		int iKeyVersion = 0;
		HKEY hKeyVersion;
		...
		while (WCE_FCTN(RegEnumKey)(hKeyTypeLib, iKeyVersion, szVersion, _MAX_PATH) ==
			ERROR_SUCCESS)
		{
			hKeyVersion = NULL;
			BOOL bSurgicalVersion = FALSE;

			if (WCE_FCTN(RegOpenKey)(hKeyTypeLib, szVersion, &hKeyVersion) !=
				ERROR_SUCCESS)
			{
				++iKeyVersion;
				continue;
			...
			while (WCE_FCTN(RegEnumKey)(hKeyVersion, iKeyLocale, szLocale, _MAX_PATH) ==
				ERROR_SUCCESS)
			{
				...
				if (WCE_FCTN(RegOpenKey)(hKeyVersion, szLocale, &hKeyLocale) !=
					ERROR_SUCCESS)
				{
					++iKeyLocale;
					continue;
				...
				HKEY hkey;
				if (WCE_FCTN(RegOpenKey)(hKeyLocale, TYPELIBWIN_2, &hkey) ==
					ERROR_SUCCESS)
				{
					RegCloseKey(hkey);
	...
	WORD wVerMinor)
{
	...
	if (::WCE_FCTN(RegCreateKey)(HKEY_CLASSES_ROOT, szScratch, &hkeyClassID) !=
		ERROR_SUCCESS)
		goto Error;
	if (::WCE_FCTN(RegCreateKey)(HKEY_CLASSES_ROOT, pszProgID, &hkeyProgID) !=
		ERROR_SUCCESS)
		goto Error;
	...
	{
		bSuccess =
			(::WCE_FCTN(RegSetValue)(hkeyProgID, _T("Insertable"), REG_SZ, _T(""), 0) ==
				ERROR_SUCCESS) &&
				(::WCE_FCTN(RegSetValue)(hkeyClassID, _T("Insertable"), REG_SZ, _T(""), 0) ==
				ERROR_SUCCESS);
	}
	...
	if (nRegFlags & afxRegApartmentThreading)
	{
		...
		bSuccess = (::WCE_FCTN(RegOpenKey)(hkeyClassID, _T("InprocServer32"),
...
BOOL AFXAPI AfxOleUnregisterClass(REFCLSID clsid, LPCTSTR pszProgID)
{
	...
	HKEY hkey;
	BOOL bSurgical = WCE_FCTN(RegOpenKey)(HKEY_CLASSES_ROOT, szKey, &hkey) ==
		ERROR_SUCCESS;
	...
	REFCLSID clsid, UINT idTypeName, int nRegFlags)
{
	...
	TCHAR szKey[_MAX_PATH];
	wsprintf(szKey, _T("CLSID\\%s"), lpszClassID);
	if (::WCE_FCTN(RegCreateKey)(HKEY_CLASSES_ROOT, szKey, &hkeyClassID) !=
		ERROR_SUCCESS)
		goto Error;
