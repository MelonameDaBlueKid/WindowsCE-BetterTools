...
{
	ASSERT(hMenuShared != NULL && WCE_FCTN(IsMenu)(hMenuShared));
	ASSERT(hMenuSource != NULL && WCE_FCTN(IsMenu)(hMenuSource));

	BOOL bHelpMergedAsSubMenu = FALSE;
	HMENU hHelpSubMenu = NULL;

	// copy the popups from the pMenuSource
	int cMenuItems = WCE_FCTN(GetMenuItemCount)(hMenuSource);
	int cGroupWidth = 0;
	int nPosition = 0;
	...
	for (int i = 0; i < cMenuItems; i++)
	{
		...
		UINT state = WCE_FCTN(GetMenuState)(hMenuSource, i, MF_BYPOSITION);
		...
		else
		{
			...
			int nLen = WCE_FCTN(GetMenuString)(hMenuSource, i, szItemText,
			...
			if (hMenuPopup != NULL)
			{
				...
				else if (::WCE_FCTN(GetMenuItemCount)(hMenuPopup) != 0)
				{
			...
			else if (nLen > 0)
			{
				...
				InsertMenu(hMenuShared, nPosition, state | MF_BYPOSITION,
					WCE_FCTN(GetMenuItemID)(hMenuSource, i), szItemText);
				++nPosition;
				++cGroupWidth;
			}
		}
	...
	HMENU hHelpMenuPopup /* = NULL */)
{
	ASSERT(hMenuShared != NULL && WCE_FCTN(IsMenu)(hMenuShared));
	ASSERT(hMenuSource != NULL && WCE_FCTN(IsMenu)(hMenuSource));
	ASSERT(hHelpMenuPopup == NULL || WCE_FCTN(IsMenu)(hHelpMenuPopup));

	int cOurItems = WCE_FCTN(GetMenuItemCount)(hMenuSource);
	int cMenuItems = WCE_FCTN(GetMenuItemCount)(hMenuShared);
	...
	for (int i = cMenuItems-1; i >= 0; i--)
	{
		...
		if (hMenuPopup != NULL)
		{
			...
			{
				int cPopupItems = ::WCE_FCTN(GetMenuItemCount)(hMenuPopup);
...
AFX_STATIC HGLOBAL AFXAPI _AfxCopyGlobalMemory(HGLOBAL hDest, HGLOBAL hSource)
{
	...
	DWORD nSize = ::WCE_FCTN(GlobalSize)(hSource);
	if (hDest == NULL)
	{
		hDest = ::WCE_FCTN(GlobalAlloc)(GMEM_SHARE|GMEM_MOVEABLE, nSize);
		if (hDest == NULL)
			return NULL;
	}
	else if (nSize > ::WCE_FCTN(GlobalSize)(hDest))
	{
	...
	// copy the bits
	LPVOID lpSource = ::WCE_FCTN(GlobalLock)(hSource);
	LPVOID lpDest = ::WCE_FCTN(GlobalLock)(hDest);
	ASSERT(lpDest != NULL);
	ASSERT(lpSource != NULL);
	WCE_FCTN(memcpy)(lpDest, lpSource, nSize);
	::WCE_FCTN(GlobalUnlock)(hDest);
	::WCE_FCTN(GlobalUnlock)(hSource);
...
// WinCE: TYMED_ENHMF is not supported by the OS
			ASSERT(FALSE);
#endif // _WIN32_WCE
		case TYMED_HGLOBAL:
		...
		case TYMED_MFPICT:
			{
#if defined(_WIN32_WCE)
	// WinCE: TYMED_MFPICT is not supported by the OS
	ASSERT(FALSE);
#else // _WIN32_WCE
				...
				if (hDest == NULL)
					return FALSE;
				LPMETAFILEPICT lpPict = (LPMETAFILEPICT)::WCE_FCTN(GlobalLock)(hDest);
				ASSERT(lpPict != NULL);
		...
		case TYMED_GDI:
#if defined(_WIN32_WCE)
// WinCE: TYMED_GDI is not supported by the OS
			ASSERT(FALSE);
#else // _WIN32_WCE
		...
		case TYMED_FILE:
			{
				...
				if (szFileName == NULL)
					return FALSE;
				WCE_FCTN(memcpy)(szFileName, lpSource->lpszFileName,  (cbSrc+1)*sizeof(OLECHAR));
				return TRUE;
			}
	...
	case TYMED_GDI:
		{
#if defined(_WIN32_WCE)
// WinCE: TYMED_ENHMF and TYMED_GDI are not supported by the OS
			ASSERT(FALSE);
#else // _WIN32_WCE
			ASSERT(sizeof(HGLOBAL) == sizeof(HENHMETAFILE));
	...
	LPCOLESTR   lpszSrcOfCopy)
{
	...
	hMem = WCE_FCTN(GlobalAlloc)(GMEM_MOVEABLE | GMEM_SHARE | GMEM_ZEROINIT,
	   dwObjectDescSize + (dwFullUserTypeNameLen + dwSrcOfCopyLen) *
		sizeof(OLECHAR));
	if (!hMem)
		return NULL;

	lpOD = (LPOBJECTDESCRIPTOR)WCE_FCTN(GlobalLock)(hMem);
	...
	LPSIZEL         lpSizelHim)
{
	...
	else if (lpszSrcOfCopy == NULL)
	{
		...
		{
#if defined(_WIN32_WCE)
			TCHAR szUnspecifiedName[] = _T("Unspecified");
			lpszSrcOfCopy = (LPOLESTR)::CoTaskMemAlloc(sizeof(szUnspecifiedName));
			if(lpszSrcOfCopy != NULL)
				_tcscpy((TCHAR*)lpszSrcOfCopy,szUnspecifiedName);
#else // _WIN32_WCE
			CreateBindCtx(0, &pbc);
...
DVTARGETDEVICE* AFXAPI _AfxOleCreateTargetDevice(LPDEVNAMES pDN, LPDEVMODE pDM)
{
	...
	if ((ptd = (DVTARGETDEVICE*)CoTaskMemAlloc(dwPtdSize)) != NULL)
	{
		...
		WCE_FCTN(memcpy)((BYTE*)ptd + ptd->tdExtDevmodeOffset, lpDevModeOle,
...
DVTARGETDEVICE* AFXAPI _AfxOleCreateTargetDevice(LPPRINTDLG lpPrintDlg)
{
	...
	if ((pDN = (LPDEVNAMES)WCE_FCTN(GlobalLock)(lpPrintDlg->hDevNames)) == NULL)
		return NULL;

	if ((pDM = (LPDEVMODE)WCE_FCTN(GlobalLock)(lpPrintDlg->hDevMode)) == NULL)
	{
		WCE_FCTN(GlobalUnlock)(lpPrintDlg->hDevNames);
		return NULL;
	}
	...
	WCE_FCTN(GlobalUnlock)(lpPrintDlg->hDevNames);
	WCE_FCTN(GlobalUnlock)(lpPrintDlg->hDevMode);
...
DVTARGETDEVICE* AFXAPI _AfxOleCopyTargetDevice(DVTARGETDEVICE* ptdSrc)
{
	...
	WCE_FCTN(memcpy)(ptdDest, ptdSrc, (size_t)ptdSrc->tdSize);
	return ptdDest;
}
...
void AFXAPI _AfxDeleteMetafilePict(HGLOBAL hMetaPict)
{
	if (hMetaPict != NULL)
	{
		...
		WCE_FCTN(ReleaseStgMedium)(&stgMedium);
	}
}
#endif // _WIN32_WCE

#define HIMETRIC_PER_INCH   2540
#define MAP_PIX_TO_LOGHIM(x,ppli)   WCE_FCTN(MulDiv)(HIMETRIC_PER_INCH, (x), (ppli))
#define MAP_LOGHIM_TO_PIX(x,ppli)   WCE_FCTN(MulDiv)((ppli), (x), HIMETRIC_PER_INCH)
