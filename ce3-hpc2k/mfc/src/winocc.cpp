...
void CWnd::CheckDlgButton(int nIDButton, UINT nCheck)
{
	...
	if (m_pCtrlCont == NULL)
		::WCE_FCTN(CheckDlgButton)(m_hWnd, nIDButton, nCheck);
	else
...
int CWnd::GetDlgItemText(int nID, LPTSTR lpStr, int nMaxCount) const
{
	...
	if (m_pCtrlCont == NULL)
		return ::WCE_FCTN(GetDlgItemText)(m_hWnd, nID, lpStr, nMaxCount);
	else
	...
	LPARAM lParam)
{
	...
	if (m_pCtrlCont == NULL)
		return ::WCE_FCTN(SendDlgItemMessage)(m_hWnd, nID, message, wParam, lParam);
	else
...
void CWnd::SetDlgItemText(int nID, LPCTSTR lpszString)
{
	...
	if (m_pCtrlCont == NULL)
		::WCE_FCTN(SetDlgItemText)(m_hWnd, nID, lpszString);
	else
...
UINT CWnd::IsDlgButtonChecked(int nIDButton) const
{
	...
	if (m_pCtrlCont == NULL)
		return ::WCE_FCTN(IsDlgButtonChecked)(m_hWnd, nIDButton);
	else
