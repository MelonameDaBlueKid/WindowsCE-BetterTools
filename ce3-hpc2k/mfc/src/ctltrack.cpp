...
AFX_STATIC void AFXAPI _AfxOffsetTrackerRect(CRect& rect, CWnd* pWnd)
{
	...
	int dx = -nBorders * WCE_FCTN(GetSystemMetrics)(SM_CXBORDER);
	int dy = -nBorders * WCE_FCTN(GetSystemMetrics)(SM_CYBORDER);

	if (dwExStyle & WS_EX_LEFTSCROLLBAR)
		dx -= WCE_FCTN(GetSystemMetrics)(SM_CXVSCROLL);
...
void COleControl::OnNcPaint()
{
	...
	if (dwScrollStyle == (WS_HSCROLL | WS_VSCROLL))
	{
		...
		int cxVScroll = WCE_FCTN(GetSystemMetrics)(SM_CXVSCROLL);
		...
		rc.top = rc.bottom - WCE_FCTN(GetSystemMetrics)(SM_CYVSCROLL);
