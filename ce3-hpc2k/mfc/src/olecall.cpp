...
_AfxDispatchCall(AFX_PMSG /*pfn*/, void* /*pArgs*/, UINT /*nSizeArgs*/)
{
    ...
    // in a special way. So AfxDispatchCall will not be used for SH4. Rather,
    // a special function will be called instead of AfxDispatchCall. The reason
    // for special handling is because of the way floating point values are
    // passed on the stack. At present, none of the other architectures bother
    // about floating point values at all. As a result, for SH4, we need to
    // generate a thunk on the fly, encode and execute it dynamically. At
    // present, for other architectures, we are statically creating a thunk
    // that will work well for int arguments. Eventually other architectures
    // will have to follow SH4's example.
    __asm("nop\n");
