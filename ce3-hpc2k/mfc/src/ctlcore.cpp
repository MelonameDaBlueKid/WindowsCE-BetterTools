...
WCE_DEL ON_WM_MOUSEACTIVATE()
WCE_DEL ON_WM_NCLBUTTONDOWN()
    ON_WM_LBUTTONDOWN()
    ON_WM_LBUTTONUP()
    ON_WM_LBUTTONDBLCLK()
    ON_WM_MBUTTONDOWN()
...
{
    // there is no default MFC implementation
    return TRUE;
}
#else // _WIN32_WCE
BOOL COleControl::OnProperties(LPMSG, HWND hWndParent, LPCRECT)
...
COLORREF COleControl::TranslateColor(OLE_COLOR clrColor, HPALETTE hpal)
{
    ...
    ::WCE_FCTN(OleTranslateColor)(clrColor, hpal, &cr);
    return cr;
}
...
STDMETHODIMP COleControl::XSpecifyPropertyPages::GetPages(CAUUID* pPages)
{
    ...
    if (cElems > 0)
    {
        if ((pPages->pElems = (LPCLSID)(CoTaskMemAlloc(cElems * sizeof(CLSID)))) != NULL)
        {
            ...
            WCE_FCTN(memcpy)(pPages->pElems, pClassID, (int)(cElems * sizeof(CLSID)));
        }
        else
            hr = E_OUTOFMEMORY;
    }
