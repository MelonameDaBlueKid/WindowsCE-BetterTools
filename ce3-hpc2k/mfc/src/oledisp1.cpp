...
#if defined(_SH4_) // fix for 22800, currently only available for SH4
#include "SH4ThunkBldr.h"
#endif // _SH4_
...
#endif // _WIN32_WCE_CEF	// push any necessary return value stuff on the stack (pre args)
	...
	VARIANT* pvarResult, DISPPARAMS* pDispParams, UINT* puArgErr)
{
...
#if defined(_WIN32_WCE_CEF)
	AFX_PMSG pfn = *(AFX_PMSG*)&pEntry->pfn; // force-cast via pointers
#else // _WIN32_WCE_CEF
	...
	if (wFlags & (DISPATCH_PROPERTYPUT|DISPATCH_PROPERTYPUTREF))
	{
		...
		WCE_FCTN(memcpy)(pbPropSetParams, pbParams, nParams);
...
#if defined(_WIN32_WCE_CEF)
		pfn = *(AFX_PMSG*)&pEntry->pfnSet; // force-cast via pointers
#else // _WIN32_WCE_CEF
		pfn = pEntry->pfnSet;
#endif // _WIN32_WCE_CEF
		vtResult = VT_EMPTY;
	...
	pStack += _STACK_OFFSET;
...
#if defined(_SH4_) // for now, this fix is only for SH4
        // Fix for 22800: Compute an array of parameter types and sizes. We
        // only need to know whether a given parameter should go into an int
		...
		unsigned buffer[32]; // thunks on SH4 cannot be larger than 32 bytes

        // We need to walk through our argument list to obtain the necessary
        // information.
	...
	if (sc == S_OK)
	{
		TRY
		{
#if !defined(_SH4_) // fix for 22800
			// PushStackArgs will fail on argument mismatches
			DWORD (AFXAPI *pfnDispatch)(AFX_PMSG, void*, UINT) =
				&_AfxDispatchCall;
#else // _SH4_
            // SH4 has a split cache. It is a good idea to flush it before we execute our dynamically generated thunk.
			...
			case VT_R4:
				result.fltVal = 
#if !defined(_SH4_)
                ((float (AFXAPI*)(AFX_PMSG, void*, UINT))
					pfnDispatch)(pfn, pStack, nSizeArgs);
#else // _SH4_
                ((float (*)(AFX_PMSG, void*, UINT))
			...
			case VT_R8:
				result.dblVal = 
#if !defined(_SH4_)
                ((double (AFXAPI*)(AFX_PMSG, void*, UINT))
					pfnDispatch)(pfn, pStack, nSizeArgs);
#else // _SH4_
                ((double (*)(AFX_PMSG, void*, UINT))
                    (unsigned *)buffer)(pfn, pStack, nSizeArgs);
#endif // _SH4_
				break;
			case VT_DATE:
				result.dblVal =
#if !defined(_SH4_)
                ((DATE (AFXAPI*)(AFX_PMSG, void*, UINT))
					pfnDispatch)(pfn, pStack, nSizeArgs);
#else // _SH4_
                ((DATE (*)(AFX_PMSG, void*, UINT))
                    (unsigned *)buffer)(pfn, pStack, nSizeArgs);
#endif // _SH4_
				break;
			...
			default:
                ...
                pfnDispatch(pfn, pStack, nSizeArgs);
#else // _SH4_
                ((unsigned long (*)(AFX_PMSG, void*, UINT))
                    (unsigned *)buffer)(pfn, pStack, nSizeArgs);
#endif // _SH4_
				break;
			}
