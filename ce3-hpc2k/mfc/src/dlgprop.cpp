...
void CPropertyPage::CommonConstruct(LPCTSTR lpszTemplateName, UINT nIDCaption)
{
	...
	m_psp.pfnDlgProc = WCE_IF(wce_FirstDlgProc, AfxDlgProc);
...
CPropertyPage::~CPropertyPage()
{
	...
	if (m_hDialogTemplate != NULL)
		WCE_FCTN(GlobalFree)(m_hDialogTemplate);
}
...
void CPropertyPage::PreProcessPageTemplate(PROPSHEETPAGE& psp, BOOL bWizard)
{
	...
	else
	{
		...
		//LockResource() gives us read-only memory with the PSP_DLGINDIRECT bit set
		//According to PROPSHEETPAGE docs,
		// this is an invalid combination that will cause a memory exception in COMMCTRL.DLL
		// so for CE we change it to read/write
		DWORD dwResult;
		MEMORY_BASIC_INFORMATION mbiMemory;

		memset (&mbiMemory, 0, sizeof(MEMORY_BASIC_INFORMATION));
		dwResult = VirtualQuery ((void*)pTemplate, &mbiMemory, sizeof(MEMORY_BASIC_INFORMATION));
		if (sizeof(MEMORY_BASIC_INFORMATION) == dwResult)
		{
			if (mbiMemory.Protect != PAGE_READWRITE && mbiMemory.Protect != PAGE_EXECUTE_READWRITE && mbiMemory.Protect != PAGE_EXECUTE_WRITECOPY)
			{
				// Mark the dialog template as read-write.
				if(!VirtualProtect((void*)pTemplate, sizeof(DLGTEMPLATE), PAGE_READWRITE, &dwResult))
					return;
			}
		}
#endif  // _WIN32_WCE
	...
	if (((DLGTEMPLATEEX*)pTemplate)->signature == 0xFFFF)
	{
		...
		if (dwVersion & 0x80000000)
		{
			...
			HINSTANCE hInst = LoadLibrary(WCE_IF(_T("COMMCTRL.DLL"),_T("COMCTL32.DLL")));
			ASSERT(hInst != NULL);
			if (hInst != NULL)
			{
...
void CPropertyPage::AssertValid() const
{
	...
	ASSERT(m_psp.pfnDlgProc == WCE_IF(wce_FirstDlgProc, AfxDlgProc));
...
void CPropertySheet::CommonConstruct(CWnd* pParentWnd, UINT iSelectPage)
{
	...
	m_psh.dwFlags = PSH_PROPSHEETPAGE | WCE_IF(PSH_NOAPPLYNOW, 0);
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
	int CALLBACK AfxPropSheetCallback(HWND hwndDlg, UINT message, LPARAM lParam);
...
#if defined(_WIN32_WCE)
// WinCE: Tweak the m_pWndInit... we reserve that for the dialog pages.
	_AFX_THREAD_STATE* pThreadState = AfxGetThreadState();
	pThreadState->m_pWndInitPropertySheet = pThreadState->m_pWndInit;
...
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
int CALLBACK 
AfxPropSheetCallback(HWND hwndDlg, UINT message, LPARAM lParam)
{
	switch(message) 
	{
		case PSCB_INITIALIZED:
		{
			HWND hWnd = GetDlgItem(hwndDlg, AFX_IDC_TAB_CONTROL);
			DWORD dwStyle = GetWindowLong(hWnd, GWL_STYLE) | TCS_BOTTOM;
...
BOOL CPropertySheet::Create(CWnd* pParentWnd, DWORD dwStyle, DWORD dwExStyle)
{
	...
	if (dwStyle == (DWORD)-1)
	{
		pState->m_dwPropStyle = DS_MODALFRAME | DS_3DLOOK | DS_CONTEXTHELP |
					WCE_IF(DS_CENTER, 0) | DS_SETFONT | WS_POPUP | WS_VISIBLE | WS_CAPTION;
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
// WinCE: 3-D controls are disabled on PSPC 2.0
		pState->m_dwPropStyle &= ~DS_3DLOOK;
#endif // _WIN32_WCE_PSPC
...
#if !defined(_WIN32_WCE) || (defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300))
...
void CPropertySheet::BuildPropPageArray()
{
    ...
    BOOL bWizard = WCE_IF(FALSE,(m_psh.dwFlags & (PSH_WIZARD | PSH_WIZARD97)));
	for (int i = 0; i < m_pages.GetSize(); i++)
	{
		CPropertyPage* pPage = GetPage(i);
		memcpy(&ppsp[i], &pPage->m_psp, sizeof(pPage->m_psp));
#if !defined(_WIN32_WCE) || (defined(_WIN32_WCE) && (_WIN32_WCE >= 300))
// WinCE:  there are two problems with PreProcessPageTemplate: changing fonts doesn't work (not really necessary),
// and LockResource() gives us read-only memory with the PSP_DLGINDIRECT bit set.  According to PROPSHEETPAGE docs,
// this is an invalid combination that will cause a memory exception in COMMCTRL.DLL
...
void CPropertySheet::AddPage(CPropertyPage* pPage)
{
	...
	if (m_hWnd != NULL)
	{
		// build new prop page array
		AFX_OLDPROPSHEETPAGE *ppsp = new AFX_OLDPROPSHEETPAGE[m_pages.GetSize()];
		WCE_FCTN(memcpy)(ppsp, m_psh.ppsp, sizeof(AFX_OLDPROPSHEETPAGE) * (m_pages.GetSize()-1));
	    delete[] (PROPSHEETPAGE*)m_psh.ppsp;
		m_psh.ppsp = (PROPSHEETPAGE*)ppsp;
		ppsp += m_pages.GetSize()-1;

		// copy processed PROPSHEETPAGE struct to end
		WCE_FCTN(memcpy)(ppsp, &pPage->m_psp, sizeof(pPage->m_psp));
#if !defined(_WIN32_WCE) || (defined(_WIN32_WCE) && (_WIN32_WCE >= 300))
		...
		wce_AdjustPropertySheetSize(this);
#endif // _WIN32_WCE_PSPC
#endif
	}
}
...
#if defined(_WIN32_WCE)
// WinCE: The OK button is somewhere else (i.e. hWndCtrl isn't set).  
// Check for it differently (and without the robustness).
	if (nID == IDOK)
	{
		if(m_bModeless)
			EndDialog(nID);
...
void CPropertySheetEx::BuildPropPageArray()
{
	...
	{
        CPropertyPage* pPage = GetPage(i);
        WCE_FCTN(memcpy)(&ppsp[i], &pPage->m_psp, sizeof(pPage->m_psp));
...
void CPropertySheetEx::AddPage(CPropertyPageEx* pPage)
{
	...
	if (m_hWnd != NULL)
	{
		// build new prop page array
		PROPSHEETPAGE *ppsp = new PROPSHEETPAGE[m_pages.GetSize()];
		WCE_FCTN(memcpy)(ppsp, m_psh.ppsp, sizeof(PROPSHEETPAGE) * (m_pages.GetSize()-1));
		delete[] (PROPSHEETPAGE*)m_psh.ppsp;
		m_psh.ppsp = ppsp;
		ppsp += m_pages.GetSize()-1;
...
WCE_DEL IMPLEMENT_DYNAMIC(CPropertyPageEx, CPropertyPage)
WCE_DEL IMPLEMENT_DYNAMIC(CPropertySheetEx, CPropertySheet)
