...
void COleDataSource::Empty()
{
	if (m_pDataCache != NULL)
	{
		...
		for (UINT nIndex = 0; nIndex < m_nSize; nIndex++)
		{
			...
			WCE_FCTN(ReleaseStgMedium)(&m_pDataCache[nIndex].m_stgMedium);
		}
	...
	LPFORMATETC lpFormatEtc, DATADIR nDataDir)
{
	...
	if (pEntry != NULL)
	{
		...
		WCE_FCTN(ReleaseStgMedium)(&pEntry->m_stgMedium);
	}
	else
	{
		...
		if (m_pDataCache == NULL || m_nSize == m_nMaxSize)
		{
			...
			if (m_pDataCache != NULL)
			{
				WCE_FCTN(memcpy)(pCache, m_pDataCache, m_nSize * sizeof(AFX_DATACACHE_ENTRY));
				delete[] m_pDataCache;
			}
			m_pDataCache = pCache;
		}
	...
	LPFORMATETC lpFormatEtc, LPSTGMEDIUM lpStgMedium)
{
	...
	if (lpFormatEtc->tymed & TYMED_HGLOBAL)
	{
		...
		WCE_IF(CMemFile, CSharedFile) file;
		if (lpStgMedium->tymed == TYMED_HGLOBAL)
		{
			ASSERT(lpStgMedium->hGlobal != NULL);
#if defined(_WIN32_WCE)
			BYTE* lpBuf = (BYTE*)LocalLock( lpStgMedium->hGlobal );
			ASSERT(lpBuf);
			file.Attach( lpBuf, LocalSize(lpStgMedium->hGlobal) );
#else // _WIN32_WCE
		...
		if (OnRenderFileData(lpFormatEtc, &file))
		{
			lpStgMedium->tymed = TYMED_HGLOBAL;
#if defined(_WIN32_WCE)
			LocalUnlock( lpStgMedium->hGlobal );
#endif // _WIN32_WCE
...
void CEnumFormatEtc::AddFormat(const FORMATETC* lpFormatEtc)
{
	...
	if (m_nSize == m_nMaxSize)
	{
		...
		m_nMaxSize += 10;
		WCE_FCTN(memcpy)(pListNew, m_pvEnum, m_nSize*sizeof(FORMATETC));
		delete m_pvEnum;
