...
void CWordArray::SetSize(int nNewSize, int nGrowBy)
{
	...
	else
	{
		...
		WCE_FCTN(memcpy)(pNewData, m_pData, m_nSize * sizeof(WORD));
...
int CWordArray::Append(const CWordArray& src)
{
	...
	WCE_FCTN(memcpy)(m_pData + nOldSize, src.m_pData, src.m_nSize * sizeof(WORD));
...
void CWordArray::Copy(const CWordArray& src)
{
	...
	WCE_FCTN(memcpy)(m_pData, src.m_pData, src.m_nSize * sizeof(WORD));
