	...
	LPCTSTR lpszFilterName, LPCTSTR lpszFilterExt)
{
	...
	if (nIconIndex != 0)
	{
		...
		HICON hIcon = (HICON)::ExtractIconEx(strPathName,nIconIndex,NULL,NULL,1);
#else // _WIN32_WCE
	...
	HKEY hKeyRoot /* = HKEY_CLASSES_ROOT */)
{
	...
	if (hKeyRoot == HKEY_CLASSES_ROOT)
		WCE_FCTN(RegOpenKey(HKEY_CLASSES_ROOT, _T("CLSID"), &hKeyTemp));
	...
	HKEY hKeyRoot /* = HKEY_CLASSES_ROOT */)
{
	...
	while (*rglpszRegister != NULL)
	{
		...
		if (!bReplace)
		{
			...
			if (::WCE_FCTN(RegQueryValue)(hKeyRoot, strKey, szBuffer, &lSize) ==
				ERROR_SUCCESS)
			{
#ifdef _DEBUG
		...
		if (::WCE_FCTN(RegSetValue)(hKeyRoot, strKey, REG_SZ, strValue, lstrlen(strValue) * sizeof(TCHAR))
			!= ERROR_SUCCESS)
		{
