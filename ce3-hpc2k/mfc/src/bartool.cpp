...
AfxLoadSysColorBitmap(HINSTANCE hInst, HRSRC hRsrc, BOOL bMono)
{
	...
	if (lpBitmapInfo == NULL)
		return NULL;
	WCE_FCTN(memcpy)(lpBitmapInfo, lpBitmap, nSize);
...
#if defined(_WIN32_WCE)
// WinCE: default to fixed size toolbars so we get wrapping without asking for it
	if(!(dwStyle & CBRS_SIZE_DYNAMIC))
		dwStyle |= CBRS_SIZE_FIXED; // default to wrapped toolbars
#endif // _WIN32_WCE
...
#if defined(_WIN32_WCE)
// WinCE: Make up for no WM_NCCREATE
	if (m_hWndOwner != NULL)
		DefWindowProc(TB_SETPARENT, (WPARAM)m_hWndOwner, 0);
	DefWindowProc(TB_BUTTONSTRUCTSIZE, (WPARAM)sizeof(TBBUTTON), 0);
#endif // _WIN32_WCE
...
BOOL CToolBar::LoadBitmap(LPCTSTR lpszResourceName)
{
	...
	hbmImageWell = ::LoadBitmap(hInstImageWell, lpszResourceName);
	HIMAGELIST hImageList = (HIMAGELIST)DefWindowProc(TB_GETIMAGELIST, 0, 0);
	if((m_pWndCommandBar != NULL) && (hImageList != NULL) && 
		m_pWndCommandBar->m_bAppendBitmaps)
	{
		// Wince: Don't replace existing bitmaps because they might be adornments,
		// so append to the ImageList directly.  
		if(ImageList_Add(hImageList, hbmImageWell, NULL) < 0)
			return FALSE;
	}
	else
	{
		if (!AddReplaceBitmap(hbmImageWell))
			return FALSE;
		if(m_pWndCommandBar != NULL)
			m_pWndCommandBar->m_bAppendBitmaps = TRUE;
	}
#else // _WIN32_WCE
	// load the bitmap
	HBITMAP hbmImageWell;
...
#if defined(_WIN32_WCE)
		bResult = TRUE; // Under some cases, TB_ADDBIMAP works, but it returns FALSE
#endif // _WIN32_WCE
	}
	else
	{
...
BOOL CToolBar::SetButtons(const UINT* lpIDArray, int nIDCount) 
{
	...
	if(m_pWndCommandBar != NULL)
	{
		// Offset bitmap indices 
		HIMAGELIST hImageList = (HIMAGELIST)DefWindowProc(TB_GETIMAGELIST, 0, 0);
		if(hImageList != NULL)
			iImageStart = ImageList_GetImageCount(hImageList);
	}
	else
#endif // _WIN32_WCE
	while (nCount--)
	...
	if (lpIDArray != NULL)
	{
		...
		int iImage = WCE_IF(iImageStart, 0);
		for (int i = 0; i < nIDCount; i++)
		{
			...
			{
#if defined(_WIN32_WCE)
// WinCE: Don't command ID's that are reserved for adornments
				if(m_pWndCommandBar != NULL)
				{
					VERIFY((button.idCommand != IDOK) && 
						   (button.idCommand != WM_HELP) && 
						   (button.idCommand != WM_CLOSE));
				}
#endif // _WIN32_WCE
...
void CToolBar::_SetButton(int nIndex, TBBUTTON* pButton)
{
	...
	if (memcmp(pButton, &button, sizeof(TBBUTTON)) != 0)
	{
		...
		if(m_pWndCommandBar != NULL)
		{
			BOOL bEnable = pButton->fsState & TBSTATE_ENABLED;
			m_pWndCommandBar->SendMessage(TB_ENABLEBUTTON, pButton->idCommand, bEnable);
		}
		else
		{
...
#if defined(_WIN32_WCE) && defined(_PPC_)
#pragma optimize( "", off )
#endif // _WIN32_WCE
...
#if defined(_WIN32_WCE)
// WinCE: Get the height of custom controls 
			if(CCeCommandBar::IsCustomControlButton(pData[i]))
...
void CToolBar::SetButtonInfo(int nIndex, UINT nID, UINT nStyle, int iImage)
{
	...
	_GetButton(nIndex, &button);
	TBBUTTON save;
	WCE_FCTN(memcpy)(&save, &button, sizeof(save));
...
WCE_DEL	ON_WM_NCHITTEST()
WCE_DEL	ON_WM_NCPAINT()
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
WCE_DEL	ON_WM_NCCALCSIZE()
WCE_DEL	ON_WM_WINDOWPOSCHANGING()
WCE_DEL	ON_WM_NCCREATE()
WCE_DEL	ON_MESSAGE(TB_SETBITMAPSIZE, OnSetBitmapSize)
WCE_DEL	ON_MESSAGE(TB_SETBUTTONSIZE, OnSetButtonSize)
WCE_DEL	ON_MESSAGE(WM_SETTINGCHANGE, OnPreserveZeroBorderHelper)
WCE_DEL	ON_MESSAGE(WM_SETFONT, OnPreserveZeroBorderHelper)
WCE_DEL ON_WM_SYSCOLORCHANGE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CToolCmdUI::Enable(BOOL bOn)
{
	...
	ASSERT(pToolBar->IsKindOf(RUNTIME_CLASS(CToolBar)) || 
	       pToolBar->IsKindOf(RUNTIME_CLASS(CCeCommandBar)));
#else // _WIN32_WCE
...
#if defined(_WIN32_WCE)
// WinCE: Don't disable close adornment		
			if(m_pWndCommandBar != NULL)
			{
				if((button.idCommand == WM_HELP) || (button.idCommand == WM_CLOSE))
				continue;
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
				// WinCE (Rapier)  Don't disable menu or SharedNew button
				if ((button.fsStyle & TBSTYLE_DROPDOWN) || 
					(button.idCommand == IDM_SHAREDNEW) ||
					(button.idCommand == IDM_SHAREDNEWDEFAULT))
				continue;
#endif // _WIN32_WCE_PSPC
			}
#endif // _WIN32_WCE			
