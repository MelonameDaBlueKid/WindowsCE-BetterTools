...
public:
	...
	void Init(WNDPROC proc, void* pThis)
	{
		...
		thunk.m_jmp  = 0xe9;
		// The i386 thunk comes across a pointer in the process slot *and* 
		// a pointer in the zero slot, so we need to normalize the pointers.
		thunk.m_relproc = (int)MapPtrToProcess(proc, GetCurrentProcess()) - 
			(((int)MapPtrToProcess(this,GetCurrentProcess())) + sizeof(_WndProcThunk));
#elif defined(_MIPS_) || defined(_MIPS16_)
