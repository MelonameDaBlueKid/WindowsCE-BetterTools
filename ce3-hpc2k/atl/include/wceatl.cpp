...
_AtlceDispatchCall(DWORD /*pfn*/, void* /*pArgs*/, UINT /*nSizeArgs*/)
{
	...
	// So AtlceDispatchCall will not be used for SH4. Rather, a special function will be called
	...
	// bother about floating point values at all. As a result, for SH4, we need to generate
	...
	// Eventually other architectures will have to follow SH4's example.
...
HRESULT wce_ATL_ProgIDFromCLSID(REFCLSID clsid, LPOLESTR *lplpszProgID)
{
	...
	*lplpszProgID = NULL;
	...
	if(StringFromCLSID(clsid, &lpszCLSID) == S_OK)
	{
		...
		{
			TCHAR strValueName[60];
			DWORD dwValueNameLength = sizeof(strValueName), dwType, cbValue;
