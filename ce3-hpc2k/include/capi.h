...
//
// the parameters are:
// Algortihm - the algorithm used to encrypt the PrivateKeyInfo 
...
typedef BOOL (CALLBACK *PCRYPT_DECRYPT_PRIVATE_KEY_FUNC)( 
                        CRYPT_ALGORITHM_IDENTIFIER  Algortihm,
                        CRYPT_DATA_BLOB             EncryptedPrivateKey,
                        BYTE                        *pClearTextKey,
                        DWORD                       *cbClearTextKey,
...
//+-------------------------------------------------------------------------
typedef struct _CRYPT_PRIVATE_KEY_BLOB_AND_PARAMS{
    ...
    LPVOID                          pVoidDecryptFunc;
} CRYPT_PRIVATE_KEY_BLOB_AND_PARAMS, *PCRYPT_PRIVATE_KEY_BLOB_AND_PARAMS;
...
//
//  Currently, pvEncryptionAuxInfo is only defined for RC2 encryption
...
WINAPI 
CryptImportPKCS8(
    CRYPT_PRIVATE_KEY_BLOB_AND_PARAMS   privateKeyAndParams,    // in
    DWORD                               dwFlags,                // in, optional
...
//
//  NOTE - THIS FLAG IS NOT CURRENTLY SUPPORTED!!!!
//  If the CRYPT_DELETE_KEYSET flag is set then the keyset will be deleted before
//  returning from this function.  Note that this flag is only valid if
//  the keyset is actually being exported, meaning that if pbPrivateKeyBlob
//  is NULL then this flag will be ignored.  Also, if the this flag is set
//  then the HCRYPTPROV passed in will be invalid upon return!
//
//  Currently, pvAuxInfo isn't used
//--------------------------------------------------------------------------
WINCRYPT32API
BOOL 
WINAPI 
...
#define CHANGES_TO_MERGE_WITH_WINBASE_H


#ifdef CHANGES_TO_MERGE_WITH_WINBASE_H

#endif // CHANGES_TO_MERGE_WITH_WINBASE_H
...
#define CRYPT_NEXT_0x800B_ERROR_CODE_AVAILABLE      0x010E
...
//      FALSE:                          Errors occured.  See GetLastError()
//
extern BOOL WINAPI CryptSIPRetrieveSubjectGuid(IN OPTIONAL LPCWSTR FileName,   // wide file name
...
#define     CRYPTCAT_OPEN_VERIFYSIGHASH         0x10000000  // verifies the signature (not the certs!)
        ...
        //
        //  fdwStoreFlags   (used internal -- do not fill)
        //
#define     CRYPTCAT_STORE_LOCALOPEN_HPROV      0x00000001  // mscat32 opened the hProv!
...
//  pvStructInfo points to SPC_PE_IMAGE_DATA
//
...
//
// see above for SPC_FLAGS...
//

//
// Oid's not used for encode/decode functions.
//
...
//
//  BUGBUG: Temporary hack until NT brings in changes to winbase.h
//
#if 0
#ifdef WIN_SPUB_ACTION_TRUSTED_PUBLISHER

extern LONG WINAPI WinVerifyTrust(IN OPTIONAL HWND hwnd,
                                  IN          GUID *pgActionID,
                                  IN          LPVOID pWintrustData);

#else

# pragma message("WINTRUST.H: BUGBUG!!!!!  Clean up time!")

extern HRESULT WINAPI WinVerifyTrust(IN OPTIONAL HWND hwnd,
                                     IN          GUID *pgActionID,
...
typedef struct _CRYPT_PROVIDER_DATA
{
    ...
    struct _CRYPT_PROVIDER_SGNR         *pasSigners;            // use Add2 function!
                                        
    DWORD                               csProvPrivData;         // use Add2 function!
    struct _CRYPT_PROVIDER_PRIVDATA     *pasProvPrivData;       // use Add2 function!
...
typedef struct _CRYPT_PROVIDER_SGNR
{
    ...
    struct _CRYPT_PROVIDER_CERT         *pasCertChain;      // use the Add2 allocator
