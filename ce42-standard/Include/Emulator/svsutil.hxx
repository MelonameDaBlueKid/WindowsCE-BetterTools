...
protected:
	...
	SVSCookie PutOnWorkerQueue (LPTHREAD_START_ROUTINE pfn, LPVOID lpParameter, __int64 ftExecuteTime) {
		...
		if (m_cThreadsRunning == 0 && m_cThreadsIdle == 0) {	// Need at least one thread...
			HANDLE hThread = CreateThread (NULL, 0, SVSThreadPoolWorkerThread, this, 0, NULL);
			if (hThread) {	// We can run out of threads.
				CloseHandle (hThread);
				m_cThreadsIdle++;	
			} else
				return 0;
	...
	void Worker(void) {
		...
		while (m_fRunning) {
			...
			if (m_cThreadsIdle)
				...
				HANDLE hThread = CreateThread (NULL, 0, SVSThreadPoolWorkerThread, this, 0, NULL);
				if (hThread) {	// It is feasible to run out of threads here
					CloseHandle(hThread);
					m_cThreadsIdle++;
				}
			}
...
public:
	...
	unsigned long Shutdown (void) {
		...
		while (m_cThreadsRunning || m_cThreadsIdle)
		{
			...
			for (int i = 0 ; i < (int)(m_cThreadsRunning + m_cThreadsIdle) ; ++i)
				SetEvent (m_hWorkerEvent);	
	...
	int UnScheduleEvent (SVSCookie ulCookie) {
		...
		Lock();
		hEnum = EnumStart();
		while (hEnum != SVSUTIL_HANDLE_INVALID) {
...
{
public:
	char              *pBuffer;                // pointer to allocated mem
	...
	BOOL AllocMem(unsigned int uiRequestedBytes)  {
		...
		if(!pBuffer) {
			pBuffer  = (char*) g_funcAlloc(uiAlloc,g_pvAllocData);
			uiBufSize = uiAlloc;
		}
		else if((uiBufSize-uiNextIn) <= uiRequestedBytes) {
			char *pTempBuffer = (char*) g_funcRealloc(pBuffer,uiAlloc+uiBufSize,g_pvAllocData);
			if (!pTempBuffer)
				return FALSE;
