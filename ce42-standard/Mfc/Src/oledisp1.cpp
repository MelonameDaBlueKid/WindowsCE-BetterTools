	...
	VARIANT* pvarResult, DISPPARAMS* pDispParams, UINT* puArgErr)
{
	...
	pStack += _STACK_OFFSET;
        ...
        // Compute an array of parameter types and sizes. We only need to
        // know whether a given parameter should go into an int or a
        // float register. That way, when we dispatch this call, the 
        ...
        // thunks on these processors cannot be larger than 32 bytes
		unsigned buffer[32];
