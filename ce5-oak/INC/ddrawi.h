...
DEFINE_GUID( GUID_D3DCommandBufferCallbacks,    0x82d4f372, 0xd097, 0x11d0, 0xa9, 0xb3, 0x00, 0xaa, 0x00, 0xc0, 0x99, 0x3e);
...
typedef struct IDirectDrawSurface5Vtbl DIRECTDRAWSURFACE5CALLBACKS;
...
#define DD_RUNTIME_VERSION      0x00000600l
...
//   This structure can be queried from the driver from DX6 onward
...
typedef struct _DDHAL_D3DBUFCALLBACKS
{
    DWORD dwSize;
    DWORD dwFlags;
...
} DDHAL_D3DBUFCALLBACKS, * LPDDHAL_D3DBUFCALLBACKS;

#ifndef DDHAL_D3DBUFCB32_CANCREATED3DBUF
#define DDHAL_D3DBUFCB32_CANCREATED3DBUF    0x00000001l
#define DDHAL_D3DBUFCB32_CREATED3DBUF       0x00000002l
#define DDHAL_D3DBUFCB32_DESTROYD3DBUF      0x00000004l
#define DDHAL_D3DBUFCB32_LOCKD3DBUF         0x00000008l
#define DDHAL_D3DBUFCB32_UNLOCKD3DBUF       0x00000010l
#endif
...
#ifndef UNDER_CE
typedef HRESULT (WINAPI *LPDDGAMMACALIBRATORPROC)( LPDDGAMMARAMP, char *);
#else
typedef HRESULT (WINAPI *LPDDGAMMACALIBRATORPROC)( LPDDGAMMARAMP, wchar_t *);
#endif
...
typedef struct _DDRAWI_DDRAWSURFACE_GBL_MORE
{
    ...
    // I believe that lpDDOptSurfaceDesc was changed to this for dx7/8, so I'm just adding it in here at the end
...
//    IUnknown              FAR *pUnkOuter; // outer IUnknown
    ...
    // --btaggart--
    HBITMAP hbmp; // for the bitmap created in CreateDCFromSurface
};
...
#else
        wchar_t                   cObsolete[12];   // Obsolete field, do not use
#endif
...
#else
/*618*/ wchar_t                 cDriverName[MAX_DRIVER_NAME]; // Display name
#endif
/*   */ //------- Fields added in Version 6.0 -------
/*638*/ ULONG_PTR                lpD3DHALCallbacks3;     // DX6 D3D callbacks
/*614*/ DWORD                   dwNumZPixelFormats;     // Number of z-buffer+stencil pixel formats
/*618*/ LPDDPIXELFORMAT         lpZPixelFormats;        // Pointer to array of z-buffer pixel formats
/*61c*/ ULONG_PTR                mcList;
/*620*/ DWORD                   hDDVxd;                 // handle to ddraw.vxd
/*624*/ DDSCAPSEX               ddsCapsMore;            // as queried via GUID_DDMoreSurfaceCaps
...
#define DDRAWI_USINGEMULATION               0x00000004l // Are we actually using emulation?
...
typedef struct _DDHALINFO
{
  ...
  union {
    ...
    LPDDHAL_D3DBUFCALLBACKS     lpD3DBufCallbacks;
  };
} DDHALINFO;
...
/****************************************************************************
 *
 * DDHAL structures for Gamma Control callbacks
 *
 ***************************************************************************/

/* Get/Set GammaRamp HAL call structs. 
typedef struct _DDHAL_GETGAMMARAMP
{
    HRESULT     ddRVal;     // OUT - Return value
    LPGAMMARAMP lpRampData; // OUT - Ramp to fill in
} DDHAL_GETGAMMARAMP;

typedef struct _DDHAL_SETGAMMARAMP
{
    HRESULT     ddRVal;     // OUT - Return value
    LPGAMMARAMP lpRampData; // IN - Ramp to use
    DWORD       dwFlags;    // IN - DDSGR_CALIBRATE/0 to use calibrator.
} DDHAL_SETGAMMARAMP;
*/
...
typedef struct _DDHAL_CREATESURFACEEXDATA
{
    ...
    LPDDRAWI_DDRAWSURFACE_LCL   lpDDSLcl;       // created surface
                                                // objects
    HRESULT                     ddRVal;         // return value
...
#endif
{
    // ~~ This method will need updating for Dx7 diff.
    return ((psurf_lcl->lpSurfMore->ddsCapsEx.dwCaps2 & DDSCAPS2_TEXTUREMANAGE) && !(psurf_lcl->lpSurfMore->lpDD_lcl->lpGbl->ddCaps.dwCaps2 & DDCAPS2_CANMANAGETEXTURE));
}
