...
BOOL CEPolyDraw(HDC  hdc, CONST LPPOINT lppt,CONST LPBYTE lpbTypes,int  cCount){
	...
	for (i=0; i<cCount; i++){
		...
		case PT_BEZIERTO :
			if (cCount < 3 || (cCount - i) < 3)break;
			m_ptBezier[1].x = lppt[i].x; // next go round start points
			m_ptBezier[1].y = lppt[i].y;
			m_ptBezier[2].x = lppt[i + 1].x; 
			m_ptBezier[2].y = lppt[i + 1].y;
			m_ptStart[0].x = m_ptBezier[3].x = lppt[i + 2].x; 
			m_ptStart[0].y = m_ptBezier[3].y = lppt[i + 2].y;
