...
{
	if(m_char == _T('\0'))
		str.Format(_T("{%d, , %ls}"), m_int, m_str);
	else
		str.Format(_T("{%d, %lc, %ls}"), m_int, m_char, m_str);
}

void AFXAPI SerializeElements(CArchive& ar, CMyStruct** ppElements, int nCount)
{
...
{
	if(m_char == _T('\0'))
		str.Format(_T("{%i, , %ls}"), m_int, m_str);
	else
		str.Format(_T("{%i, %lc, %ls}"), m_int, m_char, m_str);
}
...
BOOL CCollectDoc::OnNewDocument()
{
	...
	pMyStruct->m_char = pMyStruct->m_str[0];
	...
	pMyObject->m_char = pMyObject->m_str[0];
	...
	pMyObject2->m_char = pMyObject2->m_str[0];
	...
	pMyStruct2->m_char = pMyStruct2->m_str[0];
...
void AFXAPI DDX_Char(CDataExchange* pDX, int nIDC, TCHAR& ch)
{
	HWND hWndCtrl = pDX->PrepareEditCtrl(nIDC);
	if (pDX->m_bSaveAndValidate)
	{
		TCHAR szText[10];
		::GetWindowText(hWndCtrl, szText, 10);
		ch = szText[0];
	}
	else
	{
