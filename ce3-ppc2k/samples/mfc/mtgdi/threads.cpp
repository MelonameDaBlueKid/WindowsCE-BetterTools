...
CCriticalSection CGDIThread::m_oCriticalSection;
...
#else
	VERIFY(EventModify(m_hEventDead, EVENT_SET));
	VERIFY(EventModify(m_hAnotherDead, EVENT_SET));
#endif
}
...
#else
	VERIFY(EventModify(m_hEventKill, EVENT_SET));
#endif
...
void CBallThread::SingleStep()
{
	...
	// NOTE: We don't have to call csl.Unlock() since the csl lock is
	//       destroyed when leave this function.
	CSingleLock csl( &CGDIThread::m_oCriticalSection );
	if (csl.Lock())
	{
		CBrush* oldbrush;
