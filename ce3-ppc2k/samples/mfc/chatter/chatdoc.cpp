...
BOOL CChatDoc::OnNewDocument()
{
...
#endif
	{
		...
		while(TRUE)
		{
			...
			CString str;
			str.LoadString(IDS_CHANGEADDRESS);
			if (AfxMessageBox((LPCTSTR)str,MB_YESNO) == IDNO)
				return FALSE;
		}
	}
}
...
BOOL CChatDoc::ConnectSocket(LPCTSTR lpszHandle, LPCTSTR lpszAddress, UINT nPort)
{
	...
	if (!m_pSocket->Create())
	{
		...
		m_pSocket = NULL;
		CString str;
		str.LoadString(IDS_CREATEFAILED);
	...
	{
		CString str;
		str.LoadString(IDS_RETRYCONNECT);
		if (AfxMessageBox((LPCTSTR)str,MB_YESNO) == IDNO)
		{
			delete m_pSocket;
			m_pSocket = NULL;
