...
#ifdef _DEBUG
#define MFC42_DLL WCE_MFC_FILENAME(D.DLL)
#else
#define MFC42_DLL WCE_MFC_FILENAME(.DLL)
#endif
#else // _WIN32_WCE 
#ifdef _DEBUG
#ifndef _UNICODE
...
BOOL WINAPI RawDllMain(HINSTANCE, DWORD dwReason, LPVOID)
{
	if (dwReason == DLL_PROCESS_ATTACH)
	{
		...
		WCE_FCTN(LoadLibraryA)(MFC42_DLL);
	...
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		...
		FreeLibrary(WCE_FCTN(GetModuleHandleA)(MFC42_DLL));
	}
	return TRUE;    // ok
}
...
extern "C"
#if defined(_WIN32_WCE)
BOOL WINAPI DllMain(HANDLE wce_hInstance, DWORD dwReason, LPVOID /*lpReserved*/)
{
...
BOOL WINAPI DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID /*lpReserved*/)
{
	...
	{
#if defined(_WIN32_WCE)
		wce_UnregisterOleWindowClasses();
#endif // _WIN32_WCE
...
WCE_DEL new COleDropSource[2];
WCE_DEL new COleResizeBar[2];
	new COleStreamFile[2];
WCE_DEL new CMonikerFile[2];
WCE_DEL new COleTemplateServer[2];
WCE_DEL new COleDataObject[2];
WCE_DEL new COleDropTarget[2];
WCE_DEL new COleIPFrameWnd[2];

WCE_DEL new COleDocIPFrameWnd[2];
WCE_DEL new CAsyncMonikerFile[2];
WCE_DEL new CCachedDataPathProperty[2];
WCE_DEL new CDataPathProperty[2];

	new COleVariant[2];

WCE_DEL new CRichEditView[2];
WCE_DEL new CRichEditCntrItem[2];
}
