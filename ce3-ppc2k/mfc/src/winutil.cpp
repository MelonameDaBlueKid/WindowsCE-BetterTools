...
BOOL AFXAPI AfxCustomLogFont(UINT nIDS, LOGFONT* pLogFont)
{
	...
	if (lpszSize != NULL)
	{
		...
		pLogFont->lfHeight =
			WCE_FCTN(MulDiv)(pLogFont->lfHeight, afxData.cyPixelsPerInch, 72);
		*lpszSize = '\0';
	}
...
void AFXAPI AfxCancelModes(HWND hWndRcvr)
{
	...
	if (hWndRcvr != NULL &&
	  ...
	  ::GetParent(hWndRcvr) == ::WCE_FCTN(GetDesktopWindow)())
		return;
...
void AFXAPI AfxGlobalFree(HGLOBAL hGlobal)
{
	...
	ASSERT(WCE_FCTN(GlobalFlags)(hGlobal) != GMEM_INVALID_HANDLE);
	UINT nCount = WCE_FCTN(GlobalFlags)(hGlobal) & GMEM_LOCKCOUNT;
	while (nCount--)
	...
	// nSize is already rounded
{
	...
	if (pThreadState != NULL && pThreadState->m_pSafetyPoolBuffer != NULL)
	{
		...
		else
		{
			...
			WCE_FCTN(_expand)(pThreadState->m_pSafetyPoolBuffer,
