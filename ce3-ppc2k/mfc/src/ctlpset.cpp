...
LPSTREAM AFXAPI _AfxCreateMemoryStream()
{
	...
	HGLOBAL hGlobal = WCE_FCTN(GlobalAlloc)(GMEM_MOVEABLE|GMEM_SHARE, 0);
	if (hGlobal != NULL)
	{
		if (FAILED(WCE_FCTN(CreateStreamOnHGlobal)(hGlobal, TRUE, &lpStream)))
		{
			TRACE0("CreateStreamOnHGlobal failed.\n");
			WCE_FCTN(GlobalFree)(hGlobal);
			return NULL;
		}
		...
		LPSTGMEDIUM lpStgMedium, REFCLSID fmtid)
{
	...
	if (lpFormatEtc->tymed & TYMED_ISTORAGE)
	{
		...
		else
		{
			...
			// Used the same CreateILockBytesOnHGlobal workaround as the one in OCCSITE.CPP
			if (FAILED(StgCreateDocfile(NULL,
					STGM_SHARE_EXCLUSIVE|STGM_CREATE|STGM_READWRITE, 0,
					&lpStorage)))
			{
				TRACE0("StgCreateDocfile failed.\n");
				return FALSE;
			}
#else // _WIN32_WCE
	...
	pset.SetOSVersion(MAKELONG(LOWORD(WCE_FCTN(GetVersion)()), OSTYPE));
...
BOOL AFXAPI _AfxCoerceNumber(void* pvDst, VARTYPE vtDst, void* pvSrc, VARTYPE vtSrc)
{
	...
	if (vtSrc == vtDst)
	{
		WCE_FCTN(memcpy)(pvDst, pvSrc, cbSrc);
		return TRUE;
	}
	...
	VARIANTARG var;
	V_VT(&var) = vtSrc;
	WCE_FCTN(memcpy)((void*)&V_NONE(&var), pvSrc, cbSrc);
	...
	WCE_FCTN(memcpy)(pvDst, (void*)&V_NONE(&var), cbDst);
	return TRUE;
}
	...
	HGLOBAL* phBlob, HGLOBAL hBlobDefault)
{
	...
	else
	{
		...
		pvBlob = NULL;
		if (*phBlob != NULL)
			pvBlob = WCE_FCTN(GlobalLock)(*phBlob);
		...
		DWORD dwPropID, DWORD dwType)
{
			...
			NULL)))
	{
		...
		HGLOBAL hGlobal = WCE_FCTN(GlobalAlloc)(GMEM_MOVEABLE|GMEM_SHARE, 
			cb + (DWORD)sizeof(cb));

		if (hGlobal != NULL)
		{
			LPBYTE pbData = (LPBYTE)WCE_FCTN(GlobalLock)(hGlobal);
			if (pbData != NULL)
			{
				*(DWORD*)pbData = cb;
...
LPFONT AFXAPI _AfxCreateFontFromStream(LPSTREAM pstm)
{
	...
	if (SUCCEEDED(pstm->Read(&clsid, sizeof(CLSID), NULL)))
	{
			...
			IsEqualCLSID(clsid, _afx_CLSID_StdFont_V1))
		{
			...
			hr = ::WCE_FCTN(OleCreateFontIndirect)((LPFONTDESC)&_afxFontDescHelv, IID_IFont,
					(LPVOID*)&pFont);
		}
		else
		{
...
LPUNKNOWN AFXAPI _AfxCreateObjectFromStreamedPropset(LPSTREAM lpStream, REFGUID iid)
{
	...
	if (_AfxGetClassIDFromStreamedPropset(&clsid, lpStream))
	{
		...
		if (IsEqualCLSID(clsid, CLSID_StdFont) ||
			IsEqualCLSID(clsid, _afx_CLSID_StdFont_V1))
		{
			if (FAILED(::WCE_FCTN(OleCreateFontIndirect)((LPFONTDESC)&_afxFontDescHelv, iid,
					(LPVOID*)&pUnk)))
			{
				pUnk = NULL;
			}
	...
	DWORD dwPropID)
{
			...
			(LPVOID*)&pPersStm)))
	{
		...
		if (pstm != NULL)
		{
			if (SUCCEEDED(::WCE_FCTN(OleSaveToStream)(pPersStm, pstm)) &&
		...
		if (pstm != NULL)
		{
			...
			case VT_BLOB:
				if (_AfxPeekAtClassIDInStream(pstm, &clsid))
				{
					...
					{
						// Load the object.
						bSuccess = SUCCEEDED(::WCE_FCTN(OleLoadFromStream)(pstm, iid,
							(LPVOID*)ppUnk));
					}
				}
				break;
