...
void CUIntArray::SetSize(int nNewSize, int nGrowBy)
{
	...
	else
	{
		...
		WCE_FCTN(memcpy)(pNewData, m_pData, m_nSize * sizeof(UINT));
...
int CUIntArray::Append(const CUIntArray& src)
{
	...
	WCE_FCTN(memcpy)(m_pData + nOldSize, src.m_pData, src.m_nSize * sizeof(UINT));
...
void CUIntArray::Copy(const CUIntArray& src)
{
	...
	WCE_FCTN(memcpy)(m_pData, src.m_pData, src.m_nSize * sizeof(UINT));
