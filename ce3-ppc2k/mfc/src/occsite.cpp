...
#if defined(_WIN32_WCE) 
			//*****************************************************
			// The IQuickActivate interface is not initializing 
			// QACONTROL.dwMiscStatus, so we are initializing it ourselves.
			//*****************************************************
			if(!m_dwMiscStatus)
			{
				CLSID clsid;
				m_pObject->GetUserClassID(&clsid);
				wce_RegGetMiscStatus(clsid, &m_dwMiscStatus);
			}
#endif //_WIN32_WCE
...
#if !defined(_WIN32_WCE) 
// WinCE: not enough OS support to support IStorage persistance on the container side 
...
#if defined(_WIN32_WCE)
// WinCE: WinCE doesn't support CopyAcceleratorTable nor IsAcclerator, so we are going to 
// use TranslateAccelerator, which is basically an IsAccelerator followed by a possible
// SendMessage.  We want to eat any messages that might get sent by temporarily changing the
// window procedure.  This is not thread-safe, but chance of another thread getting the
// context switch AND sending a message to this particular control during that context
// switch is almost nil.
BOOL COleControlSite::IsMatchingMnemonic(LPMSG lpMsg)
{
	if ((m_ctlInfo.cAccel == 0) || (m_ctlInfo.hAccel == NULL))
		return FALSE;
	WNDPROC PrevWndProc = (WNDPROC)::SetWindowLong(m_hWnd,GWL_WNDPROC,(LONG)wce_IMMWndProc);
	BOOL bMatch = ::TranslateAccelerator(m_hWnd,m_ctlInfo.hAccel,lpMsg);
	::SetWindowLong(m_hWnd,GWL_WNDPROC,(LONG)PrevWndProc);
	return bMatch;
}
#else // _WIN32_WCE
BOOL COleControlSite::IsMatchingMnemonic(LPMSG lpMsg)
	...
	POINTL* pptHimetric, POINTF* pptContainer, DWORD dwFlags)
{
...
#if !defined(_WIN32_WCE) 
	// WinCE: We will use alternative coordinate transformation
	// functions (see below)
	...
	if (dwFlags & XFORMCOORDS_HIMETRICTOCONTAINER)
	{
		...
		CDC dc;
		dc.Attach(hDC);
		dc.HIMETRICtoDP((LPSIZE)(&rgptConvert[0]));
	 	dc.HIMETRICtoDP((LPSIZE)(&rgptConvert[1]));
#else // _WIN32_WCE
	...
	else if (dwFlags & XFORMCOORDS_CONTAINERTOHIMETRIC)
	{
		...
		CDC dc;
		dc.Attach(hDC);
		dc.DPtoHIMETRIC((LPSIZE)(&rgptConvert[0]));
	 	dc.DPtoHIMETRIC((LPSIZE)(&rgptConvert[1]));
#else // _WIN32_WCE
	...
	DISPID dispid)
{
	...
	if (WCE_IF(TRUE, pThis->m_defdispid == dispid))
	{
	...
	REFIID iid, LPVOID far * ppvObj)
{
	...
	if (IsEqualIID(iid, IID_IUnknown) ||
		WCE_IF(FALSE, IsEqualIID(iid, IID_INotifyDBEvents)))
	{
		*ppvObj = this;
		AddRef();
		return S_OK;
