...
// private definitions of IIDs to avoid linking with uuid.lib
static const IID IID_IClassFactory =
	{ 0x00000001, 0, 0, { 0xC0, 0, 0, 0, 0, 0, 0, 0x46 } };
#endif // _WIN32_WCE
...
#pragma warning(disable:4191)
	...
	pfn = (HRESULT (STDAPICALLTYPE*)(REFCLSID rclsid, REFIID riid, LPVOID* ppv))
		WCE_FCTN(GetProcAddress)(hInst, "DllGetClassObject");
...
BOOL AFXAPI AfxGetInProcServer(LPCTSTR lpszCLSID, CString& str)
{
	HKEY hKey = NULL;
	BOOL b = FALSE;
	if (WCE_FCTN(RegOpenKey)(HKEY_CLASSES_ROOT, _T("CLSID"), &hKey) == ERROR_SUCCESS)
	{
		HKEY hKeyCLSID = NULL;
		if (WCE_FCTN(RegOpenKey)(hKey, lpszCLSID, &hKeyCLSID) == ERROR_SUCCESS)
		{
			HKEY hKeyInProc = NULL;
			if (WCE_FCTN(RegOpenKey)(hKeyCLSID, _T("InProcServer32"), &hKeyInProc) ==
				ERROR_SUCCESS)
			{
	...
	// (both in ANSI character set)
{
	...
	LPTSTR lpszFilePart;
	if (!WCE_FCTN(GetFullPathName)(lpszFileIn, _MAX_PATH, lpszPathOut, &lpszFilePart))
	{
#ifdef _DEBUG
		if (lpszFileIn[0] != '\0')
...
BOOL AFXAPI AfxComparePath(LPCTSTR lpszPath1, LPCTSTR lpszPath2)
{
	...
	if (!WCE_FCTN(GetSystemMetrics)(SM_DBCSENABLED))
		return TRUE;
...
UINT AFXAPI AfxGetFileTitle(LPCTSTR lpszPathName, LPTSTR lpszTitle, UINT nMax)
{
	...
	if (::WCE_FCTN(GetFileTitle)(lpszPathName, lpszTemp, (WORD)nMax) != 0)
	{
