...
CThreadSlotData::~CThreadSlotData()
{
	...
	if (m_pSlotData != NULL)
	{
		HGLOBAL hSlotData = WCE_FCTN(GlobalHandle)(m_pSlotData);
		WCE_FCTN(GlobalUnlock)(hSlotData);
		WCE_FCTN(GlobalFree)(hSlotData);
...
int CThreadSlotData::AllocSlot()
{
	...
	if (nSlot >= nAlloc || (m_pSlotData[nSlot].dwFlags & SLOT_USED))
	{
		...
		if (nSlot >= nAlloc)
		{
			...
			if (m_pSlotData == NULL)
			{
				hSlotData = WCE_FCTN(GlobalAlloc)(GMEM_MOVEABLE|GMEM_SHARE,
					nNewAlloc*sizeof(CSlotData));
			}
			else
			{
				hSlotData = WCE_FCTN(GlobalHandle)(m_pSlotData);
				WCE_FCTN(GlobalUnlock)(hSlotData);
				hSlotData = WCE_FCTN(GlobalReAlloc)(hSlotData, nNewAlloc*sizeof(CSlotData),
					GMEM_MOVEABLE|GMEM_SHARE);
			}
			if (hSlotData == NULL)
			{
				WCE_FCTN(GlobalLock)(WCE_FCTN(GlobalHandle)(m_pSlotData));
				LeaveCriticalSection(&m_sect);
				AfxThrowMemoryException();
			}
			CSlotData* pSlotData = (CSlotData*)WCE_FCTN(GlobalLock)(hSlotData);
...
{
#if defined(_WIN32_WCE)
// WinCE: When pVB 2.0 apps are closed, we sometimes have a problem with MFC based ActiveX controls
	if((m_nSlot == 0) || ((_afxThreadData != NULL) && (m_nSlot >= _afxThreadData->m_nMax)))
#else // _WIN32_WCE
	if (m_nSlot == 0)
#endif // _WIN32_WCE
	{
