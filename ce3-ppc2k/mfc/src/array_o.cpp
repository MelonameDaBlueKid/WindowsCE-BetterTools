...
void CObArray::SetSize(int nNewSize, int nGrowBy)
{
	...
	else
	{
		...
		WCE_FCTN(memcpy)(pNewData, m_pData, m_nSize * sizeof(CObject*));
...
int CObArray::Append(const CObArray& src)
{
	...
	WCE_FCTN(memcpy)(m_pData + nOldSize, src.m_pData, src.m_nSize * sizeof(CObject*));
...
void CObArray::Copy(const CObArray& src)
{
	...
	WCE_FCTN(memcpy)(m_pData, src.m_pData, src.m_nSize * sizeof(CObject*));
