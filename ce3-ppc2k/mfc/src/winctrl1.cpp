...
// WinCE: Comboboxes created by CCeCommandBar::InsertComboBox don't handle LButtonDown correctly.
...
void CComboBox::OnLButtonDown(UINT nFlags, CPoint point)
{
	ASSERT(::IsWindow(m_hWnd));
	...
	if(pParent != NULL)
	{
		...
		if((pParent != NULL) && pParent->IsKindOf(RUNTIME_CLASS(CCeCommandBar)))
		{
			ShowDropDown(TRUE);
			return;
		}
