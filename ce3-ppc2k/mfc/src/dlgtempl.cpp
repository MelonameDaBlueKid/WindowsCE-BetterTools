	...
	int cxDlg, int cyDlg, SIZE* pSizePixel)
{
	...
	lf.lfHeight = -WCE_FCTN(MulDiv)(wFontSize, GetDeviceCaps(hDC, LOGPIXELSY), 72);
	...
	pSizePixel->cx = WCE_FCTN(MulDiv)(cxDlg, cxSysChar, 4);
	pSizePixel->cy = WCE_FCTN(MulDiv)(cyDlg, cySysChar, 8);
}
...
CDialogTemplate::CDialogTemplate(HGLOBAL hTemplate)
{
	...
	{
		DLGTEMPLATE* pTemplate = (DLGTEMPLATE*)WCE_FCTN(GlobalLock)(hTemplate);
		SetTemplate(pTemplate, GetTemplateSize(pTemplate));
		WCE_FCTN(GlobalUnlock)(hTemplate);
	}
}
...
BOOL CDialogTemplate::SetTemplate(const DLGTEMPLATE* pTemplate, UINT cb)
{
	...
	if ((m_hTemplate = WCE_FCTN(GlobalAlloc)(GPTR, m_dwTemplateSize + LF_FACESIZE * 2)) == NULL)
		return FALSE;
	DLGTEMPLATE* pNew = (DLGTEMPLATE*)WCE_FCTN(GlobalLock)(m_hTemplate);
	WCE_FCTN(memcpy)((BYTE*)pNew, pTemplate, (size_t)m_dwTemplateSize);

	m_bSystemFont = (::HasFont(pNew) == 0);

	WCE_FCTN(GlobalUnlock)(m_hTemplate);
	return TRUE;
}
...
{
	if (m_hTemplate != NULL)
		WCE_FCTN(GlobalFree)(m_hTemplate);
}
...
{
	DLGTEMPLATE* pTemplate = (DLGTEMPLATE*)WCE_FCTN(GlobalLock)(m_hTemplate);
...
BOOL CDialogTemplate::SetSystemFont(WORD wSize)
{
	...
	if (hFont != NULL)
	{
		...
		if (::GetObject(hFont, sizeof(LOGFONT), &lf) != 0)
		{
			...
			wDefSize = (WORD)WCE_FCTN(MulDiv)(lf.lfHeight, 72, GetDeviceCaps(hDC, LOGPIXELSY));
			::ReleaseDC(NULL, hDC);
		}
	}
