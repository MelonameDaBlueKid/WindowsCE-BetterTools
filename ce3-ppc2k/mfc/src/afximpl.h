...
public:
	...
	UINT nStartingPos; // starting position for wrap around document search
	BOOL bJustStarted; // first search for wrap around document search
...
#else
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 211) // @TODO: fix bug 2017 @BUGBUG: 211 only is wrong
