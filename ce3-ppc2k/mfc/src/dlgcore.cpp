	...
	CWnd* pParentWnd, HINSTANCE hInst)
{
	...
	TRY
	{
		...
		if (hTemplate != NULL)
			lpDialogTemplate = (DLGTEMPLATE*)WCE_FCTN(GlobalLock)(hTemplate);
...
LRESULT CDialog::HandleInitDialog(WPARAM wParam, LPARAM lParam)
{
	PreInitDialog();
...
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
// WinCE: modal dialog boxes initially want no menu, so we create an
//        empty menu bar.
	if(m_nFlags & WF_CONTINUEMODAL)
	{
