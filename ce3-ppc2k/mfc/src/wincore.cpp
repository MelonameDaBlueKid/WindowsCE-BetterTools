...
#if defined(_WIN32_WCE)
			// if the dlg was not displayed yet and the crt msg is a mouse msg for an disabled wnd
			// get rid of it in order to be able to display the dlg successfully
			BOOL bPeek = ::PeekMessage(pMsg, NULL, NULL, NULL, PM_NOREMOVE);
			if(bPeek && (pMsg->message >= WM_MOUSEFIRST && pMsg->message <= WM_MOUSELAST) && !(::IsWindowEnabled(pMsg->hwnd)) && bShowIdle)
