	...
	LPCRECT pRect, BOOL)
{
	...
	if (bSuccess)
	{
		...
		pThis->m_nControls = 0;
		::WCE_FCTN(EnumChildWindows)(pThis->GetSafeHwnd(), (WNDENUMPROC) COlePropertyPage::EnumChildProc, (LPARAM) pThis);

		if (pThis->m_nControls > 0)
			pThis->m_pStatus = new AFX_PPFIELDSTATUS [UINT(pThis->m_nControls)];

		pThis->m_nControls = 0;
		WCE_FCTN(EnumChildWindows)(pThis->GetSafeHwnd(), (WNDENUMPROC) COlePropertyPage::EnumControls, (LPARAM) pThis);
		...
		ULONG cObjects, LPUNKNOWN* ppUnk)
{
	...
	if (bLock)
		::WCE_FCTN(LockWindowUpdate(pThis->m_hWnd));
	...
	if (bLock)
		::WCE_FCTN(LockWindowUpdate(NULL));
