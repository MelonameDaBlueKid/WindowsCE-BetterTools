...
BOOL CInternetFile::SetWriteBufferSize(UINT nWriteSize)
{
	...
	if (nWriteSize != m_nWriteBufferSize)
	{
		...
		else
		{
			...
			WCE_FCTN(memcpy)(m_pbWriteBuffer, pbTemp, m_nWriteBufferPos);
			delete [] pbTemp;
		}
...
void CInternetFile::Write(const void* lpBuf, UINT nCount)
{
	...
	else
	{
		...
		{
			WCE_FCTN(memcpy)(m_nWriteBufferPos + m_pbWriteBuffer, lpBuf, nCount);
...
CHttpConnection::CHttpConnection(CInternetSession* pSession,
	LPCTSTR pstrServer, DWORD dwFlags, /* = 0 */
	INTERNET_PORT nPort /* = INTERNET_INVALID_PORT_NUMBER */,
	LPCTSTR pstrUserName /* = NULL */,
	...
	: CInternetConnection(pSession, pstrServer, nPort, dwContext)
{
	...
	ASSERT_KINDOF(CInternetSession, pSession);
#if defined(_WIN32_WCE)
	ASSERT( (dwFlags & ~INTERNET_FLAG_SECURE) == 0);
#else // _WIN32_WCE
	ASSERT((dwFlags & INTERNET_FLAG_ASYNC) == 0);
#endif // _WIN32_WCE
	ASSERT(AfxIsValidString(pstrServer));
	...
	LPCTSTR* ppstrAcceptTypes, LPCTSTR pstrVersion, DWORD dwFlags)
{
	...
	ASSERT(m_hConnection != NULL);
#if defined(_WIN32_WCE)
	ASSERT((dwFlags & ~(INTERNET_FLAG_NO_AUTO_REDIRECT | INTERNET_FLAG_SECURE)) == 0);
#else // _WIN32_WCE
	ASSERT((dwFlags & INTERNET_FLAG_ASYNC) == 0);
#endif // _WIN32_WCE
...
CGopherLocator::CGopherLocator(LPCTSTR pstrLocator, DWORD dwLocLen)
{
	...
	WCE_FCTN(memcpy)(pstr, pstrLocator, dwLocLen);
...
WCE_DEL IMPLEMENT_DYNAMIC(CGopherFile, CInternetFile)
...
WCE_DEL IMPLEMENT_DYNAMIC(CGopherConnection, CInternetConnection)
#if !defined(_WIN32_WCE_NO_FTP)
IMPLEMENT_DYNAMIC(CFtpFileFind, CFileFind)
#endif // _WIN32_WCE_NO_FTP
WCE_DEL IMPLEMENT_DYNAMIC(CGopherFileFind, CFileFind)
