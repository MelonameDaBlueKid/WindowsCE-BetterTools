...
CScrollView::CScrollView()
{
    ...
    m_nMapMode = WCE_IF(MM_TEXT,MM_NONE);
}
...
void CScrollView::OnPrepareDC(CDC* pDC, const void* pUnused)
...
#else // _WIN32_WCE_NO_GDITRANSFORM
        ASSERT(m_nMapMode > 0);
...
#if defined(_WIN32_WCE)   
        wce_ScrollChildren(m_hWnd, -(xOrig - ptDev.x), -(yOrig - ptDev.y));
#endif // _WIN32_WCE
...
void CScrollView::CalcWindowRect(LPRECT lpClientRect, UINT nAdjustType)
{
    ...
    else
    {
        ...
        ::AdjustWindowRectEx(lpClientRect, WCE_IF(GetStyle() & ~(WS_HSCROLL | WS_VSCROLL), GetStyle()), FALSE,
...
#if defined(_WIN32_WCE)  
// WinCE:  If GetScrollBarCtrl is NULL, don't ignore message
    CScrollBar* pTemp;
    if(pTemp = GetScrollBarCtrl(SB_VERT))
    {
        if(pScrollBar != pTemp) 
            return;
    }
...
#if defined(_WIN32_WCE)   
        wce_ScrollChildren(m_hWnd, (x-xOrig), (y-yOrig));
#endif // _WIN32_WCE
