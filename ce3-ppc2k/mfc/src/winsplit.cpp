...
WCE_DEL ON_WM_NCCREATE()
	ON_WM_SYSCOMMAND()
	ON_WM_CANCELMODE()
WCE_DEL	ON_MESSAGE_VOID(WM_DISPLAYCHANGE, OnDisplayChange)
	ON_MESSAGE_VOID(WM_WININICHANGE, OnDisplayChange)
WCE_DEL	ON_MESSAGE_VOID(WM_SETTINGCHANGE, OnDisplayChange)
WCE_DEL	ON_WM_MOUSEWHEEL()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
//	WinCE: 3-D controls go away in PSPC 2.0
	{		
		m_cxSplitter    = m_cySplitter    = 5 + 2;
		m_cxBorderShare = m_cyBorderShare = 1;
		m_cxSplitterGap = m_cySplitterGap = 5 + 2;
		m_cxBorder      = m_cyBorder      = 1;
	}
#elif defined(_WIN32_WCE)
	{
		m_cxSplitter    = m_cySplitter    = 7 + 2;
		m_cxBorderShare = m_cyBorderShare = 1;
		m_cxSplitterGap = m_cySplitterGap = 7 + 2 + 2;
...
#ifdef _DEBUG
	if (WCE_FCTN(GetSystemMetrics)(SM_CXBORDER) != 1 ||
		WCE_FCTN(GetSystemMetrics)(SM_CYBORDER) != 1)
	{
...
#if defined(_WIN32_WCE_PSPC) && (_WIN32_WCE >= 300)
//	WinCE: 3-D controls go away in PSPC 2.0
		pDC->FillSolidRect(rect,afxData.clrWindowFrame);
...
void CSplitterWnd::RecalcLayout()
{
	...
	layout.hDWP = ::WCE_FCTN(BeginDeferWindowPos)((m_nCols + 1) * (m_nRows + 1) + 1);
	...
	if (idcPrimary != 0)
	{
		...
		if (idcPrimary != _afx_idcPrimaryLast)
		{
			...
			if ((_afx_hcurDestroy = _afx_hcurLast =
			   ::WCE_FCTN(LoadCursor)(hInst, MAKEINTRESOURCE(idcPrimary))) == NULL)
			{
				...
				_afx_hcurLast = ::WCE_FCTN(LoadCursor)(NULL, idcSecondary);
		...
		if (hcurToDestroy != NULL)
			::WCE_IF(DestroyIcon,DestroyCursor)(hcurToDestroy); // destroy after being set
	}
