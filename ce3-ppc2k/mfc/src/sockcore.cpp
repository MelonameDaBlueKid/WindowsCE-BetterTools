...
// WinCE: CCeSocket creates worker threads, but they need to access
...
BOOL CAsyncSocket::Attach(SOCKET hSocket, long lEvent)
{
	...
	return WCE_IF(TRUE,AsyncSelect(lEvent));
}

SOCKET CAsyncSocket::Detach()
{
	SOCKET hSocket = m_hSocket;
	if (WCE_IF(TRUE,AsyncSelect(0)))
	{
...
BOOL CAsyncSocket::Bind(UINT nSocketPort, LPCTSTR lpszSocketAddress)
{
	...
	LPSTR lpszAscii = WCE_FCTN(T2CA)((LPTSTR)lpszSocketAddress);
...
BOOL CAsyncSocket::Connect(LPCTSTR lpszHostAddress, UINT nHostPort)
{
	...
	LPSTR lpszAscii = WCE_FCTN(T2CA)((LPTSTR)lpszHostAddress);
	sockAddr.sin_family = AF_INET;
	sockAddr.sin_addr.s_addr = inet_addr(WCE_IF(lpszAscii?lpszAscii:"",lpszAscii));
...
CAsyncSocket* WCE_IF(WINSOCKAPI,PASCAL) CAsyncSocket::LookupHandle(SOCKET hSocket, BOOL bDead)
{
...
void WCE_IF(WINSOCKAPI,PASCAL) CAsyncSocket::AttachHandle(
...
void WCE_IF(WINSOCKAPI,PASCAL) CAsyncSocket::DetachHandle(SOCKET hSocket, BOOL bDead)
{
	ASSERT(CAsyncSocket::LookupHandle(hSocket, bDead) != NULL);
...
void WCE_IF(WINSOCKAPI,PASCAL) CAsyncSocket::KillSocket(SOCKET hSocket, CAsyncSocket* pSocket)
{
...
void WCE_IF(WINSOCKAPI,PASCAL) CAsyncSocket::DoCallBack(WPARAM wParam, LPARAM lParam)
{
	...
	case FD_ACCEPT:
		...
		break;
	case FD_CONNECT:
		WCE_INS if(pSocket->m_hSocket == INVALID_SOCKET || pSocket->m_hSocket == 0) return;
...
int WCE_IF(WINSOCKAPI,PASCAL) CSocket::ProcessAuxQueue()
{
	_AFX_SOCK_THREAD_STATE* pState = _afxSockThreadState;
...
void WCE_IF(WINSOCKAPI,PASCAL) CSocket::AuxQueueAdd(UINT message, WPARAM wParam, LPARAM lParam)
{
	_AFX_SOCK_THREAD_STATE* pState = _afxSockThreadState;
...
#if defined(_WIN32_WCE)
// WinCE: under some circumstances (i.e. modal dialog box loops), 
// we don't get the timeout timer message, so use an alternative timeout scheme.
	...
	while (bBlocking)
	{
		TRY
		{
			MSG msg;
#if defined(_WIN32_WCE)
// WinCE: We need an alternate way of signaling the message pump that the socket
// is now connected.  This is where the m_bIsConnected flag is used.
			if(IsKindOf(RUNTIME_CLASS(CCeSocket)))
			{
				if((uStopFlag == FD_CONNECT) && (((CCeSocket*)this)->m_bIsConnected))
				{
					m_nConnectError = 0;
					bBlocking = FALSE;
					break;
...
#if defined(_WIN32_WCE)
			else if((GetTickCount() - dwStartTime) >= (DWORD)m_nTimeOut)
			{
				break;
			}
#endif // _WIN32_WCE
