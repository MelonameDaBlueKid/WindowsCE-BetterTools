...
void CCommandLineInfo::ParseParam(const TCHAR* pszParam,BOOL bFlag,BOOL bLast)
{
	...
	{
		USES_CONVERSION;
		ParseParamFlag(WCE_FCTN(T2CA)(pszParam));
	}
	else
...
void CCommandLineInfo::ParseLast(BOOL bLast)
{
	if (bLast)
	{
		...
		if (m_nShellCommand == FileNothing && !m_strFileName.IsEmpty())
			m_nShellCommand = FileOpen;
#endif // _WIN32_WCE_PSPC
		m_bShowSplash = WCE_IF(TRUE, !m_bRunEmbedded && !m_bRunAutomated);
	}
}
...
int CWinApp::Run()
{
	if (m_pMainWnd == NULL && WCE_IF(TRUE, AfxOleGetUserCtrl()))
	{
