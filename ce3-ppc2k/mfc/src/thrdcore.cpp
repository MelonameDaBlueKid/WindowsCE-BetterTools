...
void AFXAPI AfxEndThread(UINT nExitCode, BOOL bDelete)
{
	...
	WCE_FCTN(_endthreadex)(nExitCode);
#endif //!_MT
}
...
void CWinThread::CommonConstruct()
{
	...
	m_nMsgLast = WM_NULL;
	::WCE_FCTN(GetCursorPos)(&m_ptCursorLast);
	...
	LPSECURITY_ATTRIBUTES lpSecurityAttrs)
{
	...
	m_hThread = (HANDLE)WCE_FCTN(_beginthreadex)(lpSecurityAttrs, nStackSize,
...
#if defined(_WIN32_WCE)
// WinCE: there's a timing issue in Windows CE 2.1 that requires a sleep
	if (dwCreateFlags & CREATE_SUSPENDED)
	{
		Sleep(50);
...
BOOL CWinThread::IsIdleMessage(MSG* pMsg)
{
	...
	if (pMsg->message == WM_MOUSEMOVE || WCE_IF(FALSE, pMsg->message == WM_NCMOUSEMOVE))
	{
