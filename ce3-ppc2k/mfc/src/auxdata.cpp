...
void AFXAPI AfxEnableWin40Compatibility()
{
	if (afxData.bWin4)
	{
		...
		afxData.cxVScroll = WCE_FCTN(GetSystemMetrics)(SM_CXVSCROLL) + CX_BORDER;
		afxData.cyHScroll = WCE_FCTN(GetSystemMetrics)(SM_CYHSCROLL) + CY_BORDER;
...
{
	afxData.cxVScroll = WCE_FCTN(GetSystemMetrics)(SM_CXVSCROLL);
	afxData.cyHScroll = WCE_FCTN(GetSystemMetrics)(SM_CYHSCROLL);
...
AUX_DATA::AUX_DATA()
{
	...
	// Standard cursors
	hcurWait = ::WCE_FCTN(LoadCursor)(NULL, IDC_WAIT);
	hcurArrow = ::WCE_FCTN(LoadCursor)(NULL, IDC_ARROW);
...
{
	// System metrics
	cxIcon = WCE_FCTN(GetSystemMetrics)(SM_CXICON);
	cyIcon = WCE_FCTN(GetSystemMetrics)(SM_CYICON);
