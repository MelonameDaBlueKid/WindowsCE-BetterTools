...
BOOL WINAPI GetStoreInformation (LPSTORE_INFORMATION lpsi);
...
HANDLE WINAPI CeFindFirstDatabase (DWORD dwClassID);
CEOID  WINAPI CeFindNextDatabase (HANDLE hEnum);
CEOID  WINAPI CeCreateDatabase (LPWSTR lpszname, DWORD dwClassID, WORD wNumSortOrder, SORTORDERSPEC *rgSortSpecs);
BOOL   WINAPI CeSetDatabaseInfo (CEOID oidDbase, CEDBASEINFO *pNewInfo);
HANDLE WINAPI CeOpenDatabase (PCEOID poid, LPWSTR lpszName, CEPROPID propid, DWORD dwFlags, HWND hwndNotify);
BOOL   WINAPI CeDeleteDatabase (CEOID oid);
CEOID  WINAPI CeReadRecordProps (HANDLE hDbase, DWORD dwFlags, LPWORD lpcPropID, CEPROPID *rgPropID, LPBYTE *lplpBuffer, LPDWORD lpcbBuffer);

HANDLE WINAPI CeFindFirstDatabaseEx (PCEGUID pguid, DWORD dwClassID);
CEOID  WINAPI CeFindNextDatabaseEx (HANDLE hEnum, PCEGUID pguid);
CEOID  WINAPI CeCreateDatabaseEx (PCEGUID pguid, CEDBASEINFO *pInfo);
BOOL   WINAPI CeSetDatabaseInfoEx (PCEGUID pguid, CEOID oidDbase, CEDBASEINFO *pNewInfo);
HANDLE WINAPI CeOpenDatabaseEx (PCEGUID pguid, PCEOID poid, LPWSTR lpszName, CEPROPID propid, DWORD dwFlags, CENOTIFYREQUEST *pReq);
BOOL   WINAPI CeDeleteDatabaseEx (PCEGUID pguid, CEOID oid);
CEOID  WINAPI CeSeekDatabase (HANDLE hDatabase, DWORD dwSeekType, DWORD dwValue, LPDWORD lpdwIndex);
BOOL   WINAPI CeDeleteRecord (HANDLE hDatabase, CEOID oidRecord);
CEOID  WINAPI CeReadRecordPropsEx (HANDLE hDbase, DWORD dwFlags, LPWORD lpcPropID, CEPROPID *rgPropID, LPBYTE *lplpBuffer,
	LPDWORD lpcbBuffer, HANDLE hHeap);
CEOID  WINAPI CeWriteRecordProps (HANDLE hDbase, CEOID oidRecord, WORD cPropID, CEPROPVAL *rgPropVal);

BOOL   WINAPI CeMountDBVol(PCEGUID pguid, LPWSTR lpszVol, DWORD dwFlags);
BOOL   WINAPI CeUnmountDBVol(PCEGUID pguid);
BOOL   WINAPI CeFlushDBVol(PCEGUID pguid);
BOOL   WINAPI CeEnumDBVolumes(PCEGUID pguid, LPWSTR lpBuf, DWORD dwSize);
BOOL   WINAPI CeFreeNotification(PCENOTIFYREQUEST pRequest, PCENOTIFICATION pNotify);
...
// Functions
BOOL WINAPI CeOidGetInfoEx (PCEGUID pguid, CEOID oid, CEOIDINFO *oidInfo);
BOOL WINAPI CeOidGetInfo (CEOID oid, CEOIDINFO *oidInfo);
