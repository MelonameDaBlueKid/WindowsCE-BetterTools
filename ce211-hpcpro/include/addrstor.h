...
EXTERN_C int WINAPI GetPropertyDataStruct(int fFlag, ULONG uValue, PropertyDataStruct *ppds);

// Opening and closing the address book
EXTERN_C void WINAPI CloseAddressBook();
EXTERN_C BOOL WINAPI CreateAddressBook(HHPRTAG *rghhProps, int numProps);
EXTERN_C BOOL WINAPI OpenAddressBook(HWND hwnd, HHPRTAG hhProp);

// Getting oids and indexes of cards
EXTERN_C CEOID WINAPI GetAddressCardOid(int nCard);
EXTERN_C int WINAPI GetAddressCardIndex(CEOID oidCard);

// Retrieving cards
// Flag for OpenAddressCard
#define OAC_ALLOCATE        0x01

EXTERN_C BOOL WINAPI OpenAddressCard(CEOID oidCard, AddressCard *pac, ULONG uFlags);
EXTERN_C BOOL WINAPI GetAddressCardProperties(CEOID oidCard, AddressCard *pac, int numRequestedProps, HHPRTAG *rghhProp);
EXTERN_C BOOL WINAPI GetMatchingEntry(TCHAR *szMatch, int *pBufSize, TCHAR *szField, CEOID *poidMatch, HHPRTAG *phhMatch);

// Changing cards in the store
EXTERN_C BOOL WINAPI AddAddressCard(AddressCard *pac, CEOID *poidCard, int *pindex);
EXTERN_C BOOL WINAPI DeleteAddressCard(CEOID oidCard);
EXTERN_C BOOL WINAPI ModifyAddressCard(AddressCard *pac, CEOID oidCard, int *pindex);

// Getting a clean address card and freeing an address card
EXTERN_C void WINAPI FreeAddressCard(AddressCard *pac);

// Replication
EXTERN_C void WINAPI RecountCards();

// Getting and setting properties from the address book registry settings
EXTERN_C BOOL WINAPI GetColumnProperties(HHPRTAG *propList, int *pColumns);
EXTERN_C BOOL WINAPI SetColumnProperties(HHPRTAG *propList, int nColumns);
EXTERN_C HHPRTAG WINAPI GetSortOrder(BOOL fDatabaseSort);
EXTERN_C BOOL WINAPI SetSortOrder(HHPRTAG hhProp);
EXTERN_C void WINAPI SetMask(AddressCard *pac, HHPRTAG hhProp);
EXTERN_C int WINAPI GetNumberOfAddressCards();
#ifdef JAPAN
...
EXTERN_C CEOID WINAPI FindFirstEntry(TCHAR *szFind, HHPRTAG hhProp, int *pindex, DWORD dwFlags);
