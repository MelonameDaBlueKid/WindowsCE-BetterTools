...
#ifdef _WIN32_WCE_EMULATION
LPVOID WINAPI MapUncompressedFileW(LPCWSTR pFileName, LPDWORD pLen);
#else
...
#if !defined(UNDER_CE)   // The WinCE case is in kfuncs.h
WINBASEAPI
LPVOID
WINAPI
LockResource(
...
#if !defined(UNDER_CE) && !defined(_WIN32_WCE_EMULATION)    // The WinCE case is in kfuncs.h
WINBASEAPI
HANDLE
WINAPI
GetCurrentThread(
...
#ifdef _WIN32_WCE_EMULATION
HANDLE WINAPI RegisterDevice (LPCWSTR lpszName, DWORD index, LPCWSTR lpszLib, DWORD dwInfo);
BOOL   WINAPI DeregisterDevice (HANDLE hDevice);
