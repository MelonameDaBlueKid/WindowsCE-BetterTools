...
typedef struct   _HSE_VERSION_INFO {
    ...
    TCHAR   lpszExtensionDesc[HSE_MAX_EXT_DLL_NAME_LEN];
...
typedef struct _EXTENSION_CONTROL_BLOCK {
	...
	WCHAR      lpszLogData[HSE_LOG_BUFFER_LEN];// null terminated log info specific to this Extension DLL
	
    LPWSTR     lpszMethod;             // REQUEST_METHOD
    LPWSTR     lpszQueryString;        // QUERY_STRING
    LPWSTR     lpszPathInfo;           // PATH_INFO
    LPWSTR     lpszPathTranslated;     // PATH_TRANSLATED

    DWORD     cbTotalBytes;           // Total bytes indicated from client
    DWORD     cbAvailable;            // Available number of bytes
    LPBYTE    lpbData;                // pointer to cbAvailable bytes

    LPWSTR     lpszContentType;        // Content type of client data

    BOOL (WINAPI * GetServerVariable) ( HCONN       hConn,
                                        LPWSTR       lpszVariableName,
                                        LPVOID      lpvBuffer,
                                        LPDWORD     lpdwSize );
