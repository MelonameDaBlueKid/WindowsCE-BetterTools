...
const TCHAR kszCapCurrentService[]              = L"CurrentService";
...
// Determines if all outgoing messages should be signed and/or encrypted
#define PR_CE_SMIME_SEND_FLAGS              PROP_TAG (PT_LONG, 0x811C)

// Stores the serial number blob of the selected signing certificate
#define PR_CE_SMIME_SIGNING_CERTIFICATE     PROP_TAG (PT_BINARY, 0x811D)

// If set on a store that supports SMIME, will request full MIME from the server
#define PR_CE_SMIME_FROM_SERVER             PROP_TAG(PT_BOOLEAN, 0x811E)
...
HRESULT MailComposeMessage(MAILCOMPOSEFIELDS *pmcf);
...
HRESULT MailSwitchToFolder(LPENTRYID lpEntryID, ULONG cbEntryID);
...
HRESULT MailSyncMessages(LPCTSTR pszAccount, DWORD dwFlags);
