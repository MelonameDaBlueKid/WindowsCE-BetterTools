...
Description:
    ...
    IMsgStore::SetProps how to retrieve and set properties on message stores. 
    It will read in an XML file (mailset.xml) which contains a list of CEMAPI
    store properties that can be change. The XML document has the following 
    format:
...
Usage:
    Put mailset.xml and executable in the same directoy and run the program.
