...
static const DWORD g_rgTimeout[] = { 0, 1, 5, 15, 30, 60, 90, 120, 12 * 60, 24 * 60 };

BOOL    g_fEnrolled;   // Flag if we completed enrollment.


//  DefaultPasswordSaver provides a callback that performs the default password saving action.
//
class DefaultPasswordSaver : public PasswordSaveCallback
{
public:
    virtual void operator()(HWND wnd, PASSWDINFO const & oldPasswd, PASSWDINFO const & newPasswd)
    {
        // Save settings to registry.
        if(SUCCEEDED(SavePasswordInfo(&newPasswd)))
        {
            ...
            SaveLKGPolicy(&oldPasswd.Policy);
            ...
            // The error code is deliberately ignored.
            AddPasswordToHistory(&newPasswd);

            // Setup authentication reset
            arc::SetupAR(wnd, newPasswd.szPassword);
...
//  Do the enrollment parameterized by the PasswordSaver callback
//
void DoEnrollment(HWND parent, BOOL fSecure, PasswordSaveCallback & passwdSaver)
{
    ...
    EnrollmentParams params(fSecure, passwdSaver);

    DialogBoxParam(g_hInst
                 , MAKEINTRESOURCE(IDD_ENROLL)
                 , parent
                 , EnrollDlgProc
...
BOOL DoInitializeDlg(HWND hwndDlg, LONG lParam, PASSWDINFO * ppwi, EnrollmentParams * params)
{
    // Store the window handle.
    WndManager()[LAP_WINDOW_ENROLL] = hwndDlg;

    // Load password info from registry.
    PASSWDINFO empty = {0};
    ppwi = new (localalloc) PASSWDINFO(empty);

    if(NULL == ppwi || FAILED(GetPasswordInfo(ppwi)))
    {
        // We can't access the password information so just end now.
        localalloc_delete(ppwi);
    ...
    ppwi->bSecure = ppwi->bSecure || params->m_secure;
...
BOOL DoActivateDlg(HWND hwndDlg, UINT wParam, LONG lParam)
{
...
void DoUserOnOK(HWND hwndDlg, PASSWDINFO * ppwi, EnrollmentParams * params)
{
    ...
    if(IsPasswordInfoChanged(ppwi, &pwiNew))
    {
        ...
        if(fSave)
        {
            ...
            {
                params->m_passwdSaver(hwndDlg, *ppwi, pwiNew);
            }
        }
...
BOOL DoTimer(HWND hwndDlg, UINT wParam)
{
...
void DoDestroy(HWND hwndDlg, PASSWDINFO * ppwi)
{
...
BOOL DoCommand(HWND hwndDlg, UINT wParam, PASSWDINFO * ppwi)
{
    BOOL fHandled = FALSE;
    ...
    case IDOK:
        {
            ...
            fHandled = BOOLIFY(SendMessage(hwndDlg, WM_APP_ONOK, 0, 0));
            if(fHandled)
            {
                // Exit the dialog.
    ...
    case IDCANCEL:
        {
            ...
            fHandled = BOOLIFY(SendMessage(hwndDlg, WM_APP_ONCANCEL, 0, 0));
            if(fHandled)
            {
                // Exit the dialog.
                (void)EndDialog(hwndDlg, IDCANCEL);
            ...
            if(EN_UPDATE == HIWORD(wParam))
            {
                ...
                if(!ppwi->bPasswordChanged)
                {
                    ...
                    (void)SetDlgItemText(hwndDlg, 
                        (IDC_PASSWORD == LOWORD(wParam) ? IDC_CONFIRM : IDC_PASSWORD), K_SZRESETTEXT);
                }
...
{
    // staticly stored parameters are needed to pass the data between the EnrollDlgProc invokations
    static EnrollmentParams * params = NULL;
    ...
    {
        case WM_INITDIALOG:
            // paramenters are passed on initialization
            params = reinterpret_cast<EnrollmentParams *>(lParam);

            fHandled = DoInitializeDlg(hwndDlg, lParam, ppwi, params);
            break;
        ...
        case WM_APP_ENABLE:
            ...
            if(ppwi->bSecure)
            {
                // Only display this if we're in secure mode. There are cases
                // (like AuthReset) where we don't meet policy, but we're not
                // in forced enrollment so we don't want to notify the user.
        ...
        case WM_ACTIVATE:
            fHandled = DoActivateDlg(hwndDlg, wParam, lParam);
            break;
        ...
        case WM_APP_ONOK:
            DoUserOnOK(hwndDlg, ppwi, params);
            fHandled = TRUE;
            break;

        case WM_TIMER:
            fHandled = DoTimer(hwndDlg, wParam);
            break;
        ...
        case WM_DESTROY:
            DoDestroy(hwndDlg, ppwi);
            fHandled = TRUE;
            break;

        case WM_COMMAND:
            fHandled = DoCommand(hwndDlg, wParam, ppwi);
            break;
    }
...
void InitControls(HWND hwndDlg, PASSWDINFO* ppwi)
{
    ...
    ASSERT(NULL != hwndCtl);
    // Reset all content.
    (void)SendMessage(hwndCtl, CB_RESETCONTENT, 0, 0);
    ...
    for(int i=0; i < ARRAYSIZE(g_rgTimeout) + 1; i++)
    {
            ...
            pszFormat, fTimeValue, pszResource)))
        {
            ...
            nIndex = SendMessage(hwndCtl, CB_ADDSTRING, 0, 
                (LPARAM)szTimeout);
            if(CB_ERR != nIndex)
            {
                // Add the timeout in muntes as the item data.
                (void)SendMessage(hwndCtl, CB_SETITEMDATA, 
    ...
    (void)SendMessage(hwndCtl, CB_SETCURSEL, (WPARAM)uSel, 0);
...
BOOL VerifyPasswordInfo(HWND hwndDlg, PASSWDINFO* ppwi)
{
    ...
    if(nErrorID > 0)
    {
        ...
        size_t cchText = STRSAFE_MAX_CCH;
        size_t cchResource = 0;
        size_t cchInsert = 0;
        ...
        if(NULL != pszResource)
        {
            if(IDS_ERROR_ENUMBER == nErrorID)
            {
                ...
                if(SUCCEEDED(StringCchLength(pszEmergencyNumber, STRSAFE_MAX_CCH, &cchInsert))
                   && SUCCEEDED(StringCchLength(pszResource, STRSAFE_MAX_CCH, &cchResource)))
                {
                    // Add length of emergency number string
                    if (!safeIntUAdd(cchResource, cchInsert, &cchText)
                        || cchText > STRSAFE_MAX_CCH)
                    {
                        cchText = STRSAFE_MAX_CCH;
                    }
                }
                ...
                pszText = new (localalloc) TCHAR[cchText];
                if(NULL != pszText)
                {
                    ...
                    StringCchPrintfEx(pszText, cchText, NULL,
            ...
            else if(IDS_ERROR_STRENGTH == nErrorID)
            {
                ...
                // Calculate length of this complete message in a safe way.
                if(SUCCEEDED(StringCchLength(pszComplexity, STRSAFE_MAX_CCH, &cchInsert))
                   && SUCCEEDED(StringCchLength(pszResource, STRSAFE_MAX_CCH, &cchResource)))
                {
                    // Add length of emergency number string
                    if (!safeIntUAdd(cchResource, cchInsert, &cchText)
                    ...
                    {
                        if (!safeIntUAdd(cchText, 2 * MAX_INT_LENGTH, &cchText)
                            || cchText > STRSAFE_MAX_CCH)
                        {
                            cchText = STRSAFE_MAX_CCH;
                        }
                ...
                if(NULL != pszText)
                {
                    ...
                    if(MAX_COMPLEXITY != ppwi->Policy.uComplexity)
                    {
                        ...
                        LPTSTR pszFormat = new (localalloc) TCHAR[cchText];
                        if(NULL != pszFormat)
                        {
                            ...
                            // format buffer.
                            (void)StringCchCopy(pszFormat, cchText, pszText);
                            
                            // Format a second time to insert the 
                            // complexity number.
                            (void)StringCchPrintf(pszText, cchText, 
...
BOOL ValidatePassword(PASSWDINFO* ppwi)
{
    ...
    {   
        ...
        pwCharType = new (localalloc) WORD[cchPassword];
        ASSERT(pwCharType);
