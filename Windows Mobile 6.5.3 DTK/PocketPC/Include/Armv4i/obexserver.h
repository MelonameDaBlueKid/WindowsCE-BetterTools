...
typedef HRESULT  (*GetRemoteInfo) (unsigned int uiTransactionId, struct _obex_remote_info& remoteInfo);
...
#define BD_MAXNAME     248      // same as BTH_REMOTE_NAME.szNameBuffer

typedef struct _obex_remote_info {
    // Note: this structure is currently specific to Bluetooth. The address
    //       member is mapped to BT_ADDR type as defined in ws2bth.h.
    //       Other transport can use this structure in the future, but must 
    //       convert its remote address representation to ULONGLONG.
    //       The pwszName member is the remote device's friendly name.
    ULONGLONG       address;
    WCHAR           pwszName[BD_MAXNAME];
} ObexRemoteInfo;
