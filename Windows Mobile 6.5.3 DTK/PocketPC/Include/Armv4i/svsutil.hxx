...
#define PREFAST_SUPPRESS(code, msg)
#endif
...
// Allow inclusion in /W4 clean environments.
#pragma warning(push)
...
protected:
	...
	void EndWorkerThread(HANDLE *phThread, HANDLE hLocalEvent, HANDLE *phLocalInList) {
		...
		if (phLocalInList) {
			SVSUTIL_ASSERT(*phLocalInList == hLocalEvent);
#if defined (SVSUTIL_DEBUG_ANY)
			BOOL f = 
#endif
...
#define MAX_FAST_SHUTDOWN_ATTEMPTS    25
	...
	static BOOL IsThreadRunning(void *pvContext, void *) {
...
private:
	WCHAR                   *szVersion;
	SVSUTIL_XMLWRITER_STATE fState;
...
inline void svsutil_EatChar(CHAR* szInput, CHAR ch)
{
    LPSTR szRead = szInput;
...
public:
	...
	BOOL LogConfigure(const WCHAR *szLogFile, const WCHAR *szLogFileOld, DWORD dwMaxLogSize=SVS_LOGFILE_DEFAULT_SIZE) {
		...
		if (((wcslen(szLogFile)+1)    > SVSUTIL_ARRLEN(m_szLogFile)) ||
		    ((wcslen(szLogFileOld)+1) > SVSUTIL_ARRLEN(m_szLogFile))) 
		{
			return FALSE;
		}
		
		wcscpy(m_szLogFile,szLogFile);
		wcscpy(m_szOldLogFile,szLogFileOld);
