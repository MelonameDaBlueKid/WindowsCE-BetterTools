#ifndef __CECONFIG_H__
#define __CECONFIG_H__
// CEFilter Component List:
#define CE_MODULES_COREDLL 1
#define CE_MODULES_NK 1
#define CE_MODULES_IMJP31 1
#define CE_MODULES_IMJP31K 1
#define CE_MODULES_IMJP31_DICS_COMPACT 1
#define CE_MODULES_IMJP31DX_Q 1
#define CE_MODULES_COMMCTRL 1
#define CE_MODULES_COMMDLG 1
#define CE_MODULES_SHELL 1
#define CE_MODULES_LOADDBG 1
#define CE_MODULES_SHELLCELOG 1
#define CE_MODULES_OSAXST0 1
#define CE_MODULES_RELFSD 1
#define CE_MODULES_SCARD 1
#define CE_MODULES_WINSCARD 1
#define CE_MODULES_SECUR32 1
#define CE_MODULES_NTLMSSP 1
#define CE_MODULES_NTLMSSP_SVC 1
#define CE_MODULES_SPNEGO 1
#define CE_MODULES_SCHANNEL 1
#define CE_MODULES_CREDMAN 1
#define CE_MODULES_PCL 1
#define CE_MODULES_PRNPORT 1
#define CE_MODULES_PRNERR 1
#define CE_MODULES_CUSTSAT 1
#define CE_MODULES_SQMCE 1
#define CE_MODULES_USPCE 1
#define CE_MODULES_WLDAP32 1
#define CE_MODULES_ASYNCMAC 1
#define CE_MODULES_UNIMODEM 1
#define CE_MODULES_TCPSTK 1
#define CE_MODULES_TCPIP6 1
#define CE_MODULES_IPV6HLP 1
#define CE_MODULES_DHCPV6L 1
#define CE_MODULES_IRDASTK 1
#define CE_MODULES_DNSAPI 1
#define CE_MODULES_AFD 1
#define CE_MODULES_WINSOCK 1
#define CE_MODULES_AFD 1
#define CE_MODULES_WS2 1
#define CE_MODULES_WS2INSTL 1
#define CE_MODULES_WSPM 1
#define CE_MODULES_NSPM 1
#define CE_MODULES_SSLLSP 1
#define CE_MODULES_RTL8139 1
#define CE_MODULES_E100BEX 1
#define CE_MODULES_SMSC100FD 1
#define CE_MODULES_DP83815 1
#define CE_MODULES_NE2000 1
#define CE_MODULES_DHCP 1
#define CE_MODULES_ETHMAN 1
#define CE_MODULES_WZCSVC 1
#define CE_MODULES_WZCSAPI 1
#define CE_MODULES_PCX500 1
#define CE_MODULES_ISLP2NDS 1
#define CE_MODULES_WLCLIENT 1
#define CE_MODULES_WZCTOOL 1
#define CE_MODULES_RMGR 1
#define CE_MODULES_EAPOL 1
#define CE_MODULES_EAP 1
#define CE_MODULES_EAPCHAP 1
#define CE_MODULES_EAPTLS 1
#define CE_MODULES_NETMUI 1
#define CE_MODULES_NDISUIO 1
#define CE_MODULES_VEIM 1
#define CE_MODULES_DHCPSRV 1
#define CE_MODULES_NDIS 1
#define CE_MODULES_IPNAT 1
#define CE_MODULES_PPP 1
#define CE_MODULES_IRCOMM 1
#define CE_MODULES_TAPI 1
#define CE_MODULES_PPTP 1
#define CE_MODULES_L2TP 1
#define CE_MODULES_IPSEC 1
#define CE_MODULES_IPSECSVC 1
#define CE_MODULES_RSAENH 1
#define CE_MODULES_DSSDH 1
#define CE_MODULES_CRYPT32 1
#define CE_MODULES_MSASN1 1
#define CE_MODULES_REDIR 1
#define CE_MODULES_NETBIOS 1
#define CE_MODULES_BTD 1
#define CE_MODULES_BTDRT 1
#define CE_MODULES_AVCTRL 1
#define CE_MODULES_BTHHID 1
#define CE_MODULES_BTA2DP 1
#define CE_MODULES_SBC 1
#define CE_MODULES_BTHUART 1
#define CE_MODULES_BTHAMB 1
#define CE_MODULES_BTHSC 1
#define CE_MODULES_BTHCSR 1
#define CE_MODULES_BTHUNIV 1
#define CE_MODULES_WENDYSER 1
#define CE_MODULES_WCESTREAMBT 1
#define CE_MODULES_SIO950 1
#define CE_MODULES_IPHLPAPI 1
#define CE_MODULES_SOFTKB 1
#define CE_MODULES_KBDUS 1
#define CE_MODULES_KEYBD 1
#define CE_MODULES_POINTER 1
#define CE_MODULES_DISPLAY 1
#define CE_MODULES_FONTS 1
#define CE_MODULES_TOUCHRECOGNIZER 1
#define CE_MODULES_D3DMDRIVERS 1
#define CE_MODULES_PHYSICSENGINE 1
#define CE_MODULES_DDGUID 1
#define CE_MODULES_GSM610 1
#define CE_MODULES_WAVEAPI 1
#define CE_MODULES_AUDIODRV 1
#define CE_MODULES_WAVESAMPLES 1
#define CE_MODULES_GPSID 1
#define CE_MODULES_GPSAPI 1
#define CE_MODULES_CXPORT 1
#define CE_MODULES_BATTDRVR 1
#define CE_MODULES_NLEDDRVR 1
#define CE_MODULES_NULLCAM 1
#define CE_MODULES_DEVICE 1
#define CE_MODULES_REGENUM 1
#define CE_MODULES_BUSENUM 1
#define CE_MODULES_PM 1
#define CE_MODULES_GIISR 1
#define CE_MODULES_MMTIMER 1
#define CE_MODULES_PCI 1
#define CE_MODULES_PCMCIA 1
#define CE_MODULES_PCMCONV 1
#define CE_MODULES_SERIAL 1
#define CE_MODULES_PCCARD 1
#define CE_MODULES_8042KEYBOARD 1
#define CE_MODULES_NOPKEYBOARD 1
#define CE_MODULES_CSP 1
#define CE_MODULES_CSP_MININPUT 1
#define CE_MODULES_ATADISK 1
#define CE_MODULES_HIDPARSE 1
#define CE_MODULES_KBDHID 1
#define CE_MODULES_CONSHID 1
#define CE_MODULES_USBFN 1
#define CE_MODULES_RNDISFN 1
#define CE_MODULES_SERIALUSBFN 1
#define CE_MODULES_STRATAD 1
#define CE_MODULES_SDNPCID 1
#define CE_MODULES_RAMFMD 1
#define CE_MODULES_ENUMFASLPCI 1
#define CE_MODULES_FASLD 1
#define CE_MODULES_CEDDK 1
#define CE_MODULES_FATFSD 1
#define CE_MODULES_TFAT 1
#define CE_MODULES_FATUTIL 1
#define CE_MODULES_DISKCACHE 1
#define CE_MODULES_MENCFILT 1
#define CE_MODULES_CACHEFILT 1
#define CE_MODULES_IMGUPD 1
#define CE_MODULES_PACKAGEINFOAPI 1
#define CE_MODULES_IMGFS 1
#define CE_MODULES_MSPART 1
#define CE_MODULES_FSDMGR 1
#define CE_MODULES_ZLIB 1
#define CE_MODULES_GB18030 1
#define CE_MODULES_HWXJPN 1
#define CE_MODULES_HWXKOR 1
#define CE_MODULES_FILESYS 1
#define CE_MODULES_INITVMMAP 1
#define CE_MODULES_FSREPLXFILT 1
#define CE_MODULES_CECOMPR 1
#define CE_MODULES_TOOLHELP 1
#define CE_MODULES_VMINI 1
#define CE_MODULES_XMLLITE 1
#define CE_MODULES_LASSD 1
#define CE_MODULES_RT_TESTS 1
#define CE_MODULES_COREDLL 1
#define CE_MODULES_NOTIFY 1
#define CE_MODULES_COMMCTRL 1
#define CE_MODULES_BTAGSVC 1
#define CE_MODULES_BTWATCH 1
#define CE_MODULES_GWES 1
#define DCOM_MODULES_ATL 1
#define DCOM_MODULES_OLE32 1
#define DCOM_MODULES_OLEAUT32 1
#define DCOM_MODULES_UUID 1
#define GDIEX_MODULES_IMAGING 1
#define IE_MODULES_MSLS31 1
#define IE_MODULES_MSHTML 1
#define IE_MODULES_PNGFILT 1
#define IE_MODULES_IMGUTIL 1
#define IE_MODULES_DXTRANS 1
#define IE_MODULES_SHDOCLC 1
#define IE_MODULES_SHDOCVW 1
#define IE_MODULES_MSXML3 1
#define IE_MODULES_URLMON 1
#define IE_MODULES_WININET 1
#define IE_MODULES_SHLWAPI 1
#define IE_MODULES_UUID 1
#define IE_MODULES_IECEEXT 1
#define IE_MODULES_MLANG 1
#define NETCFV2_MODULES_DOTNETV2 1
#define SQLCE_MODULES_EDB 1
#define SCRIPT_MODULES_JSCRIPT 1
#define SCRIPT_MODULES_VBSCRIPT 1
#define SERVERS_MODULES_OBEXAPI 1
#define SERVERS_MODULES_OBEXSRVR 1
#define SERVERS_MODULES_SQMSVC 1
#define SERVERS_MODULES_SQMEVENT 1
#define SERVERS_MODULES_SERVICES 1
#define DIRECTX_MODULES_MP3DMOD 1
#define DIRECTX_MODULES_STRMBASE 1
#define DIRECTX_MODULES_QUARTZ 1
#define DIRECTX_MODULES_MSACMCE 1
#define DIRECTX_MODULES_MSRLE32 1
#define DIRECTX_MODULES_ICM 1
#define DIRECTX_MODULES_MSDMO 1
#define DIRECTX_MODULES_WMVDMOE 1
#define DIRECTX_MODULES_D3DMREF 1
#define DIRECTX_MODULES_D3DM_MSDRV 1
#define DIRECTX_MODULES_D3DM 1
#define DIRECTX_MODULES_D3DMSAMPLES 1
#define DIRECTX_MODULES_DIRECTDRAW 1
#define DIRECTX_MODULES_DDSAMPLES 1
#define DIRECTX_MODULES_MSG711 1
#define DIRECTX_MODULES_ACMDWRAP 1
#define WCESHELLFE_MODULES_MSHIME97 1
#define WCESHELLFE_MODULES_KIMEUI 1
#define WCESHELLFE_MODULES_CHSIME03 1
#define WCESHELLFE_MODULES_MSPY3DB1 1
#define WCESHELLFE_MODULES_MSIMECJ 1
#define WCESHELLFE_MODULES_MSIMEPH 1
#define WCESHELLFE_MODULES_MSIMEUIC 1
#define WCESHELLFE_MODULES_PIMETCDB 1
#define WCESHELLFE_MODULES_DWXFER 1
#define CELLCORE_MODULES_INTSHARE 1
#define CELLCORE_MODULES_NETWORKPOLICYCSP 1
#define CELLCORE_MODULES_RIL 1
#define CELLCORE_MODULES_CELLTSP 1
#define CELLCORE_MODULES_TAPIOCA 1
#define CELLCORE_MODULES_ATCMDINT 1
#define CELLCORE_MODULES_TBDALTER 1
#define CELLCORE_MODULES_SMS_PROVIDERS 1
#define CELLCORE_MODULES_SMS 1
#define CELLCORE_MODULES_WAPDRV 1
#define CELLCORE_MODULES_WAP 1
#define CELLCORE_MODULES_CONNMGR 1
#define CELLCORE_MODULES_CONNCFG 1
#define CELLCORE_MODULES_CMUTIL 1
#define CELLCORE_MODULES_CSPNET 1
#define CELLCORE_MODULES_CCOREDRV 1
#define CELLCORE_MODULES_CCOREUTL 1
#define CELLCORE_MODULES_CELLCORE 1
#define CELLCORE_MODULES_EXECDIAL 1
#define CELLCORE_MODULES_CONNPLAN 1
#define CELLCORE_MODULES_CSPRAS 1
#define CELLCORE_MODULES_CSPPROXY 1
#define CELLCORE_MODULES_CSPVOICE 1
#define CELLCORE_MODULES_SIMTKIT 1
#define CELLCORE_MODULES_TKITAPP 1
#define CELLCORE_MODULES_WWAN 1
#define CELLCORE_MODULES_AUTOBIND_LSP 1
#define CELLCORE_MODULES_CSPOMADMCONN 1
#define CELLCORE_MODULES_CSPWWAN 1
#define CELLCORE_MODULES_VEI 1
#define CELLCORE_MODULES_CAIFMUX 1
#define CELLCORE_MODULES_RILLOG 1
#define CELLCORE_MODULES_MUX07_10 1
#define CELLCORE_MODULES_VCOMLIB 1
#define CELLCORE_MODULES_DTPT_LSP 1
#define CELLCORE_MODULES_DTPT_NSP 1
#define CELLCORE_MODULES_DTPT_SRV 1
#define VOIP2_MODULES_RTCDLL 1
#define VOIP2_MODULES_RTCUUID 1
#define RDP_MODULES_MSTSCAX 1
#define OSTEST_MODULES_KATO 1
#define OSTEST_MODULES_TUX 1
#define OSTEST_MODULES_TOOLTALK 1
#define OSTEST_MODULES_ITLGEN 1
#define OSTEST_MODULES_NDTCRYPTLIB 1
#define OSTEST_MODULES_TTRACKER 1
#define OSTEST_MODULES_TTRACKERK 1
#define OSTEST_MODULES_UMKTEST 1
#define OSSVCS_MODULES_BTHATCI 1
#define OSSVCS_MODULES_FAKERIL 1
#define OSSVCS_MODULES_FAKERIL_INTERMEDIATE 1
#define OSSVCS_MODULES_FRINTERFACE 1
#define OSSVCS_MODULES_SWITCHRIL 1
#define OSSVCS_MODULES_BTHAGPHONEBOOK 1
#define OSSVCS_MODULES_BTHPBAP 1
#define OSSVCS_MODULES_SUPSVCS 1
#define OSSVCS_MODULES_ELDRV 1
#define OSSVCS_MODULES_OSSVCS 1
#define OSSVCS_MODULES_REGEXTDEV 1
#define OSSVCS_MODULES_RUNDLL32 1
#define OSSVCS_MODULES_CONFIGMANAGER 1
#define OSSVCS_MODULES_CONFIGMANAGER2 1
#define OSSVCS_MODULES_PUSHPRXY 1
#define OSSVCS_MODULES_WSP 1
#define OSSVCS_MODULES_WSPPUSH 1
#define OSSVCS_MODULES_PNDTAPI 1
#define OSSVCS_MODULES_EXSMIME 1
#define OSSVCS_MODULES_CEPERF 1
#define OSSVCS_MODULES_CEPERFSTUB 1
#define OSSVCS_MODULES_CERTMOD 1
#define OSSVCS_MODULES_UTCORE 1
#define OSSVCS_MODULES_DMSAPI 1
#define OSSVCS_MODULES_ENROLLSVC 1
#define OSSVCS_MODULES_ENROLLNOT 1
#define OSSVCS_MODULES_SWMGMTSERVICE 1
#define OSSVCS_MODULES_SWMGMTREGTODATABASE 1
#define OSSVCS_MODULES_FILELOGGER 1
#define OSSVCS_MODULES_BTHLINK 1
#define OSSVCS_MODULES_BTHUTIL 1
#define OSSVCS_MODULES_SMASH_EVENTSND 1
#define OSSVCS_MODULES_SMASH_SERVICES 1
#define OSSVCS_MODULES_SMASH_CONFIGMANAGER2 1
#define OSSVCS_MODULES_SMASH_TESTCSP 1
#define OSSVCS_MODULES_SMASH_CONFIGMANAGER 1
#define OSSVCS_MODULES_SMASH_REGEXT 1
#define OSSVCS_MODULES_SMASH_WSP 1
#define OSSVCS_MODULES_SMASH_STATSTOR 1
#define OSSVCS_MODULES_SMASH_STUBPVDR 1
#define OSSVCS_MODULES_SMASH_DRM_EXTERNAL 1
#define OSSVCS_MODULES_SMASH_DRM_INTERNAL 1
#define OSSVCS_MODULES_SMASH_CERTMOD 1
#define OSSVCS_MODULES_SMASH_MISC 1
#define OSSVCS_MODULES_SMASH_WRLSPWR 1
#define OSSVCS_MODULES_SMASH_WRLSPWREXT 1
#define OSSVCS_MODULES_UT_UTCORE_UIHELPERTESTS 1
#define OSSVCS_MODULES_UT_UTCORE_LOGGINGTESTS 1
#define OSSVCS_MODULES_UT_UTCORE_MACROS 1
#define OSSVCS_MODULES_UT_UTCORE_GROUPSETUPFAILURE 1
#define OSSVCS_MODULES_UT_UTCORE_GROUPTEARDOWNTEST 1
#define OSSVCS_MODULES_UT_UTCORE_TS_TT 1
#define OSSVCS_MODULES_UT_CESTL 1
#define OSSVCS_MODULES_UT_PHNUMUTIL 1
#define OSSVCS_MODULES_UT_DMSAPI 1
#define OSSVCS_MODULES_UT_OSSVCS 1
#define OSSVCS_MODULES_UT_BLUETOOTH 1
#define OSSVCS_MODULES_UT_ENROLLENGINE 1
#define OSSVCS_MODULES_UT_PERSIST 1
#define OSSVCS_MODULES_UT_PUSHROUTER 1
#define OSSVCS_MODULES_UT_PRE2E 1
#define SHELLW_MODULES_MSIM 1
#define SHELLW_MODULES_MSIMFR 1
#define SHELLW_MODULES_MSIMES 1
#define SHELLW_MODULES_MSIMDE 1
#define SHELLW_MODULES_MSIMPT 1
#define SHELLW_MODULES_MSIMIT 1
#define SHELLW_MODULES_MSIMNL 1
#define SHELLW_MODULES_MSIMPTG 1
#define SHELLW_MODULES_MSIMNO 1
#define SHELLW_MODULES_MSIMDA 1
#define SHELLW_MODULES_MSIMSW 1
#define SHELLW_MODULES_MSIMRU 1
#define SHELLW_MODULES_MSIMPL 1
#define SHELLW_MODULES_MSIMCS 1
#define SHELLW_MODULES_MSIMEL 1
#define SHELLW_MODULES_MSIMJ 1
#define SHELLW_MODULES_MSKANA 1
#define SHELLW_MODULES_MSIMK 1
#define SHELLW_MODULES_MSIMAR 1
#define SHELLW_MODULES_HWHELP 1
#define SHELLW_MODULES_WRLSMGR 1
#define SHELLW_MODULES_STARTMENU 1
#define SHELLW_MODULES_HELPSTUB 1
#define SHELLW_MODULES_TRARECOENU 1
#define SHELLW_MODULES_TRARECOFRA 1
#define SHELLW_MODULES_TRARECODEU 1
#define SHELLW_MODULES_TRANSCRIBER 1
#define SHELLW_MODULES_TRANSCRIBERSIP 1
#define SHELLW_MODULES_TRASHAPESELECTOR 1
#define SHELLW_MODULES_TRARES0409 1
#define SHELLW_MODULES_TRARES040C 1
#define SHELLW_MODULES_TRARES0407 1
#define SHELLW_MODULES_TRARES0409 1
#define SHELLW_MODULES_TRARES0409 1
#define SHELLW_MODULES_TRARES0409 1
#define SHELLW_MODULES_TRARES040C 1
#define SHELLW_MODULES_TRARES040C 1
#define SHELLW_MODULES_TRARES040C 1
#define SHELLW_MODULES_TRARES0407 1
#define SHELLW_MODULES_TRARES0407 1
#define SHELLW_MODULES_TRARES0407 1
#define SHELLW_MODULES_MBOXJPN 1
#define SHELLW_MODULES_MBOXCHS 1
#define SHELLW_MODULES_MBOXCHT 1
#define SHELLW_MODULES_SSKIM 1
#define SHELLW_MODULES_PHONIM 1
#define SHELLW_MODULES_CHAJEIIM 1
#define SHELLW_MODULES_PRTSCRN 1
#define SHELLW_MODULES_CESHELL 1
#define SHELLW_MODULES_SHELL32 1
#define SHELLW_MODULES_FEXPLORE 1
#define SHELLW_MODULES_CTLPNL 1
#define SHELLW_MODULES_AYGSHELL 1
#define SHELLW_MODULES_MSTLI 1
#define SHELLW_MODULES_FBROWSER 1
#define SHELLW_MODULES_COMPIME 1
#define SHELLW_MODULES_MSIM 1
#define SHELLW_MODULES_MSIMFR 1
#define SHELLW_MODULES_MSIMES 1
#define SHELLW_MODULES_MSIMDE 1
#define SHELLW_MODULES_MSIMPT 1
#define SHELLW_MODULES_MSIMIT 1
#define SHELLW_MODULES_MSIMNL 1
#define SHELLW_MODULES_MSIMPTG 1
#define SHELLW_MODULES_MSIMNO 1
#define SHELLW_MODULES_MSIMDA 1
#define SHELLW_MODULES_MSIMSW 1
#define SHELLW_MODULES_MSIMRU 1
#define SHELLW_MODULES_MSIMPL 1
#define SHELLW_MODULES_MSIMCS 1
#define SHELLW_MODULES_MSIMEL 1
#define SHELLW_MODULES_MSIMJ 1
#define SHELLW_MODULES_MSKANA 1
#define SHELLW_MODULES_MSIMK 1
#define SHELLW_MODULES_MSIMAR 1
#define SHELLW_MODULES_MBOXJPN 1
#define SHELLW_MODULES_SSKIM 1
#define SHELLW_MODULES_PHONIM 1
#define SHELLW_MODULES_CHAJEIIM 1
#define SHELLW_MODULES_MBOXKOR 1
#define SHELLW_MODULES_MBOXCHS 1
#define SHELLW_MODULES_MBOXCHT 1
#define SHELLW_MODULES_CPLMAIN 1
#define SHELLW_MODULES_CPLPHONE 1
#define SHELLW_MODULES_ASSISTEDDIALING 1
#define SHELLW_MODULES_IMGDECMP 1
#define SHELLW_MODULES_MSSCRIPT 1
#define SHELLW_MODULES_NETUIMDD 1
#define SHELLW_MODULES_OBEXINBX 1
#define SHELLW_MODULES_MSNSSPC 1
#define SHELLW_MODULES_TSHRES 1
#define SHELLW_MODULES_SIMSEC 1
#define SHELLW_MODULES_REMNET 1
#define SHELLW_MODULES_RNAAPP 1
#define SHELLW_MODULES_BROWSUI 1
#define SHELLW_MODULES_BROWSER 1
#define SHELLW_MODULES_WEBVIEW 1
#define SHELLW_MODULES_HTMLVIEW 1
#define SHELLW_MODULES_WMLVIEW 1
#define SHELLW_MODULES_WMLVIEW7 1
#define SHELLW_MODULES_WEBVIEW_CS 1
#define SHELLW_MODULES_IEXPLORE 1
#define SHELLW_MODULES_SHUTIL 1
#define SHELLW_MODULES_CSHUUID 1
#define SHELLW_MODULES_CCORE 1
#define SHELLW_MODULES_SYSCSPS 1
#define SHELLW_MODULES_BTHIDSVC 1
#define SHELLW_MODULES_SPDUMMY 1
#define SHELLW_MODULES_GX 1
#define SHELLW_MODULES_CEIPUI 1
#define SHELLW_MODULES_HWSETUP 1
#define SHELLW_MODULES_HWPENCE 1
#define SHELLW_MODULES_HWTOUCH 1
#define SHELLW_MODULES_HWXCHS 1
#define SHELLW_MODULES_HWXCHT 1
#define SHELLW_MODULES_IMAGELIB 1
#define SHELLW_MODULES_PIMGDLL 1
#define SHELLW_MODULES_CAMERADIAGNOSE 1
#define SHELLW_MODULES_BROWSRESDLL 1
#define SHELLW_MODULES_SHELLRESDLL 1
#define SHELLW_MODULES_SMASH_PIMGDLL 1
#define SHELLW_MODULES_SMASH_AYGSHELL 1
#define SHELLW_MODULES_SMASH_SHELL 1
#define SHELLW_MODULES_SMASH_LEGACY 1
#define SHELLW_MODULES_SMASH_DEVDETAIL 1
#define SHELLW_MODULES_SMASH_DMACC 1
#define SHELLW_MODULES_SMASH_CPLPHONE 1
#define SHELLW_MODULES_UT_WEBVIEW 1
#define SHELLW_MODULES_UT_BROWSER 1
#define SHELLW_MODULES_UT_WMLVIEW 1
#define SHELLW_MODULES_UT_CAMERAUI 1
#define SHELLW_MODULES_UT_PPCBATTERYICON 1
#define SHELLW_MODULES_UT_NETUIMDD 1
#define SHELLW_MODULES_UT_AYGSHELL 1
#define SHELLW_MODULES_UT_SHELLEXECUTE 1
#define SHELLW_MODULES_UT_CONFETTI 1
#define SHELLW_MODULES_UT_CONFETTIPLUS 1
#define SHELLW_MODULES_UT_CERTADDUI 1
#define SHELLW_MODULES_UT_CERTENROLLCSP 1
#define SHELLW_MODULES_UT_REMOTEWIPE 1
#define SHELLW_MODULES_UT_NOTIFICATIONAPI 1
#define SHELLW_MODULES_UT_BTAGSVC 1
#define SHELLW_MODULES_UT_BTWATCH 1
#define SHELLW_MODULES_UT_ALARM 1
#define SHELLW_MODULES_UT_ALARMNOTIFY 1
#define SHELLW_MODULES_UT_CERTSTORECSP 1
#define SHELLW_MODULES_UT_DOWNLOADCSP 1
#define SHELLW_MODULES_UT_APPDISABLE 1
#define SHELLW_MODULES_UT_CAMERACSP 1
#define SHELLW_MODULES_UT_STORAGECSP 1
#define SHELLW_MODULES_UT_DMACCOUNTS 1
#define SHELLW_MODULES_UT_LOCKUNLOCKVERB 1
#define SHELLW_MODULES_UT_COMPIME 1
#define SHELLW_MODULES_UT_TOUCHGESTURE 1
#define SHELLW_MODULES_UT_WINDOWAUTOGESTURE 1
#define SHELLW_MODULES_UT_THEMEPRIMITIVE 1
#define SHELLW_MODULES_UT_ROAMBLINK 1
#define SHELLW_MODULES_UT_BUBBLEBAR 1
#define SHELLW_MODULES_RIPLUGIN 1
#define SHELLW_MODULES_SYSTEMSTATEOM 1
#define SHELLW_MODULES_WIDGETSOM 1
#define SHELLW_MODULES_RIAMIMEHANDLER 1
#define SHELLW_MODULES_RIAHOST 1
#define SHELLW_MODULES_RIAHOSTRESDLL 1
#define SHELLW_MODULES_WIDGETINSTALLERDLL 1
#define SHELLW_MODULES_WMWIDGETINSTALLER 1
#define SHELLW_MODULES_IANIMATE 1
#define DIGITALMEDIA_MODULES_WMA9DECODER 1
#define DIGITALMEDIA_MODULES_WMV9DECODER 1
#define DIGITALMEDIA_MODULES_WMAV9DECODER 1
#define DIGITALMEDIA_MODULES_WMA9PRODECODER 1
#define DIGITALMEDIA_MODULES_WMCORE 1
#define DIGITALMEDIA_MODULES_WMPLAYER 1
#define DIGITALMEDIA_MODULES_WMPOCX 1
#define DIGITALMEDIA_MODULES_XMEEVTSND 1
#define DIGITALMEDIA_MODULES_WMPLOC 1
#define DIGITALMEDIA_MODULES_CEDRMUTIL 1
#define DIGITALMEDIA_MODULES_CACHERDR 1
#define DIGITALMEDIA_MODULES_AVRCP_MPPLUGIN 1
#define RM_MODULES_MSDRM 1
#define RM_MODULES_SECPROC 1
#define RM_MODULES_SECPROC_TEST 1
#define RM_MODULES_RMACTIVATE 1
#define RM_MODULES_RMACTIVATE_TEST 1
#define RM_MODULES_RMUT_END2END 1
#define APPS_MODULES_RIRESDLL 1
#define APPS_MODULES_RICHINK 1
#define APPS_MODULES_NOTE_PRJ 1
#define APPS_MODULES_WISPLITE 1
#define APPS_MODULES_GDIPLUS 1
#define APPS_MODULES_INKCTRLS 1
#define APPS_MODULES_DOCLIST 1
#define APPS_MODULES_VOICECTL 1
#define APPS_MODULES_RICHED20 1
#define APPS_MODULES_MGDWINMOB 1
#define APPS_MODULES_MGDFORMS 1
#define APPS_MODULES_MGDCFG 1
#define APPS_MODULES_VOICEDLL 1
#define APPS_MODULES_VCOMCTL 1
#define APPS_MODULES_HOTVOICE 1
#define APPS_MODULES_VOICEBAR 1
#define APPS_MODULES_VBARCALL 1
#define APPS_MODULES_INKX 1
#define APPS_MODULES_SHFIND 1
#define APPS_MODULES_RICHED20_CS 1
#define APPS_MODULES_DWUI 1
#define APPS_MODULES_DW 1
#define APPS_MODULES_PERFMAN 1
#define APPS_MODULES_FSDSPY 1
#define APPS_MODULES_DEVHEALTH 1
#define APPS_MODULES_PEGCARDS 1
#define APPS_MODULES_CITYDB 1
#define APPS_MODULES_CLOCKDLL 1
#define APPS_MODULES_CLOCKNOT 1
#define APPS_MODULES_CLOCK 1
#define APPS_MODULES_PEGHELP 1
#define APPS_MODULES_WELCOME 1
#define APPS_MODULES_WELCOMECENTER 1
#define APPS_MODULES_WCSAN 1
#define APPS_MODULES_WCPLUGIN 1
#define APPS_MODULES_AUTOTIMEDLL 1
#define APPS_MODULES_AUTOTIMEUPDATE 1
#define APPS_MODULES_SOLITARE 1
#define APPS_MODULES_TASKMGR 1
#define APPS_MODULES_ADC 1
#define APPS_MODULES_INREPLCE 1
#define APPS_MODULES_CEFOBJ 1
#define APPS_MODULES_CEMEDIA 1
#define APPS_MODULES_RRA_STM 1
#define APPS_MODULES_WCELOAD 1
#define APPS_MODULES_REPLLOG 1
#define APPS_MODULES_RAPICLNT 1
#define APPS_MODULES_UDP2TCP 1
#define APPS_MODULES_UMARSHALLER 1
#define APPS_MODULES_TMARSHALLER 1
#define APPS_MODULES_TRUSTMARSHALLER 1
#define APPS_MODULES_CABINSTL 1
#define APPS_MODULES_SNDPLAY 1
#define APPS_MODULES_INKOBJ 1
#define APPS_MODULES_PWORD 1
#define APPS_MODULES_NOTES 1
#define APPS_MODULES_PXL 1
#define APPS_MODULES_PXLFILE 1
#define APPS_MODULES_XLS2PXL 1
#define APPS_MODULES_PXL2XLS 1
#define APPS_MODULES_PWWIFF 1
#define APPS_MODULES_PWORD 1
#define APPS_MODULES_PPT 1
#define APPS_MODULES_OFFICE 1
#define APPS_MODULES_CESPELL 1
#define APPS_MODULES_ZIPVIEW 1
#define APPS_MODULES_FAXVIEW 1
#define APPS_MODULES_ONENOTE 1
#define APPS_MODULES_OFFICESETUPDLL 1
#define APPS_MODULES_FIND 1
#define APPS_MODULES_PIMAPI 1
#define APPS_MODULES_OUTRES 1
#define APPS_MODULES_PIMUTIL 1
#define APPS_MODULES_CALSTORE 1
#define APPS_MODULES_CALUPD 1
#define APPS_MODULES_IRSQRT 1
#define APPS_MODULES_BEAM 1
#define APPS_MODULES_PIMDLG 1
#define APPS_MODULES_PIMSTORE 1
#define APPS_MODULES_MGDMAPIPOOM 1
#define APPS_MODULES_CALDLL 1
#define APPS_MODULES_TASKDLL 1
#define APPS_MODULES_CALNOT 1
#define APPS_MODULES_TASKS 1
#define APPS_MODULES_MSNEWMNU 1
#define APPS_MODULES_CALENDAR 1
#define APPS_MODULES_POUTLOOK 1
#define APPS_MODULES_CONTACTS 1
#define APPS_MODULES_MCF 1
#define APPS_MODULES_MDDRM 1
#define APPS_MODULES_IRMACTIVATE 1
#define APPS_MODULES_IRMTEST 1
#define APPS_MODULES_RMACTIVATE 1
#define APPS_MODULES_PIMG 1
#define APPS_MODULES_PIMGRESDLL 1
#define APPS_MODULES_IDLEDETECT 1
#define APPS_MODULES_MOBILECALC 1
#define APPS_MODULES_CHNGTRK 1
#define APPS_MODULES_CEMAPI 1
#define APPS_MODULES_HTMLNOTE 1
#define APPS_MODULES_TMAILUTL 1
#define APPS_MODULES_TMAIL 1
#define APPS_MODULES_REHOST 1
#define APPS_MODULES_SHPTUTIL 1
#define APPS_MODULES_MSNMOBILEPLUS 1
#define APPS_MODULES_MAILTRNS 1
#define APPS_MODULES_CMMAILTRNS 1
#define APPS_MODULES_SICLNT 1
#define APPS_MODULES_TOXICD 1
#define APPS_MODULES_SYNCRES 1
#define APPS_MODULES_SYNCUTIL 1
#define APPS_MODULES_SYNCMGR 1
#define APPS_MODULES_RECIPIENTINFO 1
#define APPS_MODULES_ACTSYNCC 1
#define APPS_MODULES_SYNCCSP 1
#define APPS_MODULES_SYNCLOG 1
#define APPS_MODULES_TASKSCHEDULER 1
#define APPS_MODULES_TSKSCHCSP 1
#define APPS_MODULES_TSKSCHEDULE 1
#define APPS_MODULES_CPROG 1
#define APPS_MODULES_PHONE 1
#define APPS_MODULES_MGDTELAPI 1
#define APPS_MODULES_SIMINIT 1
#define APPS_MODULES_ATCIUI 1
#define APPS_MODULES_INTERNETSHARING 1
#define APPS_MODULES_BASICPHONE 1
#define APPS_MODULES_VOIPPHONECANVAS 1
#define APPS_MODULES_TPCUTIL 1
#define APPS_MODULES_TAPRES 1
#define APPS_MODULES_PUSHROUTER 1
#define APPS_MODULES_CORESECPROVIDERS 1
#define APPS_MODULES_CFGHOST 1
#define APPS_MODULES_APPINSTALLER 1
#define APPS_MODULES_OMADMCLIENT 1
#define APPS_MODULES_COREDPUS 1
#define APPS_MODULES_CERTINSTALLER 1
#define APPS_MODULES_CORESECPROVIDERS 1
#define APPS_MODULES_CFGHOST 1
#define APPS_MODULES_COLDINIT 1
#define APPS_MODULES_CIPHASE2 1
#define APPS_MODULES_UPDATEBIN 1
#define APPS_MODULES_ADOBEFLASH 1
#define APPS_MODULES_ADOBEFLASHCODECS 1
#define APPS_MODULES_EVTLGRES 1
#define APPS_MODULES_SQLCEMOBILE 1
#define APPS_MODULES_LAP_PW 1
#define APPS_MODULES_ARINVALID 1
#define APPS_MODULES_SDPUTIL 1
#define APPS_MODULES_DEVICEENCRYPTIONPOLICY 1
#define APPS_MODULES_SDPREADY 1
#define APPS_MODULES_AUXDISP 1
#define APPS_MODULES_CMCLIENT 1
#define APPS_MODULES_MACHINEENROLLER 1
#define APPS_MODULES_MACHINEENROLLERLAUNCH 1
#define APPS_MODULES_SOFTWAREDISABLE 1
#define APPS_MODULES_WINDOWSLIVE 1
#define APPS_MODULES_WINDOWSLIVEIM 1
#define APPS_MODULES_MSTAG 1
#define APPS_MODULES_LIVESEARCH 1
#define APPS_MODULES_SKYBOX 1
#define APPS_MODULES_SKYMARKET 1
#define APPS_MODULES_OUTLOOKLIVEUPDATER 1
#define APPS_MODULES_MFCCE 1
#define APPS_MODULES_ISVAPPSDOGFOODSETUP 1
#define APPS_MODULES_BUILDSIGNATURE 1
#define APPS_MODULES_CORERESDLL 1
#define APPS_MODULES_MISCRESDLL 1
#define APPS_MODULES_OFFICERESDLL 1
#define APPS_MODULES_PPTRESDLL 1
#define APPS_MODULES_SHELLRESAPPSDLL 1
#define APPS_MODULES_TAPRES 1
#define APPS_MODULES_SMASH_WMPPLUGIN 1
#define APPS_MODULES_SMASH_MAILAPP 1
#define APPS_MODULES_SMASH_PIMSTORE 1
#define APPS_MODULES_SMASH_TPCUTIL 1
#define APPS_MODULES_DUMPCARDS 1
#define APPS_MODULES_SMASH_PIMUTIL 1
#define APPS_MODULES_SMASH_DF_READ 1
#define APPS_MODULES_SMASH_DF_WRITE 1
#define APPS_MODULES_SMASH_SAXDLL 1
#define APPS_MODULES_SMASH_CALLPOLICY 1
#define APPS_MODULES_BTHASPLUGIN 1
#define APPS_MODULES_CEBITS 1
#define APPS_MODULES_IMGUPDATECSP 1
#define APPS_MODULES_MDSRES 1
#define APPS_MODULES_HISTORYAPI 1
#define APPS_MODULES_HISTORYUI 1
#define APPS_MODULES_DOWNLOADSERVICE 1
#define APPS_MODULES_DISSERVICEMANAGER 1
#define APPS_MODULES_DOWNLOADINSTALLSERVICE 1
#define APPS_MODULES_INSTALLSERVICE 1
#define APPS_MODULES_DOWNLOADSERVICEPROCESS 1
#define APPS_MODULES_UISERVICE 1
#define APPS_MODULES_DISSENDRESULT 1
#define APPS_MODULES_SQMDATA 1
#define APPS_MODULES_SQMCONN 1
#define APPS_MODULES_UPTIME 1
#define APPS_MODULES_UPTIMESQM 1
#define APPS_MODULES_UT_CEMAPI_STORETESTS 1
#define APPS_MODULES_UT_CEMAPI_TMAILUTLTESTS 1
#define APPS_MODULES_UT_MAILTRNSSHARED 1
#define APPS_MODULES_UT_CEMAPI_IMAP 1
#define APPS_MODULES_UT_SYNCMANAGER 1
#define APPS_MODULES_UT_CALSTORE 1
#define APPS_MODULES_UT_SIMSMS 1
#define APPS_MODULES_UT_CLEANSMSINBOX 1
#define APPS_MODULES_UT_INITSIMSMS 1
#define APPS_MODULES_UT_MRSMS_ERRHANDL 1
#define APPS_MODULES_UT_EMN 1
#define APPS_MODULES_UT_CMCONFIG 1
#define APPS_MODULES_UT_CMBOMB 1
#define APPS_MODULES_UT_HTMLNOTE 1
#define APPS_MODULES_UT_PHLOGUTL 1
#define APPS_MODULES_UT_SYNCSERVICES 1
#define APPS_MODULES_UT_TMAIL_SMARTFILTER_TESTS 1
#define APPS_MODULES_UT_REGEX 1
#define APPS_MODULES_UT_TPCUTIL 1
#define APPS_MODULES_UT_CPROG 1
#define APPS_MODULES_UT_TMAIL_UTILS 1
#define APPS_MODULES_UT_UPTIME 1
#define APPS_MODULES_UT_UPTTEST 1
#define APPS_MODULES_UT_UPTIMESQM 1
#define APPS_MODULES_UT_PIMSTORE 1
#define APPS_MODULES_UT_COREDPUS 1
#define APPS_MODULES_UT_PIMUTIL 1
#define APPS_MODULES_UT_SYNCPROVIDERS 1
#define APPS_MODULES_UT_DVS 1
#define APPS_MODULES_UT_SYNCFETCHQUEUE 1
#define APPS_MODULES_UT_SYNCCSP 1
#define APPS_MODULES_UT_SYNCCONTROLLER 1
#define APPS_MODULES_UT_SYNCUTIL 1
#define APPS_MODULES_UT_TASKSCHEDULER 1
#define APPS_MODULES_UT_TSKSCHCSP 1
#define APPS_MODULES_UT_DETAILS 1
#define APPS_MODULES_UT_SHPTUTIL 1
#define APPS_MODULES_UT_ACTSYNCC 1
#define APPS_MODULES_UT_PENDINGFETCH 1
#define APPS_MODULES_UT_WCELOAD 1
#define APPS_MODULES_UT_CONTACTS 1
#define APPS_MODULES_UT_CERTINSTALLER 1
#define APPS_MODULES_SHIMARC 1
#define APPS_MODULES_UT_LAP 1
#define APPS_MODULES_UT_PHONE 1
#define APPS_MODULES_UT_MAILWIZARD 1
#define APPS_MODULES_UT_PWORDREAD 1
#define APPS_MODULES_UT_PWORDWRITE 1
#define APPS_MODULES_UT_POP3 1
#define APPS_MODULES_UT_REHOST 1
#define APPS_MODULES_UT_AUTOTIMEUPDATE 1
#define APPS_MODULES_UT_THREADEDSMS 1
#define APPS_MODULES_UT_CMHOME 1
#define APPS_MODULES_UT_BASICPHONE 1
#define APPS_MODULES_UT_DISSENDRESULT 1
#define APPS_MODULES_UT_LOGSTRING 1
#define APPS_MODULES_UT_HISTORYAPI 1
#define APPS_MODULES_UT_DOWNLOADSERVICE 1
#define APPS_MODULES_UT_INSTALLSERVICE 1
#define APPS_MODULES_UT_DISSENDRESULT 1
#define APPS_MODULES_UT_SDPUTIL 1
#define APPS_MODULES_UT_MAILPVDR 1
#define APPS_MODULES_UT_PROVISION 1
#define APPS_MODULES_UT_LOCKSCREENPICKER 1
#define APPS_MODULES_UT_RECIPIENTINFO 1
#define APPS_MODULES_UT_LOCKSCREEN 1
#define APPS_MODULES_CMHOME 1
#define APPS_MODULES_CMHOMEWMPPLUGIN 1
#define APPS_MODULES_LOCKSCREEN 1
#define APPS_MODULES_DLREGCSP 1
#define APPS_MODULES_AUTOUPDATE 1
#define APPS_MODULES_AUTOUPDATERES 1
#define APPS_MODULES_AUTOUPDATECPL 1
#define APPS_MODULES_DULOGGER 1
#define APPS_MODULES_IPSECVPNVNIC 1
#define APPS_MODULES_IPSECVPN 1
#define APPS_MODULES_IPSECVPNPM 1
#define APPS_MODULES_IPSECVPNCSP 1
#define APPS_MODULES_IPSECVPNAPP 1
#define APPS_MODULES_IPSECVPNRESDLL 1
#define APPS_MODULES_IPSECVPNUI 1
#define APPS_MODULES_ALERTER 1
#define APPS_MODULES_TDETCLIENT 1
#define APPS_MODULES_NODEMONCSP 1
#define APPS_MODULES_NODEMONINITSESS 1
#define VOICECOMMAND_MODULES_SPCACHEAUDIO 1
#define VOICECOMMAND_MODULES_VOICECMD_PPC 1
#define VOICECOMMAND_MODULES_VOICECOMMAND_PPC 1
#define VOICECOMMAND_MODULES_VCOSSHIM_PPC05 1
#define VOICECOMMAND_MODULES_VOICECOMMANDWMP_PPC 1
#define VOICECOMMAND_MODULES_VOICECMDRES_PPC_0407 1
#define VOICECOMMAND_MODULES_VOICECMDRES_PPC_0409 1
#define VOICECOMMAND_MODULES_VOICECMDRES_PPC_0809 1
#define VOICECOMMAND_MODULES_VOICECMDRES_PPC_040C 1
#define COREDLL_COREMAIN 1
#define COREDLL_LMEM 1
#define COREDLL_SHOWERR 1
#define COREDLL_THUNKS 1
#define COREDLL_CORECRT 1
#define COREDLL_CORESTRW 1
#define COREDLL_SECUREWIPE 1
#define COREDLL_TNOTIFY 1
#define COREDLL_TAPILIB 1
#define COREDLL_CRYPTAPI 1
#define COREDLL_TBTCORE 1
#define COREDLL_CORESIP 1
#define COREDLL_RECTAPI 1
#define COREDLL_WMGR_C 1
#define COREDLL_MGDI_C 1
#define COREDLL_ACCEL_C 1
#define COREDLL_MESSAGEDIALOGBOXTHUNK 1
#define COREDLL_SHCORE 1
#define COREDLL_SHORTCUT 1
#define COREDLL_SHEXEC 1
#define COREDLL_SHMISC 1
#define COREDLL_FILEOPEN 1
#define COREDLL_FILEINFO 1
#define COREDLL_SHELLAPIS 1
#define COREDLL_DSA 1
#define COREDLL_MMACM 1
#define COREDLL_MMACMUI 1
#define COREDLL_MMWAVE 1
#define COREDLL_MMSND 1
#define COREDLL_MMMIX 1
#define COREDLL_BATTERY 1
#define COREDLL_NLED 1
#define COREDLL_DEVENUM 1
#define COREDLL_DEVLOAD 1
#define COREDLL_FIBER 1
#define COREDLL_COREIMM 1
#define COREDLL_FMTRES 1
#define COREDLL_FMTMSG 1
#define COREDLL_CORESTRA 1
#define COREDLL_CORESIOW 1
#define COREDLL_CORESIOA 1
#define COREDLL_SERDEV 1
#define COREDLL_CORELOC 1
#define COREDLL_MULTIUI 1
#define COREDLL_CRT_CPP_EH_AND_RTTI 1
#define COREDLL_FULL_CRT 1
#define COREDLL_CRYPTHASH 1
#define COREDLL_RSA32 1
#define COREDLL_SHEXEC 1
#define COREDLL_FILEINFO 1
#define COREDLL_SHORTCUT 1
#define COREDLL_FILEOPEN 1
#define COREDLL_SECLOADER 1
#define NK_NKCOMPR 1
#define NK_NKMAPFILE 1
#define COMMCTRL_TOOLBAR 1
#define COMMCTRL_UPDOWN 1
#define COMMCTRL_STATUS 1
#define COMMCTRL_PROPSHEET 1
#define COMMCTRL_LISTVIEW 1
#define COMMCTRL_TREEVIEW 1
#define COMMCTRL_DATE 1
#define COMMCTRL_TAB 1
#define COMMCTRL_PROGRESS 1
#define COMMCTRL_TRACKBAR 1
#define COMMCTRL_CAPEDIT 1
#define COMMCTRL_REBAR 1
#define COMMCTRL_CMDBAR 1
#define COMMCTRL_DSA 1
#define COMMCTRL_TOOLTIPS 1
#define COMMCTRL_FE 1
#define COMMCTRL_CUTCOPYPASTE 1
#define COMMCTRL_LABELEDIT 1
#define COMMCTRL_BOXSELECT 1
#define COMMCTRL_SHAPIS 1
#define COMMCTRL_COMMCTRLVIEW 1
#define AFD_AFDDNSLIB 1
#define AFD_AFDDNSPROXY 1
#define WINSOCK_SSLSOCK 1
#define AFD_AFDDNSLIB 1
#define AFD_AFDDNSPROXY 1
#define PPP_PPP2SRVSTUB 1
#define CRYPT32_WINCRMSG 1
#define CRYPT32_PFX 1
#define BTD_HCI 1
#define BTD_L2CAP 1
#define BTD_SDP 1
#define BTD_RFCOMM 1
#define BTD_PORTEMU 1
#define BTD_SYS 1
#define BTD_TDI 1
#define BTD_BTPAN 1
#define BTD_AVDTP 1
#define BTD_AVCTP 1
#define BTD_AVRCP 1
#define BTD_UNIV 1
#define BTDRT_SDPUSER 1
#define BTDRT_BTHNS 1
#define FONTS_TAHOMA_1_07 1
#define FONTS_COUR_1_30 1
#define FONTS_WINGDING 1
#define FONTS_MSGOTHIC_1_60 1
#define WAVEAPI_WAPIACM 1
#define WAVEAPI_WAPIACMUI 1
#define WAVEAPI_WAPIWAVE 1
#define WAVEAPI_WAPIMAP 1
#define DEVICE_DEVCORE 1
#define DEVICE_IORM 1
#define DEVICE_PMIF 1
#define PM_PM_PDA_PDD 1
#define PM_PM_MDD 1
#define PM_PM_PDD_COMMON 1
#define FATUTIL_FATUTIL_NOUI 1
#define FATUTIL_FATUTIL_MAIN 1
#define IMGUPD_KEY_FILE 1
#define IMGUPD_CIF_FILE 1
#define IMGUPD_SOF_FILE 1
#define IMGUPD_FSDMGR_INI 1
#define IMGFS_IMGFS_MAIN 1
#define IMGFS_IMGFS_NOWRITE 1
#define FILESYS_FSHEAP 1
#define FILESYS_FSMAIN 1
#define FILESYS_FSPROFILE 1
#define FILESYS_FSREGHIVE 1
#define FILESYS_FSYSROM 1
#define FILESYS_FSREPLBIT 1
#define FILESYS_FSDBASE 1
#define FILESYS_FSPASS 1
#define FILESYS_FSADVERTISE 1
#define FILESYS_MSGQUEUE 1
#define CECOMPR_XPRDECOMP 1
#define COREDLL_COREMAIN 1
#define COREDLL_LMEM 1
#define COREDLL_SHOWERR 1
#define COREDLL_THUNKS 1
#define COREDLL_CORECRT 1
#define COREDLL_CORESTRW 1
#define COREDLL_SECUREWIPE 1
#define COREDLL_TNOTIFY 1
#define COREDLL_TAPILIB 1
#define COREDLL_CRYPTAPI 1
#define COREDLL_TBTCORE 1
#define COREDLL_CORESIP 1
#define COREDLL_RECTAPI 1
#define COREDLL_WMGR_C 1
#define COREDLL_MGDI_C 1
#define COREDLL_ACCEL_C 1
#define COREDLL_MESSAGEDIALOGBOXTHUNK 1
#define COREDLL_SHCORE 1
#define COREDLL_SHORTCUT 1
#define COREDLL_SHEXEC 1
#define COREDLL_SHMISC 1
#define COREDLL_FILEOPEN 1
#define COREDLL_FILEINFO 1
#define COREDLL_SHELLAPIS 1
#define COREDLL_DSA 1
#define COREDLL_MMACM 1
#define COREDLL_MMACMUI 1
#define COREDLL_MMWAVE 1
#define COREDLL_MMSND 1
#define COREDLL_MMMIX 1
#define COREDLL_BATTERY 1
#define COREDLL_NLED 1
#define COREDLL_DEVENUM 1
#define COREDLL_DEVLOAD 1
#define COREDLL_FIBER 1
#define COREDLL_COREIMM 1
#define COREDLL_FMTRES 1
#define COREDLL_FMTMSG 1
#define COREDLL_CORESTRA 1
#define COREDLL_CORESIOW 1
#define COREDLL_CORESIOA 1
#define COREDLL_SERDEV 1
#define COREDLL_CORELOC 1
#define COREDLL_MULTIUI 1
#define COREDLL_CRT_CPP_EH_AND_RTTI 1
#define COREDLL_FULL_CRT 1
#define COREDLL_CRYPTHASH 1
#define COREDLL_RSA32 1
#define COREDLL_SHEXEC 1
#define COREDLL_FILEINFO 1
#define COREDLL_SHORTCUT 1
#define COREDLL_FILEOPEN 1
#define COREDLL_SECLOADER 1
#define NOTIFY_NOTIFPUB 1
#define NOTIFY_NOTIFUI 1
#define NOTIFY_NOTIFPUB_WPC 1
#define NOTIFY_NOTIFPUB 1
#define NOTIFY_DLGCMN2 1
#define COMMCTRL_TOOLBAR 1
#define COMMCTRL_UPDOWN 1
#define COMMCTRL_STATUS 1
#define COMMCTRL_PROPSHEET 1
#define COMMCTRL_LISTVIEW 1
#define COMMCTRL_TREEVIEW 1
#define COMMCTRL_DATE 1
#define COMMCTRL_TAB 1
#define COMMCTRL_PROGRESS 1
#define COMMCTRL_TRACKBAR 1
#define COMMCTRL_CAPEDIT 1
#define COMMCTRL_REBAR 1
#define COMMCTRL_CMDBAR 1
#define COMMCTRL_DSA 1
#define COMMCTRL_TOOLTIPS 1
#define COMMCTRL_FE 1
#define COMMCTRL_CUTCOPYPASTE 1
#define COMMCTRL_LABELEDIT 1
#define COMMCTRL_BOXSELECT 1
#define COMMCTRL_SHAPIS 1
#define COMMCTRL_COMMCTRLVIEW 1
#define BTAGSVC_BTAG_MDD_PHONEEXT 1
#define BTAGSVC_BTAG_MDD_NETWORK 1
#define BTAGSVC_BTAG_MDD_BOND 1
#define BTWATCH_BTWATCH_NOTIFY 1
#define BTWATCH_BTWATCH_UTIL 1
#define BTWATCH_BTWATCH_BOND 1
#define GWES_GWE1 1
#define GWES_WMBASE 1
#define GWES_GWESHARE 1
#define GWES_GWESMAIN 1
#define GWES_IMMTHUNK 1
#define GWES_MSGQUE 1
#define GWES_GSETWINLONG 1
#define GWES_CEPTR 1
#define GWES_GWEPERF 1
#define GWES_TOUCHPERF 1
#define GWES_FOREGND 1
#define GWES_IDLE 1
#define GWES_KBDUI 1
#define GWES_UIBASE 1
#define GWES_MSGBEEP 1
#define GWES_PIXELDOUBLE 1
#define GWES_AUDIO 1
#define GWES_GWE2 1
#define GWES_MGBASE 1
#define GWES_MGBITMAP 1
#define GWES_MGBLT 1
#define GWES_MGBLT2 1
#define GWES_MGDC 1
#define GWES_MGDIBSEC 1
#define GWES_MGDRAW 1
#define GWES_MGRGN 1
#define GWES_MGWINMGR 1
#define GWES_TCHUI 1
#define GWES_CALIBRUI 1
#define GWES_MGGRADFILL 1
#define GWES_MGALPHABLEND 1
#define GWES_MGTT 1
#define GWES_DECOMPDRV 1
#define GWES_MGFE 1
#define GWES_MGFNTFIX 1
#define GWES_MGDRWTXT 1
#define GWES_MGPRINT 1
#define GWES_MGPAL 1
#define GWES_MGPALNAT 1
#define GWES_HAPTICRESPONSE 1
#define GWES_GWE3 1
#define GWES_ACCEL 1
#define GWES_BTNCTL 1
#define GWES_CARET 1
#define GWES_CASCADE 1
#define GWES_IMECTL 1
#define GWES_CLIPBD 1
#define GWES_CMBCTL 1
#define GWES_DEFWNDPROC 1
#define GWES_DLGMGR 1
#define GWES_DLGMNEM 1
#define GWES_EDCTL 1
#define GWES_GCACHE 1
#define GWES_GWECTRL 1
#define GWES_ICON 1
#define GWES_ICONCMN 1
#define GWES_IMGCTL 1
#define GWES_LBCTL 1
#define GWES_LOADBMP 1
#define GWES_LOADIMG 1
#define GWES_MENU 1
#define GWES_MENUSCRL 1
#define GWES_LAYEREDWINDOWS 1
#define GWES_TRANSITIONANIMATIONS 1
#define GWES_MOVERLAP 1
#define GWES_MOUSEANDTOUCHCURSOR 1
#define GWES_ICONCURS 1
#define GWES_MCURSOR 1
#define GWES_MCURSOR8 1
#define GWES_CURSOR 1
#define GWES_CURSOR8 1
#define GWES_MTAPUI 1
#define GWES_TOUCHGESTURE 1
#define GWES_GESTUREANIMATION 1
#define GWES_PHYSICSWRAPPER 1
#define GWES_GWE4 1
#define GWES_NCLIENT 1
#define GWES_OOM 1
#define GWES_SBCMN 1
#define GWES_SCBCTL 1
#define GWES_STARTUP 1
#define GWES_STCCTL 1
#define GWES_WINMGR 1
#define GWES_SBCMNVIEW 1
#define GWES_NCLIENTVIEW 1
#define GWES_GCACHEVIEW 1
#define GWES_BTNCTLVIEW 1
#define GWES_STCCTLVIEW 1
#define GWES_CMBCTLVIEW 1
#define GWES_EDCTLVIEW 1
#define GWES_LBCTLVIEW 1
#define GWES_MENUVIEW 1
#define GWES_EDIMEFE 1
#define GWES_MSGBOX 1
#define GWES_MSGBOX_PPC 1
#define GWES_DLGMGR_PPC 1
#define GWES_MENU_PPC 1
#define GWES_CMBCTL_PPC 1
#define GWES_EDITCONTROLPPC 1
#define GWES_LBCTL_PPC 1
#define GWES_BTNCTL_PPC 1
#define GWES_TIMER 1
#define GWES_COLUMN 1
#define GWES_ATOM 1
#define GWES_DRAWMBAR 1
#define GWES_HOTKEY 1
#define GWES_SYSCOLOR 1
#define GWES_MGDX 1
#define GWES_MGALIAS 1
#define GWES_JOURNAL 1
#define GWES_D3DMHOOK 1
#define GWES_DDCORE 1
#define GWES_MGTCI 1
#define GWES_CALIBRUI_WPC 1
#define GWES_STARTUI_WPC 1
#define GWES_OOMUI_WPC 1
#define GWES_CURSOR8 1
#define GWES_GCACHEVIEW 1
#define GWES_GCACHEVW_WPC 1
#define GWES_MSGBOX_PPC 1
#define GWES_STARTUP 1
#define GWES_CALIBRUI 1
#define GWES_DLGCMN 1
#define GWES_CURSOR 1
#define GWES_NCLIENTVIEW 1
#define GWES_NCLIENTVIEW_WPC 1
#define GWES_BTNCTLVIEW 1
#define GWES_BTNCTLVIEW_WPC 1
#define GWES_CMBCTLVIEW 1
#define GWES_EDCTLVIEW 1
#define GWES_LBCTLVIEW 1
#define GWES_LBCTLVIEW_WPC 1
#define GWES_MENUVIEW 1
#define GWES_SBCMNVIEW_WPC 1
#define GWES_STCCTLVIEW 1
#define GWES_PHONECOMMUNICATOR 1
#define GWES_AYGSHELL 1
#define OLE32_MCOMBASE 1
#define OLE32_MCOMMEM 1
#define OLE32_MCOMSTR 1
#define OLE32_MCOMMON 1
#define OLE32_MCOMLIB 1
#define OLE32_OLE232 1
#define OLE32_DOCFILE 1
#define OLE32_MSF 1
#define OLE32_EXP 1
#define OLE32_MCOMGUID 1
#define OLE32_MCOMSTM 1
#define OLEAUT32_OAALL 1
#define IMAGING_IMG_ICO 1
#define IMAGING_IMG_TIFF 1
#define IMAGING_IMG_LIBTIFF 1
#define IMAGING_IMG_PNG 1
#define IMAGING_IMG_LIBPNG 1
#define IMAGING_IMG_PNGENCODER 1
#define IMAGING_IMG_PNGDECODER 1
#define IMAGING_IMG_GIF 1
#define IMAGING_IMG_LIBLZW 1
#define IMAGING_IMG_GIFENCODER 1
#define IMAGING_IMG_GIFDECODER 1
#define IMAGING_IMG_JPEG 1
#define IMAGING_IMG_JPEGFULL 1
#define IMAGING_IMG_JPEGMEM 1
#define IMAGING_IMG_JPEGENCODER 1
#define IMAGING_IMG_JPEGDECODER 1
#define IMAGING_IMG_BMP 1
#define IMAGING_IMG_BMPENCODER 1
#define IMAGING_IMG_BMPDECODER 1
#define MSHTML_MSHTML_DBIND 1
#define SHDOCVW_URLMON 1
#define SHDOCVW_ZONES 1
#define SHDOCVW_DOCHOST 1
#define SHDOCVW_DHEXT 1
#define SHDOCVW_SHDOCRES 1
#define MSXML3_XMLDOM 1
#define MSXML3_XMLMIME 1
#define MSXML3_XMLXSLT 1
#define MSXML3_XMLISLANDS 1
#define MSXML3_XMLXQL 1
#define MSXML3_XMLSAX 1
#define MSXML3_XMLHTTP 1
#define MSXML3_XMLNETFULL 1
#define URLMON_ZONES 1
#define WININET_PPSTUBS 1
#define SHLWAPI_SHLWIE 1
#define IECEEXT_IECEEXTMCOM 1
#define VBSCRIPT_VBSSTUBS 1
#define QUARTZ_QUARTZ0 1
#define QUARTZ_AMUTIL 1
#define QUARTZ_FGCTL 1
#define QUARTZ_FILGRAPH 1
#define QUARTZ_QUARTZ1 1
#define QUARTZ_WAVEOUT 1
#define QUARTZ_WAVEMSR 1
#define QUARTZ_AVIMSR 1
#define QUARTZ_AVIDEC 1
#define QUARTZ_ACMWRAP 1
#define QUARTZ_DMOWRAP 1
#define QUARTZ_IMAGE 1
#define QUARTZ_QUARTZ2 1
#define QUARTZ_ASYNCRDR 1
#define QUARTZ_QTZBASE 1
#define QUARTZ_IMGSINK 1
#define QUARTZ_SMARTTEE 1
#define QUARTZ_QUARTZ3 1
#define QUARTZ_ASFWRITER 1
#define QUARTZ_ASFMUXCORE 1
#define QUARTZ_WAVEIN 1
#define QUARTZ_VIDCAP 1
#define QUARTZ_FILCAP 1
#define QUARTZ_QUARTZ4 1
#define QUARTZ_BUFFILTER 1
#define QUARTZ_COLOUR 1
#define MSTSCAX_RDRBASE 1
#define MSTSCAX_RDRDRIVE 1
#define MSTSCAX_RDRCLIP 1
#define MSTSCAX_RDRAUDIO 1
#define MSTSCAX_GWEAPI 1
#define MSTSCAX_TSCWINCTRL 1
#define EXSMIME_DSTRUCT 1
#define EXSMIME_EXSMIME_COMMON 1
#define SYSCSPS_CERTIFICATECSP 1
#define SYSCSPS_DEVINFOCSP 1
#define SYSCSPS_MAILCSP2 1
#define SYSCSPS_FILECSP 1
#define SYSCSPS_REVOCATIONLISTCSP 1
#define SYSCSPS_POLICYCSP 1
#define SYSCSPS_WAPCSP 1
#define SYSCSPS_METABASECSP 1
#define SYSCSPS_INSTDLLCSP 1
#define SYSCSPS_SPEEDDIALCSP 1
#define SYSCSPS_UNINSTALLCSP 1
#define SYSCSPS_LEGACYCSP 1
#define SYSCSPS_APPLCSP 1
#define SYSCSPS_WIFICSP 1
#define SYSCSPS_ENCRYPTIONCSP 1
#define SYSCSPS_CERTIFICATEENROLLERCSP 1
#define SYSCSPS_REMOTEWIPECSP 1
#define SYSCSPS_APPDISABLECSP 1
#define SYSCSPS_DEVICEENCRYPTIONCSP 1
#define SYSCSPS_CAMERACSP 1
#define SYSCSPS_STORAGECSP 1
#define SYSCSPS_DEVINFO2CSP 1
#define SYSCSPS_DEVDETAILCSP 1
#define SYSCSPS_DMACCCSP 1
#define SYSCSPS_DMSCSP 1
#define SYSCSPS_DOWNLOADCSP 1
#define SYSCSPS_INVENTORYCSP 1
#define SYSCSPS_FILESYSTEMCSP 1
#define RICHINK_RINPUT 1
#define RICHINK_PWIRTF 1
#define OFFICE_WORDITEM 1
#define OFFICE_FINDWORD 1
#define OFFICE_OFFNWMNU 1
#define CESPELL_SL 1
#define CESPELL_SF 1
#define CESPELL_CM 1
#define CESPELL_IP 1
#define CESPELL_WIZLAYER 1
#define FIND_VLIST 1
#define OUTRES_OUTRES_WPC 1
#define PIMUTIL_PARSERX 1
#define PIMUTIL_PARSERXSTUBS_WPC 1
#define PIMUTIL_PARSERO11 1
#define CALDLL_LUNAR_WPC 1
#define CALDLL_CALDLL_WPC 1
#define CALDLL_CALVIEW 1
#define CALDLL_AGENDA_WPC 1
#define CALDLL_YEAR_WPC 1
#define CALDLL_MONTH_WPC 1
#define CALDLL_WEEK_WPC 1
#define CALDLL_DAY_WPC 1
#define CALDLL_DETAILS 1
#define CALDLL_SHORTCUTSHANDLER 1
#define CALDLL_CALUTIL 1
#define TASKDLL_RCNTOBJ 1
#define CHNGTRK_CTUTIL 1
#define CHNGTRK_CTCEMAPI 1
#define CEMAPI_MAILUTIL_MISC 1
#define CEMAPI_MAILUTIL_DUPSTRING 1
#define CEMAPI_MAILUTIL_XLIST 1
#define CEMAPI_CEMAPI_WPC 1
#define TMAILUTL_MAILUTIL_MISC 1
#define TMAILUTL_MAILUTIL_XLIST 1
#define TMAILUTL_MAILUTIL_PPCMISC 1
#define TMAILUTL_MAILTRNS_SUPPORT 1
#define TMAILUTL_SHORTCUTSHANDLER 1
#define TMAILUTL_MAILUTIL_TNEF 1
#define TMAILUTL_MAILUTIL_VCAL 1
#define TMAILUTL_MAILUTIL_VCAL_WPC 1
#define TMAILUTL_MAILUTIL_RTF 1
#define TMAILUTL_MAILUTIL_MIME 1
#define TMAILUTL_MAILUTIL_DUPSTRING 1
#define TMAILUTL_MAILUTIL_MAILCERT 1
#define TMAILUTL_SMIME_CORE 1
#define MAILTRNS_MAILUTIL_XLIST 1
#define MAILTRNS_IMAP_LIB 1
#define MAILTRNS_SMTP_LIB 1
#define MAILTRNS_POP3_LIB 1
#define MAILTRNS_MAILTRNS_SUPPORT 1
#define CMMAILTRNS_MAILUTIL_XLIST 1
#define CMMAILTRNS_CMPOP3_LIB 1
#define CMMAILTRNS_CMSMTP_LIB 1
#define CMMAILTRNS_MAILTRNS_SUPPORT 1
#define SYNCRES_SYNCRES_WPC 1
#define SYNCMGR_SYNCSTATE_WPC 1
#define SYNCMGR_SMUTIL 1
#define SYNCMGR_SMUTIL_WPC 1
#define SYNCMGR_MAILPVDR 1
#define SYNCMGR_MAILPVDR_WPC 1
#define SYNCMGR_SMPRVDR 1
#define SYNCMGR_SMPRVDR_WPC 1
#define SYNCMGR_HTTPAUTD 1
#define SYNCMGR_HTTPAUTD_WPC 1
#define SYNCMGR_PROVISION 1
#define SYNCMGR_SRVRSYNC 1
#define SYNCMGR_SYNCMGR_WPC 1
#define SYNCMGR_SYNCNOTIFY 1
#define SYNCMGR_ACTVENGINE 1
#define SYNCMGR_ACTVENGINE_WPC 1
#define SYNCMGR_AIRSYNCCONST 1
#define SYNCMGR_AIRSYNCOPTIONS 1
#define SYNCMGR_AIRSYNCPROT 1
#define SYNCMGR_AIRSYNCSCHEMA 1
#define SYNCMGR_AIRSYNCSYNC 1
#define SYNCMGR_AIRSYNCUTIL 1
#define SYNCMGR_AIRSYNCXML 1
#define SYNCMGR_AIRSYNC 1
#define SYNCMGR_AIRSYNCCONST 1
#define SYNCMGR_AIRSYNCOPTIONS 1
#define SYNCMGR_AIRSYNCPROT 1
#define SYNCMGR_AIRSYNCSCHEMA 1
#define SYNCMGR_AIRSYNCSYNC 1
#define SYNCMGR_AIRSYNCUTIL 1
#define SYNCMGR_AIRSYNCXML 1
#define SYNCMGR_AIRSYNC 1
#define SYNCMGR_SYNCSERVICES 1
#define TPCUTIL_CONTACTS 1
#define TPCUTIL_CONTACTS_PPC 1
#define TPCUTIL_PIMQA 1
#define TPCUTIL_DIALPARSCORE 1
#define TPCUTIL_DIALPARSEXTENS 1
#define TPCUTIL_SMSTRN_LIB 1
#define TPCUTIL_MAILTRNS_SUPPORT 1
#define TPCUTIL_MAILUTIL_XLIST 1
#define TPCUTIL_MAILUTIL_DUPSTRING 1
#define CORESECPROVIDERS_DEFAULTAUTHPROV 1
#define CORESECPROVIDERS_WAPAUTHPROV_WPC 1
#define CORESECPROVIDERS_SYNCMLAUTHPROV 1
#define COREDPUS_SYNCMLDPU 1
#define CORESECPROVIDERS_DEFAULTAUTHPROV 1
#define CORESECPROVIDERS_WAPAUTHPROV_WPC 1
#define CORESECPROVIDERS_SYNCMLAUTHPROV 1
#endif
