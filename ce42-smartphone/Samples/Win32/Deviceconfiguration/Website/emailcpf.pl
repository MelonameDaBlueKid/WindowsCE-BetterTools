...
$mailfrom   = $query->param('mailfrom');
$servername = $query->param('emailserver');
...
print MYXML "$xmlbody";
close(MYXML) or die "Can't close pipe to profiles\\$profname\\cpf\\$xmlfile";
...
else
{
    ...
    print "Done";
    print "<meta http-equiv=\"Refresh\" content=\"1; URL=emailcpf.asp?mailto=$mailto&cpffile=profiles\\$profname\\cpf\\$cpffile&emailserver=$servername&mailfrom=$mailfrom\">";
    print "<p>If this page is not forwarded, click <a href=\"emailcpf.asp?mailto=$mailto&cpffile=profiles\\$profname\\cpf\\$cpffile&emailserver=$servername&mailfrom=$mailfrom\">here</a>";
    print "</BODY></HTML>";
}
