                   ...
                   int       CmdShow)
{
    ...
    if(!_tcsicmp(lpCmdLine, TEXT("/register"))) 
    {
        ...
        RETAILMSG(SUCCEEDED(hr), (TEXT("CfgClient: Client successfully registered.\r\n")));
        goto Error;
    } 
    else if(!_tcsicmp(lpCmdLine, TEXT("/unregister"))) 
    {
        hr = PushRouter_UnRegisterClient(c_szContentType, c_szAppId);
        RETAILMSG(SUCCEEDED(hr), (TEXT("CfgClient: Client successfully unregistered.\r\n")));
        goto Error;
    }
...
Error:
    ...
    {
        RETAILMSG(1, (TEXT("CfgClient: FAILURE!!! (hr = 0x%08x)\r\n"), hr));
        return 1;
    }
}