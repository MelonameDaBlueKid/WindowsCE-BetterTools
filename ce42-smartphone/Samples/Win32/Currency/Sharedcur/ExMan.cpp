    ...
    )
{
    ...
    if(!m_fMounted)
        ...
        #else
        // Pocket PC doesn't have this defined so
        // put our directory at the root.  This will
        // likely change in the future and this code
        // should still work.
        _tcscpy(szVolumeFile, TEXT("\\"));
        #endif

        if(_tcslen(szVolumeFile) + ARRAYSIZE(g_szAppDataDir) + ARRAYSIZE(g_szAppDataBaseVolumeFileName) + 10 >
        ...
        if(szVolumeFile[_tcslen(szVolumeFile) - 1] != TEXT('\\'))
            {
            _tcscat(szVolumeFile, TEXT("\\"));
            }
        _tcscat(szVolumeFile, g_szAppDataDir);
        ...
        _tcscat(szVolumeFile, TEXT("\\"));
        _tcscat(szVolumeFile, g_szAppDataBaseVolumeFileName);

        if(!CeMountDBVol(&m_guidVolume, szVolumeFile, OPEN_ALWAYS))
            {
            hr = HRESULT_FROM_WIN32(GetLastError());
            goto Error;
            }
    ...
    if(INVALID_HANDLE_VALUE == m_hdb)
        ...
        if(INVALID_HANDLE_VALUE == m_hdb)
            ...
            _tcscpy(dbi.szDbaseName, c_pszDatabaseName);
            dbi.wNumSortOrder = 1;
            dbi.rgSortSpecs[0].propid = CPR_TLA;
            dbi.rgSortSpecs[0].dwFlags = CEDB_SORT_CASEINSENSITIVE;
            m_oiddb = CeCreateDatabaseEx(&m_guidVolume, &dbi);
            if(NULL == m_oiddb)
                {
            ...
            notreq.hwnd = m_hwndWorker;
            notreq.dwFlags = 0; // we want V1.0-2.0 behavior
            m_hdb = CeOpenDatabaseEx(&m_guidVolume, &m_oiddb, NULL , CPR_TLA, 0, &notreq);
            if(INVALID_HANDLE_VALUE == m_hdb)
                {
    ...
    )
{
    ...
    if(NULL == m_rgVisibleLookup)
        ...
        if(!CeOidGetInfoEx(&m_guidVolume, oidDB, &oi))
            {
            hr = HRESULT_FROM_WIN32(GetLastError());
            goto Error;
            }
        cDatabaseRecords = (UINT) oi.infDatabase.wNumRecords;
        ...
        do
            ...
            if((oidCur == CeReadRecordPropsEx(hdb, CEDB_ALLOWREALLOC, &cProps, &id, (BYTE**)&pVals, &cb, NULL)) &&
                ...
                m_cVisible++;
                if(m_cVisible >= cDatabaseRecords)
                    {
                    // I guess this could happen if someone added
                    // records to the database while we were running
                    // the scan.  Truncate the lookup table.  The worker
                    // window will be getting a database notify that
                    // the records changed which will cause this table
                    // to get re-built.
                    break;
                    }
                }
