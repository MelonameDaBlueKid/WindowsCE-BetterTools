    ...
    )
{
    ...
    if(INVALID_HANDLE_VALUE == m_hdb)
        ...
        m_hdb = CeOpenDatabase(&m_oiddb, const_cast<TCHAR*>(c_pszDatabaseName), CPR_TLA, 0, m_hwndWorker);
        if(INVALID_HANDLE_VALUE == m_hdb)
            {
            ...
            sos.propid = CPR_TLA;
            sos.dwFlags = CEDB_SORT_CASEINSENSITIVE;
            m_oiddb = CeCreateDatabase(const_cast<TCHAR*>(c_pszDatabaseName), 0, 1, &sos);
            if(NULL == m_oiddb)
                {
            ...
            m_hdb = CeOpenDatabase(&m_oiddb, NULL, CPR_TLA, 0, m_hwndWorker);
            if(INVALID_HANDLE_VALUE == m_hdb)
                {
    ...
    )
{
    ...
    if(0 == CeReadRecordProps(hdb, CEDB_ALLOWREALLOC, &cProps, &id, (BYTE**)&pVals, &cb))
        {
        hr = HRESULT_FROM_WIN32(GetLastError());
        goto Error;
        }
    ...
    )
{
    ...
    if(NULL == m_rgVisibleLookup)
        ...
        do
            {
            if((oidCur == CeReadRecordProps(hdb, CEDB_ALLOWREALLOC, &cProps, &id, (BYTE**)&pVals, &cb)) &&
    ...
    )
{
    ...
    cProps = 0;
    cb = 0;
    if(NULL == CeReadRecordProps(hdb, CEDB_ALLOWREALLOC, &cProps, NULL, (BYTE**)&pVals, &cb))
        {
        hr = HRESULT_FROM_WIN32(GetLastError());
        goto Error;
        }
