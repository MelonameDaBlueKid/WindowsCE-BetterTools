...
void CStocks::UpdateView( void )
{
    ...
    // If m_pStocks is NULL, we just want to clear the current items in the list.
    if (m_pStocks != NULL)
    {
    ...
    {
        // If m_pStocks is NULL, we can add stocks, but we cannot change or delete them.
        EnableMenuItem( GetMenu( m_hwndMain ), IDC_CHANGE, MF_GRAYED );
        EnableMenuItem( GetMenu( m_hwndMain ), IDC_DELETE, MF_GRAYED );
        EnableMenuItem( GetMenu( m_hwndMain ), IDC_ADD, MF_ENABLED );
    }
...
void CStocks::SyncNow
    Find the wcesmgr.exe in Windows CE Services and ask it to synchronize now
--*/
                       ...
                       0, KEY_READ, &hKey ) == ERROR_SUCCESS )
    {
        ...
        if ( RegQueryValueEx( hKey, "InstalledDir", 0, &dwType, (BYTE*)szExePath, &cb ) == ERROR_SUCCESS )
        {
            if ( szExePath[ lstrlen( szExePath ) - 1 ] != '\\')
                lstrcat( szExePath, "\\" );
            lstrcat( szExePath, "wcesmgr.exe /syncnow" );
            WinExec( szExePath, 0 );
        }
        RegCloseKey( hKey );
    }
