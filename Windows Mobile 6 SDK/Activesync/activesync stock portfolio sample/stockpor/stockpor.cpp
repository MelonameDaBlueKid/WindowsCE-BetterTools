...
int APIENTRY WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow )
{
    ...
    if ( SystemParametersInfo( SPI_GETPLATFORMTYPE,
                               ...
                               0 ) )
    {
        v_fSmartPhone = !lstrcmpi(szPlatform, _T("SmartPhone"));
    }
#endif
...
BOOL CALLBACK dlgAddChg( HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    ...
    case WM_INITDIALOG:
        ...
        mbi.hwndParent   = hDlg; 
        mbi.nToolBarId   = v_fAddStock ? IDR_SP_ADD_BAR : IDR_SP_CHANGE_BAR; // Menubar resource
        mbi.hInstRes   = v_hInstance; //HINST in which resource is located
    ...
    case WM_COMMAND:
        ...
        {
        case IDC_ADD_CHG:
        case IDM_STOCK_SK1_ADD:
        case IDM_STOCK_SK1_CHANGE:
        ...
        case IDC_PUR_PRICE:
            ...
            if ( (HIWORD( wParam ) == EN_CHANGE ))
            {
                ...
                if ( !uPur || !uLast )
                    StringCchCopy( sz, ARRAYSIZE(sz), TEXT( "N/A" ) );
                else if ( uPur < uLast )
                    StringCchPrintf( sz, ARRAYSIZE(sz), TEXT( "Gain %d%%" ), uLast * 100 / uPur - 100 );
                else if ( uPur > uLast )
                    StringCchPrintf( sz, ARRAYSIZE(sz), TEXT( "Loss %d%%" ), 100 - uLast * 100 / uPur );
                else
                    StringCchCopy( sz, ARRAYSIZE(sz), TEXT( "Even" ) );
...
BOOL CALLBACK dlgDetails( HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    int     iSel;
    ...
    case WM_INITDIALOG:
        {
            ...
            mbi.cbSize   = sizeof(SHMENUBARINFO);
            mbi.hwndParent   = hDlg; 
            mbi.nToolBarId   = IDR_SP_DETAIL_BAR; // Menubar resource
            mbi.hInstRes   = v_hInstance; //HINST in which resource is located
            ...
            else
            {
                ...
                if ( !ListView_GetItem( v_hwndLv, &lvi ) || !v_pStocks->SetupDlg( hDlg, lvi.lParam ) )
                {
                    MessageBox( hDlg, TEXT( "Unable to show details." ), TEXT( "Error" ), MB_OK | MB_ICONSTOP );
                    PostMessage( hDlg, WM_COMMAND, IDCANCEL, 0 );
                }
                else
                {
                    //
                    //  Fill in the details for the stock
                    //
                    v_pStocks->FillDetails(hDlg, (CEOID)lvi.lParam);
                }
            }
...
#ifdef UNDER_CE
        DialogBox( v_hInstance, 
            TEXT( "Add_Change_SP" ),
            v_hwndMain, (DLGPROC)dlgAddChg );
...
#ifdef UNDER_CE
    // On smartphone menus are handled through the soft keys
        SHMENUBARINFO mbi;

        memset(&mbi, 0, sizeof(SHMENUBARINFO)); 
        mbi.cbSize   = sizeof(SHMENUBARINFO);
        mbi.hwndParent   = hwnd; 
        mbi.nToolBarId   = IDR_SP_MENUBAR; // Menubar resource
        mbi.hInstRes   = v_hInstance; //HINST in which resource is located
...
void OnDetails()
{
    DialogBox( v_hInstance, TEXT( "SHOW_DETAILS" ), 
        v_hwndMain, (DLGPROC)dlgDetails);
    ...
    case WM_COMMAND:
        ...
        case IDC_OPEN:      
            // try open the default file first, if requested (lParam == 111)
            {
                TCHAR* pszPath;
#ifdef UNDER_CE   // {
                pszPath = SZ_DEFAULT_PORTFILE;
#else // } else {
                TCHAR szPath[MAX_PATH];

                if(SHGetSpecialFolderPath(hwnd, szPath, CSIDL_PERSONAL, TRUE))
                {
                    StringCchCat(szPath, ARRAYSIZE(szPath), SZ_DEFAULT_PORTFILE);
                }
                else
                {
                    StringCchCopy(szPath, ARRAYSIZE(szPath), SZ_DEFAULT_PORTFILE);
                }

                pszPath = szPath;
#endif // }
                if ( lParam == 111 && v_pStocks->Open( pszPath, TRUE ) )
                    break;
