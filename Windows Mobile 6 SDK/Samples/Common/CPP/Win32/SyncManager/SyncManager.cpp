...
// SyncManager.cpp : Defines the entry point for the application.
//
...
const WCHAR c_wszActiveSyncStatStore[]    = L"System\\State\\ActiveSync";

const WCHAR c_wszCradled[]                = L"Cradled";
const WCHAR c_wszSynchronizing[]          = L"Synchronizing";

HREGNOTIFY	g_CrdlReg;
HREGNOTIFY	g_SyncReg;
...
#ifndef WIN32_PLATFORM_WFSP
	HACCEL hAccelTable;
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_SYNCMANAGER));
#endif // !WIN32_PLATFORM_WFSP
...
ATOM MyRegisterClass(HINSTANCE hInstance, LPTSTR szWindowClass)
{
	...
	wc.hIcon         = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_SYNCMANAGER));
	wc.hCursor       = 0;
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    LoadString(hInstance, IDC_SYNCMANAGER, szWindowClass, MAX_LOADSTRING);
	...
	// creating the STATIC controls to display sync and cradle status
	...
	CreateWindow(TEXT("STATIC"), 
		         ...
		         WS_VISIBLE,
                 30, 38, 200, 14, 
				 hWnd, (HMENU)IDC_SYNC, hInstance, NULL);
	...
	CreateWindow(TEXT("STATIC"), 
		         ...
		         WS_VISIBLE,
                 30, 78, 200, 14, 
				 hWnd, (HMENU)IDC_CRADLED, hInstance, NULL);

	// initialising the status to be shown to the user
	OnInit();
...
#ifndef WIN32_PLATFORM_WFSP
                case IDM_QUIT:
					// unregister for Registry Key change updates
					Unregister();
                    SendMessage (hWnd, WM_CLOSE, 0, 0);				
                    break;
#endif // !WIN32_PLATFORM_WFSP
