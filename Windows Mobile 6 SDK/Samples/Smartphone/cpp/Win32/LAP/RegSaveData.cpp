...
HRESULT CRegistryBufferIterator::Init (HKEY hKey)
{
    ...
    if (SUCCEEDED(hr))
    {
        if (m_dwIndex != 0)
        {
            ...
            UINT cbBufferSize = m_cchMaxNameLen;

            // Add 1 for nul terminator & convert to byte count
            if (   safeIntUAdd (cbBufferSize, 1, &cbBufferSize)
                && safeIntUMul (cbBufferSize, sizeof (TCHAR), &cbBufferSize))
            {
                m_pszValueName = (TCHAR *)LocalAlloc (LMEM_FIXED, cbBufferSize);
                if (m_pszValueName)
                {
                    m_pbData = (BYTE *)LocalAlloc (LMEM_FIXED, m_cbMaxValueLen);
                    if (m_pbData)
                    {
                        hr = ReadCurrentData ();
                    }
