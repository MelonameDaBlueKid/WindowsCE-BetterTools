...
//       ------------------------------------------------------
//     
//       VU_NO_UI                Return from VerifyUser without displaying the UI. Returns TRUE only if user is not enrolled for verification.
//     
//     
//       VU_HANDLE_MASTER_KEY    Ignored.
//     
//     
//       VU_UNTIL_SUCCESS        Ignored.
//
//
                     ...
                     DWORD dwOptions, PVOID pExtended)
{
    ...
    if(g_fPolicyRequiresUpdate && SUCCEEDED(hr) && 
        !ISWINDOWSTORED(LAP_WINDOW_ENROLL))
    {
        RESERVEWINDOW(LAP_WINDOW_ENROLL);
        
        // Use the verification dialog as the parent.
        if(!DoEnrollment(GETSTOREDWINDOW(LAP_WINDOW_VERIFY), TRUE))
        {
            // If forced enrollment fails, consider it a cancel.
            hr = E_ABORT;
        }

        CLEARSTOREDWINDOW(LAP_WINDOW_ENROLL);
    }
    ...
    if(!ISWINDOWSTORED(LAP_WINDOW_ENROLL))
    {
                     ...
                     DWORD dwOptions, PVOID pExtended)
{
    ...
    if(LapGetPasswordOn() && !(dwOptions & VU_NO_UI))
    {
        ...
        if(SUCCEEDED(DoVerifyUserStart(NULL, pwszAEDisplayText)))
        {
            ...
            (void)DisplayPolicyUpdate(GETSTOREDWINDOW(LAP_WINDOW_VERIFY));        
...
//
// Remarks:
//   After the LASS makes the last VerifyUser (LAP) call, 
//     it calls VerifyUserStop. Must be 
//     preceded by VerifyUserStart.
// 
void VerifyUserStop()
{
    DoVerifyUserStop();
...
BOOL LAPCreateEnrollmentConfigDialog(HWND hwndParent, DWORD dwOptions)
{
    ...
    RESERVEWINDOW(LAP_WINDOW_ENROLL); // Reserve this window exclusively
    if(LASSVerifyUser(&AE_EVERY_TIME, CELOADSZ(IDS_AE), hwndParent, 0, NULL))
    {
        fRet = DoEnrollment(hwndParent, FALSE);
    }   
    CLEARSTOREDWINDOW(LAP_WINDOW_ENROLL); // Clear the window handle
...
BOOL LAPDisplayCodeword(HWND hwndParent, LPCWSTR pwszCodeword)
{
    ...
    {
        RESERVEWINDOW(LAP_WINDOW_CODEWORD); // Reserve this window exclusively
        // Use the verify dialog as the parent.
        fRet = DoCodeword(GETSTOREDWINDOW(LAP_WINDOW_VERIFY), pwszCodeword);
        CLEARSTOREDWINDOW(LAP_WINDOW_CODEWORD); // Clear the window handle
    }
...
BOOL LAPDisplayPreWipe(HWND hwndParent)
{
    ...
    {
        RESERVEWINDOW(LAP_WINDOW_PREWIPE); // Reserve this window exclusively
        // Use the verify dialog as the parent.
        fRet = SUCCEEDED(DisplayMessage(GETSTOREDWINDOW(LAP_WINDOW_VERIFY), 
            IDS_PREWIPE_TITLE, IDS_PREWIPE));
        CLEARSTOREDWINDOW(LAP_WINDOW_PREWIPE); // Clear the window handle
    }
...
BOOL LAPLockoutUser(HWND hwndParent, LPCWSTR pwszDisplayText, DWORD cSecondsToLockout)
{
    ...
    if(ISWINDOWSTORED(LAP_WINDOW_VERIFY) && IsWindow(GETSTOREDWINDOW(LAP_WINDOW_VERIFY)))
    {
        HWND hwnd = GETSTOREDWINDOW(LAP_WINDOW_VERIFY);
...
BOOL LAPCancelVerifySequence()
{
    ...
    if(ISWINDOWSTORED(LAP_WINDOW_CODEWORD))
    {
        (void)EndDialog(GETSTOREDWINDOW(LAP_WINDOW_CODEWORD), FALSE);
        fCancel = TRUE;
    }

    // Close the prewipe dialog, if it exists.
    if(ISWINDOWSTORED(LAP_WINDOW_PREWIPE))
    {
        (void)EndDialog(GETSTOREDWINDOW(LAP_WINDOW_PREWIPE), -1);
        fCancel = TRUE;
    }
