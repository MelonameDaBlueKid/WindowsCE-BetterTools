...
HWND    g_rghWindows[LAP_WINDOW_LAST + 1];
...
void SavePasswordForEnrollment(LPCTSTR pszPassword)
{
        ...
        &cchPassword)))
    {
        ...
        g_pszPreviousPassword = (LPTSTR)LocalAlloc(LMEM_FIXED, 
            (cchPassword + 1) * sizeof(TCHAR));
        if(NULL != g_pszPreviousPassword)
        {   
...
//     stored using the STOREWINDOW macro.
// 
BOOL SetWindowToTop(LAP_WINDOW lw)
{
    ...
    HWND hwnd = GETSTOREDWINDOW(lw);

    if(ISWINDOWRESERVED(lw))
    {
...
BOOL LASSVerifyUser(const GUID *AEKey, LPCWSTR pwszAEDisplayText, HWND hwndParent, DWORD dwOptions, PVOID pExtended)
{
    ...
    // Call FreeLibrary to be complete -- not necessary. 
    (void)FreeLibrary(hCoreDll);
            ...
            sizeof(dwAttempts), &dwSize) && 0 < dwAttempts)
        {
                ...
                sizeof(dwThreshold), &dwSize) && 0 < dwThreshold)
            {
                    ...
                    STRSAFE_MAX_CCH - (2 * MAX_INT_LENGTH), &cchAttempts)))
                {
                    ...
                    {
                        // Convert to bytes.
                        cbAttempts = cchAttempts * sizeof(TCHAR);
                        // Check for integer overflow.
                ...
                LPTSTR pszAttempts = (LPTSTR)LocalAlloc(LMEM_FIXED, 
                    cbAttempts);
                if(NULL != pszAttempts)
                {
                    // Format the number of attempts (x/y attempts).
                    if(SUCCEEDED(StringCbPrintf(pszAttempts, cbAttempts, 
                      ...
                      DWORD * pcbHash)
{
    ...
    BYTE* pbPlainText = (BYTE*)LocalAlloc(LMEM_FIXED, cbPlainText);
    if (!pbPlainText)
        return FALSE;
...
LONG GetEncryptedPasswordHash (DATA_BLOB *pblobData)
{
    ...
    if (lResult == ERROR_SUCCESS)
    {
        ...
        if (lResult == ERROR_SUCCESS)
        {
            ...
            blobTemp.pbData = static_cast<BYTE *>(LocalAlloc(LMEM_FIXED, blobTemp.cbData));
            if (blobTemp.pbData)
            {
...
{
    return DisplayMessageEx(hwndOwner, uTitle, SafeCELOADSZ(uMessage));
}
    ...
    switch(uMsg)
    {
        case WM_INITDIALOG:
        {
            ...
            else
            {
                ...
                if(ISWINDOWRESERVED(LAP_WINDOW_PREWIPE))
                {
                    STOREWINDOW(LAP_WINDOW_PREWIPE, hwndDlg);
                ...
                RECT rcMessage, rcMessageNew;
                if(GetWindowRect(hwndMessage, &rcMessage))
                {
                    // Get the font to use for drawing.
                    HDC hDC = GetDC(hwndMessage);
                    HFONT hfontOld = (HFONT)SelectObject(hDC, 
                        (HGDIOBJ)SendMessage(hwndMessage, WM_GETFONT, 0, 0));

                    // Calculate size that the message text will take.
                    (void)DrawText(hDC, pMsgInfo->pszMessage, -1, 
                        &rcMessageNew, 
                        DT_CALCRECT | DT_LEFT | DT_WORDBREAK | DT_NOCLIP);

                    // Resize control.
                    rcMessage.bottom = rcMessage.top + rcMessageNew.bottom;
                    ...
                    (void)ReleaseDC(hwndMessage, hDC);
                }

                // Set the message text.
                (void)SetWindowText(hwndMessage, pMsgInfo->pszMessage);

                // Size dialog full-screen.
                SHINITDLGINFO sidi;
                sidi.dwMask = SHIDIM_FLAGS;
                sidi.dwFlags = 
                    SHIDIF_SIZEDLGFULLSCREEN | SHIDIF_WANTSCROLLBAR;
                sidi.hDlg = hwndDlg;
                ...
                smbi.nToolBarId = (BUTTON_INFO_OK == pMsgInfo->biButtons ? 
                    IDR_MESSAGE_OK_MENUBAR : IDR_MESSAGE_YESNO_MENUBAR);
                ...
                (void)MessageBeep(BUTTON_INFO_YESNO == 
                    pMsgInfo->biButtons ? MB_ICONQUESTION : MB_ICONEXCLAMATION);
            }
