...
BOOL    g_authenticationResetUsed = FALSE;
...
HRESULT DoVerifyUserStart(HWND hwndParent, LPCWSTR pwszAEDisplayText)
{
    ...
    WndManager().Reserve(LAP_WINDOW_VERIFY); 
    ...
    if(NULL != hwndVerify)
    {
        ...
        WndManager()[LAP_WINDOW_VERIFY] = hwndVerify;
...
void DoVerifyUserStop()
{
    ...
    if(WndManager().IsStored(LAP_WINDOW_VERIFY) && 
        IsWindow(WndManager()[LAP_WINDOW_VERIFY]))
    {
        (void)DestroyWindow(WndManager()[LAP_WINDOW_VERIFY]);
    }

    // Make sure this window handle is cleared no matter what.
    WndManager().Clear(LAP_WINDOW_VERIFY);
}
...
void DoInitialize(HWND hwndDlg, LONG lParam)
{
    ...
    // Subclass password control (for Enter key).
    ...
    if(NULL != g_hwndPassword)
    {
        // Use our own window proc.
        WNDPROC pfnOrigProc = 
            (WNDPROC)SetWindowLong(g_hwndPassword, 
            GWL_WNDPROC, (LONG)PasswordSubclassProc);
        // Save the original window proc so it can be called.
        (void)SetWindowLong(g_hwndPassword, GWL_USERDATA, 
            (LONG)pfnOrigProc);
    }
    ...
    // This gets us the close button, and sizes correctly 
    // for the SIP.
    ...
    cbi.nToolBarId= IDR_VERIFY_MENUBAR;

    (void)SHCreateMenuBar(&cbi);

    // Register the talk hotkey for emergency calls.
    (void)SendMessage(cbi.hwndMB, SHCMBM_OVERRIDEKEY, 
        VK_TTALK, MAKELPARAM(SHMBOF_NODEFAULT | SHMBOF_NOTIFY, 
        SHMBOF_NODEFAULT | SHMBOF_NOTIFY));

    // Set the window caption.
    (void)SetWindowText(hwndDlg, 
        SafeCELOADSZ(IDS_PASSWD_CAPTION));

    // Make sure the SIP is up when we start.
    (void)SHSipPreference(hwndDlg, SIP_UP);

}


void DoInitMenu(UINT wParam)
{
    arc::ARState arstate;
    HRESULT hr = arc::CheckARState(arstate);
    unsigned int menuEnableFlag = MF_GRAYED;

    if (SUCCEEDED(hr) && arstate == arc::ENABLED)
    {
        menuEnableFlag = MF_ENABLED;
    }

    EnableMenuItem(reinterpret_cast<HMENU>(wParam)
        , IDM_RESET_PASSWORD
        , MF_BYCOMMAND | menuEnableFlag);
}


void DoUserEnable(HWND hwndDlg, UINT wParam, BOOL fIsEmergencyNum)
{
    ...
    ShowInfo(hwndDlg, fIsEmergencyNum, FALSE, FALSE, NULL);
    ...
    (void)SHDoneButton(hwndDlg, SHDB_SHOW);
...
void DoUserDisable(HWND hwndDlg)
{
    ...
    (void)SHDoneButton(hwndDlg, SHDB_HIDE);
...
BOOL DoHotKey(HWND hwndDlg, LONG lParam, BOOL fIsEmergencyNum)
{
    // User pressed TALK key. Handle if an emergency number is 
    // specified and we're not already in an emergency call.
    if(VK_TTALK == HIWORD(lParam) && !(LOWORD(lParam) & MOD_KEYUP) && 
        fIsEmergencyNum && !g_fEmergencyCall)
    {
        ...
        (void)PostMessage(hwndDlg, WM_APP_REGISTERCALLS, 0, 0);
        return TRUE;
    }
...
BOOL DoCommandOK(HWND hwndDlg)
{
    ...
    if(!g_fEmergencyCall && g_fEnabled)
    {
        ...
        if(!LapCheckPassword(szPassword))
        {
            ...
            else
            {
                    ...
                    0 < dwThreshold)
                {
                    ...
                    LPCTSTR pszFormat = CELOADSZ(IDS_INCORRECT_ATTEMPTS);
                    if(NULL != pszFormat)
                    {
                        ...
                        if(SUCCEEDED(
                            ...
                            STRSAFE_MAX_CCH - (2 * MAX_INT_LENGTH), 
                            &cchAttempts)))
                        {
                            // Add space for two 
                        ...
                        pszAttempts = new (localalloc) TCHAR[cchAttempts];
                        if(NULL != pszAttempts)
                        {
        ...
        else
        {
            ...
            // if the authentication reset has been used to reset a password,
            // prompt the user to setup AR anew
            if (g_authenticationResetUsed)
            {
                arc::SetupAR(hwndDlg, szPassword);
                g_authenticationResetUsed = FALSE;
            }
...
BOOL DoCommandPassword(HWND hwndDlg, UINT wParam)
{
    TCHAR szPassword[MAX_PASSWORD + 1];
...
void DoCommandResetPassword(HWND hwndDlg, UINT wParam)
{
    HRESULT hr;
#if defined(_DEBUG)
    // check if we are here because authentication reset is enabled
    arc::ARState arstate;
    hr = arc::CheckARState(arstate);
    ASSERT(FAILED(hr) || arstate == arc::ENABLED);
#endif

    // don't accept data from password field while trying to reset
    g_fEnabled = FALSE;
    KillIdleTimer(hwndDlg);

    hr = arc::RequestAR(hwndDlg);
    g_authenticationResetUsed = TRUE;

    if (SUCCEEDED(hr))
    {
        // password reset succeeded, disable the reset menu item
        EnableMenuItem(reinterpret_cast<HMENU>(wParam)
            , IDM_RESET_PASSWORD
            , MF_BYCOMMAND | MF_GRAYED);
    }

    // restore dialog behavior
    ResetIdleTimer(hwndDlg);
    g_fEnabled = TRUE;
}


void DoUserOnEdit(HWND hwndDlg, UINT wParam, BOOL fIsEmergencyNum)
{
    ...
    if(!g_fEmergencyCall)
    {
        ...
        if(DeviceHasPhone())
        {
            ...
            BOOL fOldIsEmergencyNumber = fIsEmergencyNum;
            fIsEmergencyNum = PhoneIsEmergencyNumber(FALSE, 
                pszPassword, NULL);

            // Take action if we have an emergency number 
            // or we were in emergency mode.
            if(fOldIsEmergencyNumber || fIsEmergencyNum)
            {
                // Do this synchronously so we don't 
                // have to worry about other messages.
                (void)SendMessage(hwndDlg, WM_APP_ENUMBER, 
                    MAKEWPARAM(fIsEmergencyNum, fOldIsEmergencyNumber), 
...
BOOL DoUserENumber(HWND hwndDlg, UINT wParam, LONG lParam)
{
...
BOOL DoUserActiveCalls(HWND hwndDlg, HREGNOTIFY & hRegNotify)
{
    ...
    if(0 == GetActiveCallCount())
    {
        ...
        UnregisterCallNotifications(hRegNotify);
...
BOOL DoTimer(HWND hwndDlg, UINT wParam, BOOL fIsEmergencyNum)
{
    BOOL fHandled = FALSE;
    ...
    case IDT_LOCKOUT:
        ...
        if(0 == g_csecLockout)
        {
            ...
            ShowInfo(hwndDlg, fIsEmergencyNum, FALSE, 
                TRUE, NULL);
        ...
        else
        {
            ...
            ShowInfo(hwndDlg, fIsEmergencyNum, 
...
BOOL VerifyDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_APP_ENABLE:
        DoUserEnable(hwndDlg, wParam, s_fIsEmergencyNum);
        fHandled = TRUE;
        break;
    
    case WM_APP_DISABLE:
        DoUserDisable(hwndDlg);
        fHandled = TRUE;
        break;

    case WM_HOTKEY:
        fHandled = DoHotKey(hwndDlg, lParam, s_fIsEmergencyNum);
        break;
    ...
    case WM_COMMAND:
    {
        switch(LOWORD(wParam))
        {
            ...
            case IDC_PASSWORD:
                fHandled = DoCommandPassword(hwndDlg, wParam);
                break;

            case IDM_RESET_PASSWORD:
                DoCommandResetPassword(hwndDlg, wParam);
                fHandled = TRUE;
                break;
        }
        break;
    ...
    case WM_APP_ONEDIT:
        DoUserOnEdit(hwndDlg, wParam, s_fIsEmergencyNum);
        fHandled = TRUE;
        break;

    case WM_APP_ENUMBER:
        fHandled = DoUserENumber(hwndDlg, wParam, lParam);
        break;
    ...
    case WM_APP_ACTIVECALLS:
        fHandled = DoUserActiveCalls(hwndDlg, s_hRegNotify);
        break;

    case WM_TIMER:
        fHandled = DoTimer(hwndDlg, wParam, s_fIsEmergencyNum);
        break;

    case WM_ACTIVATE:
        // Our window is being activated.
        if(WA_INACTIVE != LOWORD(wParam))
        {
            ...
            // Return FALSE to indicate that we handled this.
            fHandled = FALSE; 
        }
        break;
    ...
    case WM_LBUTTONDOWN:
        ...
        (void)SendMessage(hwndDlg, WM_NEXTDLGCTL, 
            (WPARAM)GetDlgItem(hwndDlg, IDC_PASSWORD), (LPARAM)TRUE);
...
// Summary:
//   Window proc for the password control.
//
// Parameters:
...
//
// Remarks:
//   The password control is subclassed here so that when
//     an enter key is pressed the dialog will emulate the OK
//     button. All messages are passed on to the standard 
//     edit field window proc.
// 
LRESULT PasswordSubclassProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if(WM_CHAR == uMsg && VK_RETURN == LOWORD(wParam))
    {
        ...
        (void)SendMessage(WndManager()[LAP_WINDOW_VERIFY], WM_COMMAND, 
            MAKEWPARAM(IDOK, 0), 0);
    }
    else
    {
...
//     goes to the password control (not including enter key
//     presses). All messages are passed on to the standard 
//     edit field window proc.
// 
LRESULT EmergencySubclassProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    // Pass on all input to the password control except enter key presses.
    if(WM_KEYDOWN == uMsg || WM_KEYUP == uMsg ||
        (WM_CHAR == uMsg && VK_RETURN != LOWORD(wParam)))
    {
